...
The proof is the rewritten key with the identity's revision; the key must hold exactly the
values the transition asked for. It authenticates the resulting state rather than this exact transition (the
