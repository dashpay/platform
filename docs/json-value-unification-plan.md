...
| `ValidatorSet` (V0) — Value path | `value_round_trip` | **Distinct bug surfaced after the BlsPublicKey fix:** `BTreeMap<ProTxHash, ValidatorV0>` map-key asymmetry. `platform_value::value_serialization::MapKeySerializer` reports `is_human_readable: true` (forcing `ProTxHash` through its hex-string serialize → `Value::Text` key), but `platform_value::Deserializer<Value>` reports `is_human_readable: false` (forcing `ProTxHash` through its bytes-expecting `BytesVisitor` on deserialize). Result: `"invalid type: string ..., expected bytes"`. Fix options: (a) make `MapKeySerializer` honor parent HR; (b) extend dashcore hash newtypes' `BytesVisitor` to also accept `visit_str` (UTF-8 → hex parse); (c) adjust platform_value's deserialize_bytes to also accept `Value::Text` as hex when used for hash-typed map keys. None are obviously safe — needs design pass. | 🟠 platform_value MapKeySerializer / dashcore hash BytesVisitor asymmetry |
