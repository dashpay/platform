...
use super::types::IOSSDKTokenFreezeParams;
use super::utils::{
...
use dash_sdk::platform::tokens::transitions::{FreezeResult, UnfreezeResult};
...
pub unsafe extern "C" fn ios_sdk_token_freeze(
    ...
    freezer_identity_handle: *const IdentityHandle,
    ...
    if sdk_handle.is_null()
        || freezer_identity_handle.is_null()
        || params.is_null()
    ...
    let freezer_identity = &*(freezer_identity_handle as *const Identity);
    ...
    match result {
        Ok(_freeze_result) => IOSSDKResult::success(std::ptr::null_mut()),
...
pub unsafe extern "C" fn ios_sdk_token_unfreeze(
    ...
    let result: Result<UnfreezeResult, FFIError> = wrapper.runtime.block_on(async {
        ...
        let result = wrapper
            .sdk
            .token_unfreeze(builder, identity_public_key, signer)
            .await
            .map_err(|e| {
