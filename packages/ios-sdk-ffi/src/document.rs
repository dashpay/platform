...
use dpp::document::{DocumentV0Getters, document_factory::DocumentFactory, Document};
...
use crate::types::{SDKHandle, DocumentHandle, DataContractHandle, IdentityHandle, IOSSDKDocumentInfo};
...
pub unsafe extern "C" fn ios_sdk_document_create(
    ...
    let result = wrapper.runtime.block_on(async {
        ...
        // Note: Actual publishing would require signing and broadcasting state transitions
        // This is a placeholder for the actual implementation
        Err::<Document, FFIError>(FFIError::NotImplemented(
            "Document creation requires wallet integration for signing".to_string()
        ))
    });
...
pub unsafe extern "C" fn ios_sdk_document_fetch(
    ...
    if sdk_handle.is_null() || data_contract_handle.is_null() || 
       document_type.is_null() || document_id.is_null() {
        return IOSSDKResult::error(IOSSDKError::new(
            IOSSDKErrorCode::InvalidParameter,
            "Invalid parameters".to_string(),
        ));
...
pub unsafe extern "C" fn ios_sdk_document_search(
    ...
    let result = wrapper.runtime.block_on(async {
        ...
        if !params.where_json.is_null() {
            let where_str = CStr::from_ptr(params.where_json).to_str()
        ...
        if !params.order_by_json.is_null() {
            let order_str = CStr::from_ptr(params.order_by_json).to_str()
                .map_err(|e| FFIError::from(e))?;
...
pub unsafe extern "C" fn ios_sdk_document_get_info(
    ...
    let data_contract_id_str = match CString::new(document.data_contract_id().to_string(Encoding::Base58)) {
        Ok(s) => s.into_raw(),
        Err(_) => {
            ios_sdk_string_free(id_str);
    ...
    let document_type_str = match CString::new(document.document_type_name()) {
        Ok(s) => s.into_raw(),
        Err(_) => {
            ios_sdk_string_free(id_str);
            ios_sdk_string_free(owner_id_str);
    ...
    let info = IOSSDKDocumentInfo {
        ...
        revision: document.revision() as u64,
