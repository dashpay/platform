...
//! to rebuild the rich query from the wire request so a proved
//! response can be verified against exactly what was asked.
//!
//! This is a client-side **mirror** of the server's decode —
//! rs-drive-abci's `query/document_query/v1/conversions.rs` — and
//! must be kept in lockstep with it, exactly as the v0 path's CBOR
//! clause decoding mirrors `query_documents_v0`. The bytes the
//! server decodes and the bytes the verifier decodes must agree
//! clause-for-clause, or a proof could verify against a different
//! query than the server answered. Function bodies and error
//! message strings are copied verbatim from the server; any change
//! on either side must be replayed on the other.
//!
//! Conversion contract:
...
//!   capability not yet wired). These mirror the server's
//!   `QueryError::InvalidArgument` / `QuerySyntaxError::Unsupported`
//!   respectively, preserving its error surface.
...
/// Neutral decode error for the proto → drive conversions.
///
/// Mirrors the two error shapes the server's decode produces
/// (`QueryError::InvalidArgument` and `QuerySyntaxError::Unsupported`)
/// without depending on them; the client-side `DocumentQuery`
/// decoding maps it onto the crate [`Error`](crate::error::Error).
