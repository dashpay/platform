...
//!
//! Every `extern "Rust"` body runs under [`std::panic::catch_unwind`]:
//! cxx turns a `Result::Err` into a C++ `rust::Error`, but a panic that
//! reaches its shim is a deterministic abort of the embedding process. All
//! inputs here are untrusted DAPI bytes, so a panic anywhere in the decoders
//! is reported as an error instead. (This only helps under `panic = "unwind"`;
//! the crate must not be built with `panic = "abort"`.)
...
mod ffi {
    ...
    extern "Rust" {
        ...
        /// refused), the protocol version the network runs (state
        /// transitions are built under it) and the Platform activation core
        /// height (0 = unknown; only consulted by query paths that require
        /// it). Clears any previously pushed quorum keys.
        fn set_context(
            network_id: &str,
        ...
        fn update_quorum_keys(keys: Vec<FfiQuorumKey>) -> Result<()>;
...
fn update_quorum_keys(keys: Vec<ffi::FfiQuorumKey>) -> Result<(), String> {
    guarded("update_quorum_keys", || update_quorum_keys_inner(keys))
}

fn update_quorum_keys_inner(keys: Vec<ffi::FfiQuorumKey>) -> Result<(), String> {
    let keys = keys
        .into_iter()
        .map(|key| {
    ...
    provider::update_quorum_keys(keys)
}
...
fn verify_get_identity_nonce(
    ...
    guarded("verify_get_identity_nonce", || {
...
fn verify_get_identity_contract_nonce(
    ...
    guarded("verify_get_identity_contract_nonce", || {
...
fn verify_get_identity_by_pubkey_hash(
    ...
    guarded("verify_get_identity_by_pubkey_hash", || {
...
fn verify_get_contested_vote_state(
    ...
    guarded("verify_get_contested_vote_state", || {
        verify_get_contested_vote_state_inner(request, response)
    })
}

fn verify_get_contested_vote_state_inner(
    request: &[u8],
    response: &[u8],
) -> Result<ffi::FfiVerifiedContested, String> {
...
fn decode_dpns_domain(doc_bytes: &[u8]) -> Result<ffi::FfiDpnsName, String> {
    guarded("decode_dpns_domain", || decode_dpns_domain_inner(doc_bytes))
}

fn decode_dpns_domain_inner(doc_bytes: &[u8]) -> Result<ffi::FfiDpnsName, String> {
...
fn decode_dashpay_profile(doc_bytes: &[u8]) -> Result<ffi::FfiProfile, String> {
    guarded("decode_dashpay_profile", || {
        decode_dashpay_profile_inner(doc_bytes)
    })
}

fn decode_dashpay_profile_inner(doc_bytes: &[u8]) -> Result<ffi::FfiProfile, String> {
...
fn decode_contact_request(doc_bytes: &[u8]) -> Result<ffi::FfiContactRequest, String> {
    guarded("decode_contact_request", || {
        decode_contact_request_inner(doc_bytes)
    })
}

fn decode_contact_request_inner(doc_bytes: &[u8]) -> Result<ffi::FfiContactRequest, String> {
...
fn st_build_dpns_preorder(
    ...
    guarded("st_build_dpns_preorder", || {
...
fn st_build_contact_request(
    ...
    guarded("st_build_contact_request", || {
        check_key_id(signature_public_key_id, &key)?;
        let handle = SignerHandle(signer);
        let sign_fn = |key_id: u32, digest: [u8; 32]| handle.sign(key_id, digest);
