...
/// The platform version transitions are built under: the one the connected
/// network runs, as recorded by `set_context`, so the wire format matches
/// what that network accepts rather than whatever this build calls latest.
fn platform_version() -> Result<&'static PlatformVersion, String> {
    let protocol_version = context()?.protocol_version;
    PlatformVersion::get(protocol_version)
        .map_err(|e| format!("network protocol version {protocol_version} is unknown: {e}"))
}
...
/// assembled document. dpp's builder refuses an entropy that does not derive
/// the document id (Drive would reject it after the nonce bump), and the
/// properties are sanitized for the document type first, as `dash-sdk`'s
/// put-document path does.
...
pub fn build_identity_create(
    ...
    built(&state_transition, platform_version()?)
}
