...
use dash_platform_queries::dpns_usernames::{
    build_dpns_domain_document, build_dpns_preorder_document, salted_domain_hash,
};
...
impl Signer<IdentityPublicKey> for CallbackSigner<'_> {
    ...
    fn can_sign_with(&self, key: &IdentityPublicKey) -> bool {
        is_ecdsa(key.key_type())
    }
}

/// The callback answers with a compact recoverable secp256k1 signature, so
/// only ECDSA keys can be signed for; dpp checks purpose, security level and
/// disabled state, but not the key type.
fn is_ecdsa(key_type: KeyType) -> bool {
...
fn identity_key_from_info(info: &KeyInfo) -> Result<IdentityPublicKey, String> {
    let key_type = KeyType::try_from(info.key_type)
        .map_err(|e| format!("bad key type {}: {e}", info.key_type))?;
    if !is_ecdsa(key_type) {
        return Err(format!(
            "key {} is {key_type:?}; the signing callback can only produce ECDSA signatures",
            info.id
        ));
    }
...
/// Serializes a signed transition for broadcast, after the same structure
/// validation `dash-sdk` runs before it broadcasts, so a transition Drive
/// would reject on shape fails here instead of after a nonce bump.
fn built(
    state_transition: &StateTransition,
    version: &PlatformVersion,
) -> Result<BuiltTransition, String> {
    ensure_valid_state_transition_structure(state_transition, version)
        .map_err(|e| format!("state transition failed structure validation: {e}"))?;
...
/// Which batch transition to build from an assembled document.
enum BatchKind {
    Create { entropy: [u8; 32] },
    Replace,
}

/// Builds and signs a single-document batch transition from an assembled
/// document. The properties are sanitized for the document type first, as
/// `dash-sdk`'s put-document path does; for a create, dpp's builder refuses
/// an entropy that does not derive the document id (Drive would reject it
/// after the nonce bump).
#[allow(clippy::too_many_arguments)]
fn build_document_transition(
    ...
    let (state_transition, what) = match kind {
        BatchKind::Create { entropy } => (
    ...
    let state_transition = state_transition
        .map_err(|e| format!("unable to build {document_type_name} {what} transition: {e}"))?;
    built(&state_transition, version)
}
...
fn build_document_create(
    ...
    let contract = load_contract(contract, version)?;
...
/// DPNS preorder create. The document comes from the shared builder in
...
pub fn build_dpns_preorder(
    ...
    let entropy = salted_domain_hash(label, salt);
    let contract = load_contract(SystemDataContract::DPNS, version)?;
    let preorder =
        build_dpns_preorder_document(&contract, Identifier::from(owner), label, salt, entropy)
...
pub fn build_dpns_domain(
    ...
    // The shared builder registers under the "dash" TLD; that is the only
    ...
    let salt = id32(preorder_salt, "preorder salt")?;
    let contract = load_contract(SystemDataContract::DPNS, version)?;
    let domain = build_dpns_domain_document(&contract, Identifier::from(owner), label, salt, salt)
...
pub fn build_profile(
    ...
    for (name, text) in [
        ...
        if !text.is_empty() {
            properties.insert(name.to_string(), Value::Text(text.to_string()));
        }
    }
    ...
    let contract = load_contract(SystemDataContract::Dashpay, version)?;
    let document = Document::V0(DocumentV0 {
...
pub fn build_contact_request(
    ...
    let entropy = id32(entropy, "entropy")?;
    let contract = load_contract(SystemDataContract::Dashpay, version)?;
    // The shared builder checks the crypto-material sizes against the
    // contract schema and assembles the DIP-15 property map; $createdAt and
    // $createdAtCoreBlockHeight are chain-assigned system fields, so they do
...
pub fn build_identity_create(
    ...
    let asset_lock_proof = match proof {
        AssetLockProofInput::Instant {
            ...
            // Both are consensus-bounded Core messages; anything larger than a
            // block is not one.
            const MAX_CORE_MESSAGE: usize = 2 * 1024 * 1024;
            if transaction.len() > MAX_CORE_MESSAGE || instant_lock.len() > MAX_CORE_MESSAGE {
                return Err("asset lock transaction or instant lock exceeds 2 MiB".to_string());
            }
