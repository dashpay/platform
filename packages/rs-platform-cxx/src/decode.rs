...
use crate::verify::MAX_MESSAGE_BYTES;

/// The decoders run on bytes the embedder took out of a verified response;
/// they are still bounded like every other input crossing the bridge.
...
fn decode_document(
    ...
    // Stored documents are decoded under the network's protocol version (the
    // one they were proved under), as recorded by set_context.
    let protocol_version = context()?.protocol_version;
    let version = PlatformVersion::get(protocol_version)
