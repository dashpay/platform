...
//! Flattening of dpp identities and DPNS / DashPay documents into the plain
//! types the bridge exposes. Documents arrive here already verified by the
//! SDK; the byte decoders exist for embedders that persisted serialized
...
/// Largest input the byte decoders accept. Anything bigger is not a
/// Platform object and would only make the decoders allocate.
pub const MAX_MESSAGE_BYTES: usize = 16 * 1024 * 1024;

fn check_size(bytes: &[u8], what: &str) -> Result<(), String> {
...
pub fn decode_identity_public_key(bytes: &[u8]) -> Result<KeyInfo, String> {
    check_size(bytes, "identity public key")?;
    let key = IdentityPublicKey::deserialize_from_bytes(bytes)
        .map_err(|e| format!("bad identity public key: {e}"))?;
    Ok(key_info(&key))
}

/// Decodes a stored system-contract document under `version` (the protocol
/// version it was proved under).
pub fn decode_document(
    ...
    check_size(bytes, document_type_name)?;
...
pub fn dpns_domain(document: &Document) -> Result<DpnsName, String> {
...
pub fn dashpay_profile(document: &Document) -> Result<Profile, String> {
    let properties = document.properties();
    Ok(Profile {
        document_id: document.id().to_buffer(),
        owner_id: document.owner_id().to_buffer(),
...
pub fn contact_request(document: &Document) -> Result<ContactRequest, String> {
    let properties = document.properties();
...
pub fn decode_dpns_domain(doc_bytes: &[u8], version: &PlatformVersion) -> Result<DpnsName, String> {
    dpns_domain(&decode_document(
        doc_bytes,
...
pub fn decode_dashpay_profile(
    doc_bytes: &[u8],
    version: &PlatformVersion,
) -> Result<Profile, String> {
    dashpay_profile(&decode_document(
        doc_bytes,
...
pub fn decode_contact_request(
    doc_bytes: &[u8],
    version: &PlatformVersion,
) -> Result<ContactRequest, String> {
    contact_request(&decode_document(
        doc_bytes,
        SystemDataContract::Dashpay,
        "contactRequest",
        version,
