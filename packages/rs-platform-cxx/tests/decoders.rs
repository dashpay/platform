...
fn decode_stored_dpns_domain() {
    ...
    let name =
        decode::decode_dpns_domain(&hexv(vector["serialized_hex"].as_str().unwrap()), version())
...
fn decode_stored_dashpay_profile() {
    ...
    let profile = decode::decode_dashpay_profile(
        &hexv(vector["serialized_hex"].as_str().unwrap()),
        version(),
    )
...
fn decode_stored_contact_request() {
    ...
    let request = decode::decode_contact_request(
        &hexv(vector["serialized_hex"].as_str().unwrap()),
        version(),
    )
...
fn decode_rejects_placeholder_documents() {
    assert!(decode::decode_dpns_domain(b"proved-dpns-document", version()).is_err());
    assert!(decode::decode_dashpay_profile(b"proved-profile-document", version()).is_err());
    assert!(decode::decode_contact_request(b"proved-contact-document", version()).is_err());
}

/// Decoder inputs are bounded like every other byte crossing the bridge.
#[test]
fn oversized_decoder_inputs_are_refused() {
    let huge = vec![0u8; dash_platform_cxx::decode::MAX_MESSAGE_BYTES + 1];
    for result in [
        decode::decode_identity(&huge).map(|_| ()),
        decode::decode_identity_public_key(&huge).map(|_| ()),
        decode::decode_dpns_domain(&huge, version()).map(|_| ()),
    ] {
        let err = result.expect_err("oversized input");
