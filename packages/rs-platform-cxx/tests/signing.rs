...
/// The vectors were generated at Platform v4.0.0 / protocol version 12;
/// build under that version so the wire format matches the pinned bytes.
fn version() -> &'static PlatformVersion {
    PlatformVersion::get(12).expect("protocol version 12")
}
...
fn identity_create_sorts_and_rejects_duplicate_keys() {
    ...
    reversed.reverse();
    let built = st::build_identity_create(version(), proof(), &reversed, &|key_id, digest| {
    ...
    duplicated[1].id = 0;
    let err = st::build_identity_create(version(), proof(), &duplicated, &|key_id, digest| {
        signer.sign(key_id, digest)
    })
    .unwrap_err();
...
/// The callback produces compact recoverable ECDSA signatures, so a key of
/// any other type must be refused before signing is attempted.
#[test]
fn non_ecdsa_key_is_refused() {
    let doc = vectors();
    let vector = &doc["dpns_preorder"];
    let signer = TestSigner::from_vectors(&doc);
    let mut key = high_key(&doc);
    key.key_type = 1; // BLS12_381
    let err = st::build_dpns_preorder(
        version(),
        &hexv(vector["owner_id_hex"].as_str().unwrap()),
        1,
...
/// A signer answering with something other than a 65-byte compact signature
/// is a wallet bug; it must surface as an error, not as a malformed
/// transition.
#[test]
fn wrong_signature_size_is_reported() {
    let doc = vectors();
    let vector = &doc["dpns_preorder"];
    let err = st::build_dpns_preorder(
        version(),
        ...
        &hexv(vector["salt_hex"].as_str().unwrap()),
        &high_key(&doc),
        &|_key_id, _digest| Some(vec![0u8; 64]),
    )
    .unwrap_err();
    assert!(err.contains("signature size"), "{err}");
}

/// A label the DPNS contract pattern refuses fails before anything is
/// signed.
#[test]
fn invalid_dpns_label_is_refused_before_signing() {
    let doc = vectors();
    let signer = TestSigner::from_vectors(&doc);
    let err = st::build_dpns_domain(
        version(),
    ...
    assert!(
        signer.digests.lock().unwrap().is_empty(),
