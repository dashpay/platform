...
dependencies. A consumer selects the `server` or `verify` feature to
match the `drive` build it already carries (`verify` is the default;
`rs-drive-abci` opts out and takes `server`), so the server gains no
proof-verification code and a client verifier gains no server code.
