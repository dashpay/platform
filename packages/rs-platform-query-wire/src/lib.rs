...
//! verification, no networking. Consumers select the `server` or
//! `verify` feature to match their own `drive` build (`verify` is the
//! default; rs-drive-abci opts out and takes `server`).
