...
pub fn hash_double_to_vec(payload: impl AsRef<[u8]>) -> Vec<u8> {
    Sha256::digest(Sha256::digest(payload)).to_vec()
}

pub fn hash_double(payload: impl AsRef<[u8]>) -> [u8; 32] {
    Sha256::digest(Sha256::digest(payload)).into()
}

pub fn hash_single(payload: impl AsRef<[u8]>) -> [u8; 32] {
    Sha256::digest(payload).into()
}

pub fn hash_to_hex_string(payload: impl AsRef<[u8]>) -> String {
    hex::encode(hash_double(payload))
}
