...
impl DecodeProtocolEntity {
    pub fn decode_protocol_entity<T: PlatformDeserializable>(buffer: impl AsRef<[u8]>) -> Result<(u32, T), ProtocolError> {
    ...
    pub fn decode_protocol_entity_to_value<T: PlatformDeserializable>(buffer: impl AsRef<[u8]>) -> Result<(u32, Value), ProtocolError> {
        let SplitProtocolVersionOutcome {
            protocol_version,
            main_message_bytes: document_bytes,
            ..
        ...
        Ok((
            protocol_version,
            protocol_entity.to_object(false)?,
        ))
    }
}
