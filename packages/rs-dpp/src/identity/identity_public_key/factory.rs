...
impl IdentityPublicKey {
    pub fn random_key_with_rng(id: KeyID, rng: &mut StdRng, used_key_matrix : Option<(KeyCount, &mut UsedKeyMatrix)>) -> Result<Self, ProtocolError> {
        ...
        if key_count > 64 {
            return Err(ProtocolError::PublicKeyGenerationError("too many keys already created".to_string()));
        }
        ...
        let mut counter = 0;
        key_matrix.into_iter().enumerate().for_each(|(pos, is_set)| {
            if !*is_set {
        ...
        let needed_pos = needed_pos.ok_or(ProtocolError::PublicKeyGenerationError("too many keys already created".to_string()))?;
    ...
    pub fn random_unique_keys_with_rng(key_count: KeyCount, rng: &mut StdRng) -> Result<Vec<Self>, ProtocolError> {
        let mut keys = [false;64].to_vec();
        (0..key_count)
            .map(|i| {
                Self::random_key_with_rng(i, rng, Some((i, &mut keys)))
            }).collect()
    }
}
