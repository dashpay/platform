...
#[derive(Debug, Clone)]
#[cfg_attr(all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"), derive(Serialize, Deserialize))]
pub struct ExtendedDocumentV0 {
    /// The document type name, stored as a string.
    #[cfg_attr(all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"), serde(rename = "$type"))]
    pub document_type_name: String,

    /// The identifier of the associated data contract.
    #[cfg_attr(
    all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"),
        serde(rename = "$dataContractId")
    )]
    pub data_contract_id: Identifier,

    /// The actual document object containing the data.
    #[cfg_attr(all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"), serde(flatten))]
    pub document: Document,

    /// The data contract associated with the document.
    #[cfg_attr(all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"), serde(rename = "$dataContract"))]
    ...
    #[cfg_attr(all(feature = "document-serde-conversion", feature = "data-contract-serde-conversion"), serde(rename = "$entropy"))]
    pub entropy: Bytes32,
}
...
impl ExtendedDocumentV0 {
    ...
    pub fn hash(&self, platform_version: &PlatformVersion) -> Result<Vec<u8>, ProtocolError> {
        Ok(hash_to_vec(ExtendedDocumentPlatformConversionMethodsV0::serialize(self, platform_version)?))
    }
