...
fn validate_raw_transitions<'a>(
    ...
    for raw_document_transition in raw_document_transitions.iter() {
        ...
        let document_action = match raw_document_transition.get_string("$action") {
            Ok(da) => da,
            Err(_) => {
        ...
        match action {
            Action::Create | Action::Replace => {
                ...
                let schema_validator = JsonSchemaValidator::new(schema)
                    .map_err(|e| anyhow!("unable to compile schema: {}", e))?;
