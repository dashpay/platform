...
use crate::document::document_transition::{DocumentBaseTransition, DocumentTransition, DocumentTransitionObjectLike};
...
pub fn find_duplicates_by_id<'a>(
    ...
    for transition in document_transitions {
        let fingerprint = create_fingerprint(&transition).ok_or(anyhow!("Can't create fingerprint from a document transition"))?;
