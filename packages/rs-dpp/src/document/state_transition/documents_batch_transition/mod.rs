...
use crate::prelude::{Identifier, DocumentTransition};
use crate::util::json_value::{JsonValueExt, ReplaceWith};
// TODO simplify imports
use crate::document::document_transition::DocumentTransitionObjectLike;
use crate::{state_transition::{StateTransitionConvert, StateTransitionLike, StateTransitionType, StateTransitionIdentitySigned}, identity::{KeyID, SecurityLevel}, prelude::{IdentityPublicKey, Identity}};
...
impl DocumentsBatchTransition {
	...
	pub fn from_raw_object(mut raw_object : JsonValue, data_contracts : Vec<DataContract>) -> Result<Self, ProtocolError> {
		let mut batch_transitions = DocumentsBatchTransition {
			...
			signature_public_key_id: raw_object.get_u64(PROPERTY_SIGNATURE_PUBLIC_KEY_ID).unwrap_or_default(),
		...
		if let Ok(JsonValue::Array(raw_transitions))  = maybe_transitions {
			...
			let data_contracts_map : HashMap<Vec<u8>, DataContract> =  data_contracts.into_iter().map(|dc| (dc.id.to_buffer().to_vec(), dc)).collect();

			for raw_transition in raw_transitions {
				let id  = raw_transition.get_bytes(PROPERTY_DATA_CONTRACT_ID)?;
				let data_contract = data_contracts_map.get(&id).ok_or_else(|| anyhow!("Data Contract doesn't exists for Transition: {:?}", raw_transition))?;
				let document_transition = DocumentTransition::from_raw_document(raw_transition, data_contract.clone())?;
    ...
    // requirement is the highest level across all documents affected by the ST./
		...
		for transition in self.transitions.iter() {
			...
			if let Ok(document_schema)  = maybe_document_schema {
				let document_security_level = get_security_level_requirement(document_schema, DEFAULT_SECURITY_LEVEL);
...
impl StateTransitionConvert for DocumentsBatchTransition {
	...
	fn to_json(&self) -> Result<JsonValue, ProtocolError> {
			...
			json_value.insert(String::from(PROPERTY_TRANSITIONS), JsonValue::Array(transitions))?;
	...
	fn to_object(&self, skip_signature: bool) -> Result<JsonValue, ProtocolError> {
			...
			json_object.insert(String::from(PROPERTY_TRANSITIONS), JsonValue::Array(transitions))?;
