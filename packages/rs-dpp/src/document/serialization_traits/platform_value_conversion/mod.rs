...
impl DocumentPlatformValueMethodsV0<'_> for Document {
    ...
    /// Legacy-shape ingest. Routes by the platform's
    /// `document_structure_version` into the correct V0 / V1 / ... inner.
    /// Distinct from canonical `ValueConvertible::from_object` (which
    /// dispatches on the value's own `$formatVersion` tag) — used to
    /// accept un-tagged legacy values such as DPNS / DashPay JSON
    /// fixtures and older stored Document shapes.
