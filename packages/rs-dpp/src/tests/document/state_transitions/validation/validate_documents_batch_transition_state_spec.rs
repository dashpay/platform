...
async fn should_return_invalid_result_if_timestamps_mismatch() {
    ...
    let now_ts = Utc::now().timestamp_millis() as TimestampMillis;
    state_transition
        .transitions
        .iter_mut()
...
async fn should_return_invalid_result_if_crated_at_has_violated_time_window() {
    ...
    let now_ts_minus_6_mins = (Utc::now().timestamp_millis() as u128
        - Duration::from_secs(60 * 6).as_millis()) as TimestampMillis;
    state_transition
        .transitions
        .iter_mut()
...
async fn should_not_validate_time_in_block_window_on_dry_run() {
    ...
    let now_ts_minus_6_mins = (Utc::now().timestamp_millis()
        - Duration::from_secs(60 * 6).as_millis() as i64)
        as TimestampMillis;
    state_transition
        .transitions
        .iter_mut()
