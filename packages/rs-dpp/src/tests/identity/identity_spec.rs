...
mod from_buffer {
    ...
    use crate::tests::identity::identity_spec::{DummyStruct, get_buffer};
    ...
    pub fn should_parse_hex_from_js_dpp() {
        ...
        let vec = decode_hex(real_hex).unwrap();
        let val: Vec<u8> = ciborium::de::from_reader(vec.as_slice()).unwrap();
        println!("{:?}", val);
        ...
        let identity = Identity::from_buffer(identity_buffer).unwrap();
        println!("{:?}", identity);
    }
}
