...
pub fn setup_test() -> (u32, u32, HashMap<u32, u32>) {
    ...
    (current_protocol_version, latest_protocol_version, version_compatibility_map)
}

#[test]
pub fn should_throw_unsupported_protocol_version_error_if_protocol_version_is_higher_than_latest_version() {
    let (current_protocol_version, latest_protocol_version, version_compatibility_map) = setup_test();
    ...
    let error = match consensus_error {
        ...
        _ => { panic!("Expected UnsupportedProtocolVersionError") }
    };
...
pub fn should_throw_incompatible_protocol_version_error_if_parsed_version_is_lower_than_compatible_one() {
    ...
    let validator = ProtocolVersionValidator::new(current_protocol_version, current_protocol_version, version_compatibility_map);

    let  result = validator.validate(protocol_version).unwrap();
    let consensus_error = result.first_error().unwrap();
    let error =  match consensus_error {
        ConsensusError::IncompatibleProtocolVersionError(err) => { err }
        _ => { panic!("Expected IncompatibleProtocolVersionError") }
    };
