...
mod tests {
    ...
    #[test]
    /// Mainnet state transition 35C08D574302D32D7160E603D8159042C8606BE12FD97D952CF5FD40DB57313C
    /// (block 361774, idx 1, status FAIL, gas 41880).
    ///
    /// Pinned because:
    /// - Its bytes are what platform-explorer's indexer choked on with
    ///   `duplicate key value violates unique constraint state_transition_hash`
    ///   (issue #2867 mainnet escalation, 2026-05-04). The same hash appears in
    ///   a later block the explorer hasn't indexed yet — the second copy is
    ///   what triggers the unique-key panic.
    /// - The chat thread misidentified it as a credit transfer; the bytes are
    ///   actually a single Documents Batch Replace on a DPNS-like profile doc.
    /// - Identity_contract_nonce == 3, while a sibling tx in the same block
    ///   (5962DA04... at idx 0) succeeded with nonce == 4. So the failure path
    ///   is the out-of-order in-block case, not a stale replay against committed
    ///   state — narrowing the regression target to the failed-Replace nonce
    ///   bookkeeping in batch/transformer/state.
    fn should_decode_mainnet_35c0_documents_batch_replace_replay() {
        ...
        const EXPECTED_STATE_TRANSITION_HASH: &str =
            "35C08D574302D32D7160E603D8159042C8606BE12FD97D952CF5FD40DB57313C";
        const RAW_TRANSACTION_BASE64: &str = "";
        const EXPECTED_OWNER_BASE58: &str = "6Pp1RFqRnpStnY8vmp5k3ypE6rFBvzPwcoguwDXbRA7F";
        const EXPECTED_NONCE: u64 = 3;

        let raw = STANDARD
            .decode(RAW_TRANSACTION_BASE64)
            .expect("base64 decodes");
        let state_transition = StateTransition::deserialize_from_bytes(&raw)
            .expect("dpp deserializes the wire bytes");
        ...
        let StateTransition::Batch(batch) = &state_transition else {
            panic!("expected Batch transition, got {state_transition:?}");
        };

        assert_eq!(
            batch.owner_id().to_string(Encoding::Base58),
            EXPECTED_OWNER_BASE58
        );
        ...
        let only = batch
            .first_transition()
            .expect("expected exactly one batched transition");

        let BatchedTransitionRef::Document(document_transition) = only else {
            panic!("expected document transition, got {only:?}");
        };

        assert!(
            matches!(document_transition, DocumentTransition::Replace(_)),
            "expected Replace transition, got {document_transition:?}",
        );

        assert_eq!(only.identity_contract_nonce(), EXPECTED_NONCE);
    }
