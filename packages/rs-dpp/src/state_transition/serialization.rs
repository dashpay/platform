...
mod tests {
    ...
    fn identity_create_transition_ser_de() {
        ...
        let bytes = state_transition.serialize_to_bytes().expect("expected to serialize");
        let recovered_state_transition =
            StateTransition::deserialize_from_bytes(&bytes).expect("expected to deserialize state transition");
