...
use crate::state_transition::state_transitions::identity_topup_from_addresses_transition::fields::*;
...
impl ValueConvertible<'_> for IdentityTopUpFromAddressesTransition {}
...
impl StateTransitionValueConvert<'_> for IdentityTopUpFromAddressesTransition {
    ...
    fn from_object(
        ...
        match version {
            0 => Ok(IdentityTopUpFromAddressesTransitionV0::from_object(
                raw_object,
                platform_version,
            )?
            .into()),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown IdentityTopUpFromAddressesTransition version {n}"
            ))),
        }
    }
    ...
    fn from_value_map(
        ...
        match version {
            0 => Ok(IdentityTopUpFromAddressesTransitionV0::from_value_map(
                raw_value_map,
                platform_version,
            )?
            .into()),
    ...
    fn clean_value(value: &mut Value) -> Result<(), ProtocolError> {
        ...
        match version {
            0 => IdentityTopUpFromAddressesTransitionV0::clean_value(value),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown IdentityTopUpFromAddressesTransition version {n}"
            ))),
        }
