...
impl StateTransitionValueConvert for IdentityTopUpTransition {
    ...
    fn from_raw_object(
        ...
        match version {
            0 => Ok(IdentityTopUpTransitionV0::from_raw_object(raw_object)?.into()),
    ...
    fn from_value_map(
        ...
        match version {
            0 => Ok(IdentityTopUpTransitionV0::from_value_map(
                raw_data_contract_topup_transition,
            )?
                .into()),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown IdentityTopUpTransition version {n}"
    ...
    fn clean_value(value: &mut Value) -> Result<(), ProtocolError> {
        ...
        match version {
            0 => {
                IdentityTopUpTransitionV0::clean_value(
                    value,
                )
            },
            n => Err(ProtocolError::UnknownVersionError(format!(
