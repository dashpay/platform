...
impl IdentityTopUpTransitionV0Methods for IdentityTopUpTransition {
    ...
    fn try_from_identity(identity: Identity, asset_lock_proof: AssetLockProof, asset_lock_proof_private_key: &[u8], bls: &impl BlsModule, version: FeatureVersion) -> Result<Self, ProtocolError> {
        match version { 0 => {
            Ok(IdentityTopUpTransitionV0::try_from_identity(identity, asset_lock_proof, asset_lock_proof_private_key, bls,  version)?.into())
        }
    ...
    fn set_asset_lock_proof(&mut self, asset_lock_proof: AssetLockProof) {
        match self { IdentityTopUpTransition::V0(transition) =>transition.set_asset_lock_proof(asset_lock_proof) }
    }
