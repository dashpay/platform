...
use crate::state_transition::state_transitions::identity_credit_transfer_to_single_key_transition::fields::*;
...
impl ValueConvertible<'_> for IdentityCreditTransferToSingleKeyTransition {}

impl StateTransitionValueConvert<'_> for IdentityCreditTransferToSingleKeyTransition {
    ...
    fn from_object(
        ...
        match version {
            0 => Ok(
                IdentityCreditTransferToSingleKeyTransitionV0::from_object(raw_object, platform_version)?
                    .into(),
            ),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown IdentityCreditTransferToSingleKeyTransition version {n}"
            ))),
        }
    }
    ...
    fn from_value_map(
        ...
        match version {
            0 => Ok(IdentityCreditTransferToSingleKeyTransitionV0::from_value_map(
                raw_value_map,
                platform_version,
            )?
            .into()),
    ...
    fn clean_value(value: &mut Value) -> Result<(), ProtocolError> {
        ...
        match version {
            0 => IdentityCreditTransferToSingleKeyTransitionV0::clean_value(value),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown IdentityCreditTransferToSingleKeyTransition version {n}"
            ))),
        }
