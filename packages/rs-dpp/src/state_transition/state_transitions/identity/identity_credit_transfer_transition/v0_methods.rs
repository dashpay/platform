...
impl IdentityCreditTransferTransitionV0Methods for IdentityCreditTransferTransition {
    ...
    fn set_identity_id(&mut self, identity_id: Identifier) {
        match self {
            IdentityCreditTransferTransition::V0(transition) => transition.set_identity_id(identity_id),
        }
    }
    ...
    fn set_recipient_id(&mut self, recipient_id: Identifier) {
        match self {
            IdentityCreditTransferTransition::V0(transition) => transition.set_recipient_id(recipient_id),
        }
    }
