...
impl IdentityCreditWithdrawalTransitionMethodsV0 for IdentityCreditWithdrawalTransitionV0 {
    ...
    fn try_from_identity<S: Signer>(identity: &Identity, output_script: CoreScript, amount: u64, pooling: Pooling, core_fee_per_byte: u32, signer: S, _platform_version: &PlatformVersion, _version: Option<FeatureVersion>) -> Result<StateTransition, ProtocolError> {
        ...
        let identity_public_key = identity.get_first_public_key_matching(Purpose::WITHDRAW, SecurityLevel::full_range().into(), KeyType::all_key_types().into()).ok_or(ProtocolError::DesiredKeyWithTypePurposeSecurityLevelMissing("no withdrawal public key".to_string()))?;

        transition.sign_external(identity_public_key, &signer, None::<GetDataContractSecurityLevelRequirementFn>)?;
