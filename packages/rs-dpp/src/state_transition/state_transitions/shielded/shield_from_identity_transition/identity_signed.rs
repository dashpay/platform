...
impl StateTransitionIdentitySigned for ShieldFromIdentityTransition {
    fn signature_public_key_id(&self) -> KeyID {
        match self {
            ShieldFromIdentityTransition::V0(transition) => transition.signature_public_key_id(),
        }
    }
    ...
    fn purpose_requirement(&self) -> Vec<Purpose> {
        match self {
            ShieldFromIdentityTransition::V0(transition) => transition.purpose_requirement(),
        }
    }
}
