...
impl StateTransitionIdentitySigned for UTXOTransferTransition {
    fn signature_public_key_id(&self) -> KeyID {
        match self {
            UTXOTransferTransition::V0(transition) => transition.signature_public_key_id(),
        }
    }
    ...
    fn purpose_requirement(&self) -> Vec<Purpose> {
        match self {
            UTXOTransferTransition::V0(transition) => transition.purpose_requirement(),
        }
    }
}
