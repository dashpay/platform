...
impl StateTransitionValueConvert<'_> for AddressCreditWithdrawalTransition {
        ...
        let version: FeatureVersion = raw_object
            ...
            .unwrap_or({
                platform_version
                    ...
                    .address_credit_withdrawal_state_transition
                    .default_current_version
            });

        match version {
            0 => Ok(AddressCreditWithdrawalTransitionV0::from_object(
                raw_object,
                platform_version,
            )?
            .into()),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown AddressCreditWithdrawalTransition version {n}"
            ))),
        }
    }
    ...
    fn from_value_map(
        ...
        match version {
            0 => Ok(AddressCreditWithdrawalTransitionV0::from_value_map(
                raw_value_map,
                platform_version,
            )?
            .into()),
    ...
    fn clean_value(value: &mut Value) -> Result<(), ProtocolError> {
        ...
        match version {
            0 => AddressCreditWithdrawalTransitionV0::clean_value(value),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown AddressCreditWithdrawalTransition version {n}"
            ))),
        }
