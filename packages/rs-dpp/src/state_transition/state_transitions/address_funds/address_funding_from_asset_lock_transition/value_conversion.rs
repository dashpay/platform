...
use crate::state_transition::state_transitions::address_funding_from_asset_lock_transition::fields::*;
...
impl StateTransitionValueConvert<'_> for AddressFundingFromAssetLockTransition {
    ...
    fn clean_value(value: &mut Value) -> Result<(), ProtocolError> {
        ...
        match version {
            0 => AddressFundingFromAssetLockTransitionV0::clean_value(value),
            n => Err(ProtocolError::UnknownVersionError(format!(
                "Unknown AddressFundingFromAssetLockTransition version {n}"
            ))),
        }
