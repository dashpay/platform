...
use crate::state_transition::documents_batch_transition::token_issuance_transition::v0::v0_methods::TokenIssuanceTransitionV0Methods;
...
impl TokenIssuanceTransitionV0Methods for TokenIssuanceTransition {
    ...
    fn entropy(&self) -> [u8; 32] {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.entropy,
        }
    }

    fn set_entropy(&mut self, entropy: [u8; 32]) {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.entropy = entropy,
        }
    }

    fn data(&self) -> &BTreeMap<String, Value> {
        match self {
            TokenIssuanceTransition::V0(v0) => &v0.data,
        }
    }

    fn data_mut(&mut self) -> &mut BTreeMap<String, Value> {
        match self {
            TokenIssuanceTransition::V0(v0) => &mut v0.data,
        }
    }

    fn set_data(&mut self, data: BTreeMap<String, Value>) {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.data = data,
        }
    }

    fn prefunded_voting_balance(&self) -> &Option<(String, Credits)> {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.prefunded_voting_balance(),
        }
    }

    fn prefunded_voting_balances_mut(&mut self) -> &mut Option<(String, Credits)> {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.prefunded_voting_balances_mut(),
        }
    }

    fn set_prefunded_voting_balance(&mut self, index_name: String, amount: Credits) {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.set_prefunded_voting_balance(index_name, amount),
        }
    }

    fn clear_prefunded_voting_balance(&mut self) {
        match self {
            TokenIssuanceTransition::V0(v0) => v0.clear_prefunded_voting_balance(),
        }
    }
}
