...
pub(crate) mod json_convertible_tests {
    ...
    pub(crate) fn fixture() -> TokenEmergencyActionTransition {
    ...
    fn json_round_trip_with_full_wire_shape() {
        ...
        // `TokenEmergencyAction` has `rename_all = "camelCase"` so unit
        // variant `Pause` serializes as the bare string `"pause"`.
        // `emergencyAction` / `publicNote` come from `rename_all =
        ...
        let recovered = TokenEmergencyActionTransition::from_json(json).expect("from_json");
    ...
    fn value_round_trip_with_full_wire_shape() {
        ...
        let recovered = TokenEmergencyActionTransition::from_object(value).expect("from_object");
        assert_eq!(original, recovered);
    }
}
