...
pub use v0::TokenMintToPoolTransitionV0;

/// Mints `amount` of a token straight into the token's shielded pool.
///
/// The Orchard bundle is outputs-only (spends disabled) with a value balance of `-amount`: the
/// new notes are created for whoever the minter addressed them to. The minter must be authorized
/// by `manualMintingRules` like a transparent mint, the max supply applies, and a group action
/// carries the digest of the actions so every signer commits to the same notes.
...
pub enum TokenMintToPoolTransition {
    #[display("V0({})", "_0")]
    #[cfg_attr(feature = "serde-conversion", serde(rename = "0"))]
    V0(TokenMintToPoolTransitionV0),
}

#[cfg(all(feature = "json-conversion", feature = "serde-conversion"))]
impl crate::serialization::JsonConvertible for TokenMintToPoolTransition {}

#[cfg(all(feature = "value-conversion", feature = "serde-conversion"))]
impl crate::serialization::ValueConvertible for TokenMintToPoolTransition {}

impl Default for TokenMintToPoolTransition {
    fn default() -> Self {
        TokenMintToPoolTransition::V0(TokenMintToPoolTransitionV0::default())
    }
}
...
pub(crate) mod json_convertible_tests {
    ...
    pub(crate) fn fixture() -> TokenMintToPoolTransition {
    ...
    fn json_round_trip_keeps_envelope_and_bundle() {
        ...
        let recovered = TokenMintToPoolTransition::from_json(json).expect("from_json");
    ...
    fn value_round_trip() {
        ...
        let recovered = TokenMintToPoolTransition::from_object(value).expect("from_object");
        assert_eq!(original, recovered);
    }
}
