...
pub(crate) mod json_convertible_tests {
    ...
    pub(crate) fn fixture() -> DocumentTransferTransition {
    ...
    fn json_round_trip_with_full_wire_shape() {
        ...
        // `V0` for the flattened `base`. `$identityContractNonce` and
        // `$revision` are `u64`; JSON erases the size.
        assert_eq!(
            json,
            json!({
                "$formatVersion": "0",
                "$baseFormatVersion": "0",
                    ...
                    "$revision": 4,
                    "recipientOwnerId": Identifier::new([0xee; 32]),
            })
        );
        let recovered = DocumentTransferTransition::from_json(json).expect("from_json");
    ...
    fn value_round_trip_with_full_wire_shape() {
        ...
        // `11u64`/`4u64`: `IdentityNonce` and `Revision` are `u64` aliases.
        assert_eq!(
            value,
            platform_value!({
                "$formatVersion": "0",
        ...
        let recovered = DocumentTransferTransition::from_object(value).expect("from_object");
        assert_eq!(original, recovered);
    }
}
