...
impl StateTransitionIdentityEstimatedFeeValidation for BatchTransition {
    fn validate_estimated_fee(
        ...
        let purchases_amount = match self.all_document_purchases_amount() {
            ...
            Err(ProtocolError::Overflow(e)) => {
                return SimpleConsensusValidationResult::new_with_error(ConsensusError::BasicError(
                    BasicError::OverflowError(OverflowError::new(e.to_owned())),
                ))
            }
        ...
        let conflicting_indices_collateral_amount = match self
        ...
        let base_fees = self.calculate_estimated_fee(platform_version);
