...
impl DocumentTypeRef<'_> {
    ...
    /// be enforced separately. Failing it where stored documents are re-decoded
    /// by a per-block handler would otherwise be unrecoverable.
...
mod tests {
    ...
    mod validate_byte_array_encoding {
        ...
        fn assert_rejected(old_ba: platform_value::Value, new_ba: platform_value::Value) {
            ...
            let result = old
                .as_ref()
                .validate_update_v0(new.as_ref(), platform_version)
