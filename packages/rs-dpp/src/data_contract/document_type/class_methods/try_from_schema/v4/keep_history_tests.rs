//! The keep-history document lifecycle as generation 4 admits it: a
...
/// code uses (generation 4 at protocol version 15, 3 at 14, 2 at 12 and 13).
fn parse_at_version(
    schema: Value,
...
/// The released parsers must keep parsing the same schemas the same way, so a
/// historical block replays identically: generation 2 never saw a rule about
/// the pair, and generation 3 refuses it under full validation.
#[test]
fn should_keep_the_released_parsers_verdict_on_a_keep_history_deletable_type() {
    ...
    assert!(
        parse_at_version(keep_history_deletable_schema(), 14, true).is_err(),
        "protocol 14 refuses a deletable keep-history type under full validation"
    );
}
