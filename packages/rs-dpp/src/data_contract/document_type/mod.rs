...
impl<'a> DocumentTypeV0Methods for DocumentTypeRef<'a> {
    fn index_for_types<'b>(
        &'b self,
        index_names: &[&str],
        in_field_name: Option<&str>,
        order_by: &[&str],
        platform_version: &PlatformVersion,
    ) -> Result<Option<(&'b Index, u16)>, ProtocolError> {
        match self {
...
impl DocumentTypeV0Methods for &DocumentType {
    ...
    fn serialize_value_for_key(
        ...
        match self {
            DocumentType::V0(v0) => v0.serialize_value_for_key(key, value, platform_version),
        }
    }
    ...
    fn max_size(&self, platform_version: &PlatformVersion) -> Result<u16, ProtocolError> {
        match self {
            DocumentType::V0(v0) => v0.max_size(platform_version),
        }
    }

    fn estimated_size(&self, platform_version: &PlatformVersion) -> Result<u16, ProtocolError> {
        match self {
            DocumentType::V0(v0) => v0.estimated_size(platform_version),
        }
    }

    fn unique_id_for_storage(&self) -> [u8; 32] {
        match self {
            DocumentType::V0(v0) => v0.unique_id_for_storage(),
        }
    }
    ...
    fn unique_id_for_document_field(
        ...
        match self {
            DocumentType::V0(v0) => v0.unique_id_for_document_field(index_level, base_event),
        }
    }

    fn initial_revision(&self) -> Option<Revision> {
        match self {
            DocumentType::V0(v0) => v0.initial_revision(),
        }
    }

    fn requires_revision(&self) -> bool {
        match self {
            DocumentType::V0(v0) => v0.requires_revision(),
        }
    }

    fn top_level_indices(&self) -> Vec<&IndexProperty> {
        match self {
            DocumentType::V0(v0) => v0.top_level_indices(),
        }
    }

    fn top_level_indices_of_contested_unique_indexes(&self) -> Vec<&IndexProperty> {
        match self {
            DocumentType::V0(v0) => v0.top_level_indices_of_contested_unique_indexes(),
        }
    }
    ...
    fn create_document_from_data(
        ...
        match self {
            DocumentType::V0(v0) => v0.create_document_from_data(
                data,
                owner_id,
                block_height,
                core_block_height,
    ...
    fn create_document_with_prevalidated_properties(
        ...
        match self {
            DocumentType::V0(v0) => v0.create_document_with_prevalidated_properties(
                id,
                owner_id,
                block_height,
                core_block_height,
    ...
    fn contested_vote_poll_for_document(
        ...
        match self {
            DocumentType::V0(v0) => v0.contested_vote_poll_for_document(document, platform_version),
        }
    }
}
