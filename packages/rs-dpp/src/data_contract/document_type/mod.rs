...
impl<'a> DocumentTypeV0Methods for DocumentTypeRef<'a> {
    ...
    fn index_for_types(&self, index_names: &[&str], in_field_name: Option<&str>, order_by: &[&str], platform_version: &PlatformVersion) -> Result<Option<(&Index, u16)>, ProtocolError> {
        match self {
            DocumentTypeRef::V0(v0) => v0.index_for_types(index_names, in_field_name, order_by, platform_version)
        }
    }
    ...
    fn document_field_type_for_property(&self, property: &str, platform_version: &PlatformVersion) -> Result<Option<DocumentFieldType>, ProtocolError> {
        match self {
            DocumentTypeRef::V0(v0) => v0.document_field_type_for_property(property, platform_version)
        }
    }
