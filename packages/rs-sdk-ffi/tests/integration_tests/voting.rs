...
fn test_voting_vote_polls_by_end_date() {
    ...
    unsafe {
        let result = dash_sdk_voting_get_vote_polls_by_end_date(
            handle,
            0,      // start_time_ms (0 = no start filter)
        ...
        if let Some(groups_array) = json.as_array() {
            for group in groups_array {
                assert!(group.get("timestamp").is_some(), "Group should have timestamp");
                assert!(group.get("vote_polls").is_some(), "Group should have vote_polls");
    ...
    unsafe {
        ...
        if let Some(groups_array) = json.as_array() {
            for group in groups_array {
                ...
                assert!(timestamp >= start_time_ms, 
                       "Timestamp {} should be >= start time {}", timestamp, start_time_ms);
                assert!(timestamp < end_time_ms,
                       "Timestamp {} should be < end time {}", timestamp, end_time_ms);
            }
        }
    }
...
fn test_voting_vote_polls_by_end_date_paginated() {
    ...
    unsafe {
        ...
        if groups1.len() >= 3 {
            ...
            if !groups2.is_empty() {
                let last_timestamp_page1 = groups1.last().unwrap()
                    .get("timestamp").unwrap().as_u64().unwrap();
                let first_timestamp_page2 = groups2[0]
                    .get("timestamp").unwrap().as_u64().unwrap();
                
                assert!(first_timestamp_page2 >= last_timestamp_page1,
    ...
    unsafe {
        ...
        if let Some(groups_array) = json.as_array() {
            for group in groups_array {
                ...
                assert!(timestamp >= current_time_ms,
