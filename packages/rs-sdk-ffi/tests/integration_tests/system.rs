    ...
    unsafe {
        ...
        if let Some(epochs_array) = epochs.as_array() {
            ...
            for epoch in epochs_array {
                ...
                assert!(epoch.get("first_block_height").is_some(), "Epoch should have first_block_height");
                assert!(epoch.get("first_core_block_height").is_some(), "Epoch should have first_core_block_height");
                assert!(epoch.get("start_time").is_some(), "Epoch should have start_time");
                assert!(epoch.get("fee_multiplier").is_some(), "Epoch should have fee_multiplier");
            }
            ...
            if epochs_array.len() > 1 {
                ...
                assert!(first_index < second_index, "Epochs should be in ascending order");
            }
        }
    }
        ...
        for (_quorum_type, quorum_list) in quorums.as_object().unwrap() {
            ...
            if let Some(quorum_array) = quorum_list.as_array() {
                for quorum in quorum_array {
                    ...
                    assert!(quorum.get("active_members").is_some(), "Quorum should have active_members");
                    assert!(quorum.get("created_at").is_some(), "Quorum should have created_at");
                }
            }
        }
    }
...
fn test_total_credits_in_platform() {
    ...
    unsafe {
        ...
        assert!(json.is_object(), "Expected object, got: {:?}", json);
        assert!(json.get("total_credits").is_some(), "Should have total_credits field");
        
        let total_credits = json.get("total_credits").unwrap();
        assert!(total_credits.is_string() || total_credits.is_number(), 
...
fn test_path_elements() {
    ...
    unsafe {
        let result = dash_sdk_system_get_path_elements(handle, path_query.as_ptr(), keys_query.as_ptr());
