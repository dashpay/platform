    ...
    unsafe {
        ...
        if let Some(upgrades_array) = json.as_array() {
            for upgrade in upgrades_array {
                ...
                assert!(upgrade.get("version_number").is_some(), "Should have version_number");
                assert!(upgrade.get("vote_count").is_some(), "Should have vote_count");
                
                let version_number = upgrade.get("version_number").unwrap();
                assert!(version_number.is_number(), "Version number should be a number");
