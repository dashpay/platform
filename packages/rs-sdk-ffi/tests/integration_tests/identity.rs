...
fn test_identity_read() {
    ...
    unsafe {
        ...
        assert!(json.get("publicKeys").is_some(), "Identity should have publicKeys field");
    }
    ...
    unsafe {
        ...
        if let Some(keys_array) = keys.as_array() {
            if !keys_array.is_empty() {
                ...
                assert!(first_key.get("type").is_some(), "Key should have type field");
                assert!(first_key.get("purpose").is_some(), "Key should have purpose field");
                assert!(first_key.get("securityLevel").is_some(), "Key should have securityLevel field");
            }
        }
    }
