...
fn test_contested_resources() {
    ...
    unsafe {
        ...
        assert!(json.is_object(), "Expected object, got: {:?}", json);
        assert!(json.get("contested_resources").is_some(), "Should have contested_resources field");
        
        let resources = json.get("contested_resources").unwrap();
        assert!(resources.is_array(), "Contested resources should be an array");
    }
    ...
    unsafe {
        ...
        match parse_string_result(result) {
            Ok(Some(json_str)) => {
                ...
                assert!(json.get("abstain_vote_tally").is_some(), "Should have abstain_vote_tally");
                assert!(json.get("lock_vote_tally").is_some(), "Should have lock_vote_tally");
                ...
                if let Some(contenders) = json.get("contenders") {
                    if let Some(contenders_array) = contenders.as_array() {
                        for contender in contenders_array {
                            assert!(contender.get("document").is_none() || 
