...
/// Fetches the total shielded pool balance.
///
/// # Parameters
/// * `sdk_handle` - Handle to the SDK instance
///
/// # Returns
/// * String with the total shielded pool balance (u64 as decimal string)
...
#[no_mangle]
pub unsafe extern "C" fn dash_sdk_shielded_get_pool_state(
    ...
    let result = rt.block_on(async move {
        ShieldedPoolState::fetch_current(&sdk)
            .await
            .map_err(|e| format!("Failed to fetch shielded pool state: {}", e))
    });

    match result {
        Ok(ShieldedPoolState(balance)) => match CString::new(balance.to_string()) {
