...
pub unsafe extern "C" fn dash_sdk_identity_transfer_credits(
    ...
    let signing_key = if public_key_id == 0 {
        ...
        eprintln!("🔵 dash_sdk_identity_transfer_credits: Looking for key with ID {}", public_key_id);
        match from_identity.get_public_key_by_id(public_key_id.into()) {
            Some(key) => {
                eprintln!("🔵 dash_sdk_identity_transfer_credits: Found key with ID {}", public_key_id);
                eprintln!("🔵 dash_sdk_identity_transfer_credits: Key purpose: {:?}", key.purpose());
                eprintln!("🔵 dash_sdk_identity_transfer_credits: Key type: {:?}", key.key_type());
                Some(key)
            },
            None => {
                eprintln!("❌ dash_sdk_identity_transfer_credits: Key with ID {} not found!", public_key_id);
                return DashSDKResult::error(DashSDKError::new(
                    DashSDKErrorCode::InvalidParameter,
