...
use rs_sdk::{
    mock::MockResponse,
    platform::{
        dapi::transport::TransportRequest,
...
type Config = rs_sdk::mock::config::Config<TestConfig>;
...
async fn main() {
    ...
    let identity_test_cases: BTreeMap<&'static str, Identifier> = BTreeMap::from([
        ("identity_ok", cfg.settings.existing_identity_id.clone()),
    ...
    let contract_tc: BTreeMap<&'static str, Identifier> = BTreeMap::from([
        (
            "data_contract_ok",
            cfg.settings.existing_data_contract_id.clone(),
        ),
    ...
    let document_tc: BTreeMap<&'static str, DocumentQuery> = BTreeMap::from([
        (
            ...
            base_query
                .clone()
                .with_document_id(&cfg.settings.existing_document_id),
        ),
        (
            "document_not_found",
...
fn copy_dumps(dump_dir: &Path, test_name: &str, cfg: &Config) {
    ...
    let dump_dest = cfg.dump_dir.join(format!("{}.json", test_name));
    ...
    let key_dest = cfg.dump_dir.join(key_src.file_name().unwrap());
