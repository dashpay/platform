...
pub mod drive_internal_server {
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with DriveInternalServer.
    #[async_trait]
    pub trait DriveInternal: std::marker::Send + std::marker::Sync + 'static {
        async fn get_proofs(
            &self,
    ...
    #[derive(Debug)]
    pub struct DriveInternalServer<T> {
        inner: Arc<T>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for DriveInternalServer<T>
    where
        T: DriveInternal,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/org.dash.platform.drive.v0.DriveInternal/getProofs" => {
                    ...
                    struct getProofsSvc<T: DriveInternal>(pub Arc<T>);
                    impl<T: DriveInternal> tonic::server::UnaryService<super::GetProofsRequest> for getProofsSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as DriveInternal>::get_proofs(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for DriveInternalServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "org.dash.platform.drive.v0.DriveInternal";
    impl<T> tonic::server::NamedService for DriveInternalServer<T> {
