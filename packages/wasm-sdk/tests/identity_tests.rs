...
use wasm_sdk::{
    asset_lock::{AssetLockProof, create_identity_with_asset_lock, validate_asset_lock_proof},
    fetch::{fetch_identity, FetchOptions},
    fetch_unproved::fetch_identity_unproved,
    identity_info::{fetch_identity_balance, fetch_identity_info, check_identity_balance, estimate_credits_needed},
    nonce::{get_identity_nonce, increment_identity_nonce},
    state_transitions::identity::{create_identity, update_identity, topup_identity},
};
...
async fn test_asset_lock_proof_creation() {
    ...
    let instant_proof = AssetLockProof::create_instant(
        transaction.clone(),
        0,
        instant_lock.clone()
    );
    assert!(instant_proof.is_ok(), "Should create instant asset lock proof");
...
async fn test_create_identity_state_transition() {
    ...
    js_sys::Reflect::set(&key_obj, &"data".into(), &js_sys::Uint8Array::from(&test_public_key()[..])).unwrap();
...
async fn test_update_identity_state_transition() {
    ...
    assert!(result.is_ok(), "Should create update identity state transition");
}
...
async fn test_topup_identity_state_transition() {
    ...
    assert!(result.is_ok(), "Should create topup identity state transition");
}
...
async fn test_fetch_identity() {
    ...
    assert!(result_with_options.is_ok(), "Should fetch identity with options");
}
...
async fn test_estimate_credits() {
    ...
    for (op_type, data_size, expected_base) in operations {
        ...
        assert!(credits.unwrap() >= expected_base, "Credits should be at least base cost");
    }
}
