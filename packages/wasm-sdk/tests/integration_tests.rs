...
use wasm_sdk::{
    ...
    fetch::{fetch_data_contract, fetch_documents, fetch_identity, FetchOptions},
    ...
    signer::WasmSigner,
    state_transitions::{
        broadcast::broadcast_state_transition, document::DocumentBatchBuilder,
...
async fn test_full_identity_workflow() {
    ...
    signer
        .add_private_key(0, test_private_key(), "ECDSA_SECP256K1", 0)
        .unwrap();
    ...
    let transition = put_identity(asset_lock_proof, public_keys, None, 0);
    ...
    let broadcast_result =
        broadcast_state_transition(&sdk, signed.unwrap(), Some(request_settings)).await;
...
async fn test_document_management_workflow() {
    ...
    for i in 0..5 {
        ...
        batch
            .add_create_document(&contract_id, "post", &format!("doc{}", i), data.into())
            .unwrap();
    }
    ...
    assert_eq!(
        contracts.as_f64().unwrap() as u32,
        1,
        "Should have cached contract"
    );
}
...
async fn test_optimized_sdk_with_minimal_features() {
    ...
    assert!(
        token_result.is_err(),
        "Token operations should fail with minimal features"
    );
}
...
async fn test_retry_logic_with_request_settings() {
    ...
    assert!(
        result.is_err(),
        "Should fail to fetch non-existent identity"
    );
...
async fn test_concurrent_operations() {
    ...
    let identity_ids = vec![test_identity_id(), "identity2", "identity3"];

    let contract_ids = vec![test_contract_id(), "contract2", "contract3"];
...
async fn test_error_propagation_across_layers() {
    ...
    assert!(
        invalid_query.is_err(),
        "Should fail with invalid contract ID"
    );
    ...
    assert!(
        ...
        "Should fail to broadcast empty transition"
    );
}

#[wasm_bindgen_test]
async fn test_memory_optimization() {
    use wasm_sdk::optimize::{optimize_uint8_array, MemoryOptimizer};
...
async fn test_complete_application_flow() {
    ...
    assert!(
        total_entries.as_f64().unwrap() > 0.0,
