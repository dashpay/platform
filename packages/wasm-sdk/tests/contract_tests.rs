...
use wasm_sdk::{
    contract_history::{
        check_contract_updates, fetch_contract_history, fetch_contract_versions,
        get_migration_guide, get_schema_changes,
    },
...
async fn test_create_data_contract() {
    ...
    assert!(
        result.is_ok(),
        "Should create data contract state transition"
    );
    assert!(
...
async fn test_update_data_contract() {
    ...
    assert!(
        result.is_ok(),
        "Should create update data contract state transition"
    );
}
...
async fn test_fetch_data_contract() {
    ...
    assert!(
        ...
        "Should fetch data contract with options"
    );
}
...
async fn test_schema_changes() {
    ...
    assert!(
        invalid_changes.is_err(),
        "Should fail with invalid version range"
    );
}
