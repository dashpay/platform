...
fn test_signer_error_handling() {
    ...
    let invalid_key_result = signer.add_private_key(0, test_private_key(), "INVALID_KEY_TYPE", 0);
    assert!(
    ...
    assert!(
        remove_result.is_ok(),
        "Should not error on removing non-existent key"
    );
    assert!(
        !remove_result.unwrap(),
