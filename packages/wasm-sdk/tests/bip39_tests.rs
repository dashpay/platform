...
fn test_mnemonic_to_seed() {
    let phrase =
        "abandon ability able about above absent absorb abstract absurd abuse access accident";
    let mnemonic =
        Mnemonic::from_phrase(phrase, WordListLanguage::English).expect("Should create mnemonic");
    ...
    let seed_with_pass = mnemonic
        .to_seed(Some("test".to_string()))
...
fn test_mnemonic_to_hd_private_key() {
    ...
    // Test mainnet
    let mainnet_key = mnemonic
        .to_hd_private_key(None, "mainnet")
        .expect("Should generate mainnet HD key");
    assert!(mainnet_key.starts_with("xprv"));

    // Test testnet
    let testnet_key = mnemonic
        .to_hd_private_key(None, "testnet")
...
fn test_generate_entropy() {
    ...
    let entropy_128 =
        generate_entropy(MnemonicStrength::Words12).expect("Should generate 128-bit entropy");
    assert_eq!(entropy_128.len(), 16); // 128 bits = 16 bytes

    let entropy_256 =
        generate_entropy(MnemonicStrength::Words24).expect("Should generate 256-bit entropy");
...
fn test_mnemonic_to_seed_hex() {
    ...
    let seed_hex = mnemonic_to_seed_hex(phrase, None).expect("Should convert mnemonic to seed hex");
