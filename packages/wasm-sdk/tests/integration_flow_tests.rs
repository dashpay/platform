...
use wasm_sdk::{
    bip39::*, contract_history::*, identity_info::*, monitoring::*, prefunded_balance::*,
    sdk::WasmSdk, signer::WasmSigner,
};
...
async fn test_complete_identity_workflow() {
    ...
    let exists = check_identity_exists(&sdk, test_identity)
        .await
...
async fn test_contract_deployment_workflow() {
    ...
    if let Ok(history) = history_result {
        let history_array = history.dyn_ref::<Array>().expect("Should be an array");
    ...
    let callback =
        js_sys::Function::new_with_args("update", "console.log('Contract update:', update);");

    let monitor_result = monitor_contract_updates(&sdk, contract_id, callback, Some(2000)).await;
...
async fn test_monitoring_with_operations() {
    ...
    let operations = vec![
        ("identity_check", async {
            let _ =
                check_identity_exists(&sdk, "GWRSAVFMjXx8HpQFaNJMqBV7MBgMK4br5UESsB4S31Ec").await;
        }),
        ("balance_check", async {
            let _ =
                get_identity_balance(&sdk, "GWRSAVFMjXx8HpQFaNJMqBV7MBgMK4br5UESsB4S31Ec").await;
        }),
        ("contract_fetch", async {
            let _ =
                get_contract_history(&sdk, "GWRSAVFMjXx8HpQFaNJMqBV7MBgMK4br5UESsB4S31Ec").await;
        }),
    ];
...
async fn test_mnemonic_to_identity_workflow() {
    ...
    let seed = mnemonic
        .to_seed(Some("test-passphrase".to_string()))
        .expect("Should generate seed");
    assert_eq!(seed.len(), 64);

    // Get HD private key
    let hd_key = mnemonic
        .to_hd_private_key(Some("test-passphrase".to_string()), "testnet")
...
async fn test_error_recovery_workflow() {
    ...
    initialize_monitoring(true, Some(20)).expect("Should initialize monitoring");
