...
use wasm_sdk::optimize::{
    clear_string_cache, get_optimization_recommendations, init_string_cache, intern_string,
    optimize_uint8_array, BatchOptimizer, CompressionUtils, FeatureFlags, MemoryOptimizer,
    PerformanceMonitor,
};
...
fn test_feature_flags() {
    ...
    assert!(
        size_reduction.contains("No size reduction"),
    ...
    assert!(
        minimal_reduction.contains("size reduction"),
        "Minimal should show reduction"
    );
    ...
    assert!(
        custom_reduction.contains("tokens"),
        "Should mention disabled tokens"
    );
    assert!(
        custom_reduction.contains("withdrawals"),
        "Should mention disabled withdrawals"
    );
}
...
fn test_memory_optimizer() {
    ...
    assert!(
        stats.contains("Allocations: 3"),
        "Should track 3 allocations"
    );
    assert!(
        stats.contains("Total size: 3584"),
    ...
    assert!(
        reset_stats.contains("Allocations: 0"),
        "Should reset allocations"
    );
}
...
fn test_batch_optimizer() {
    ...
    assert_eq!(
        optimizer.get_optimal_batch_count(100),
        10,
...
fn test_compression_utils() {
    ...
    assert!(
        !CompressionUtils::should_compress(100),
        "Small data shouldn't compress"
    );
    assert!(
        !CompressionUtils::should_compress(1000),
        "1KB shouldn't compress"
    );
    assert!(
        CompressionUtils::should_compress(2000),
        "2KB should compress"
    );
    ...
    assert!(
        ratio1 < 0.5,
        "Uniform data should have low compression ratio"
    );
    ...
    assert!(
        ratio2 > ratio1,
        "Random data should have higher compression ratio"
    );
}
...
fn test_performance_monitor() {
    ...
    assert!(
        report.contains("after_work"),
        "Should contain after_work mark"
    );
    ...
    assert!(
        !new_report.contains("after_work"),
        "Should not contain old marks"
    );
