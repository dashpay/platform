...
async fn test_top_up_identity() {
    ...
    signer.add_private_key(1, test_private_key(), "ECDSA_SECP256K1", 0)
...
async fn test_get_prefunded_balance_and_revision() {
    ...
    if let Ok(result_obj) = result {
        let obj = result_obj.dyn_ref::<Object>()
...
async fn test_transfer_credits() {
    ...
    signer.set_identity_id(&from_identity)
        .expect("Should set identity ID");
    signer.add_private_key(1, test_private_key(), "ECDSA_SECP256K1", 0)
        .expect("Should add private key");
    
    let result = transfer_credits(
        &sdk,
        &from_identity,
        &to_identity,
        500000,
...
async fn test_check_minimum_balance() {
    ...
    let result = check_minimum_balance(
        &sdk,
        &test_identity_id(),
        1000000
    ).await;
