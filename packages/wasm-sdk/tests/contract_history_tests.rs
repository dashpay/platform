...
async fn test_get_contract_at_version() {
    ...
    if let Ok(Some(contract)) = result {
        let contract_obj = contract.dyn_ref::<Object>().expect("Should be an object");
...
async fn test_monitor_contract_updates() {
    ...
    let callback =
        js_sys::Function::new_with_args("update", "console.log('Contract updated:', update);");
...
async fn test_compare_contract_schemas() {
    ...
    if let Ok(comparison) = result {
        let obj = comparison.dyn_ref::<Object>().expect("Should be an object");
...
async fn test_batch_get_contracts() {
    ...
    if let Ok(contracts) = result {
        // Should return a Map
        let map = contracts.dyn_ref::<Map>().expect("Should be a Map");
