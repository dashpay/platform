...
use wasm_sdk::{
    state_transitions::group::{
        create_group_state_transition_info,
...
async fn test_group_state_transition_info() {
    ...
    // Test basic creation
    let info = create_group_state_transition_info(
        0,
        Some("action-123".to_string()),
        Some(true)
    );
    ...
    let info_minimal = create_group_state_transition_info(
        1,
        None,
        None
    );
...
async fn test_validate_group_action_data() {
    ...
    js_sys::Reflect::set(&valid_data, &"timestamp".into(), &JsValue::from(Date::now())).unwrap();
...
async fn test_group_proposal_with_complex_data() {
    ...
    js_sys::Reflect::set(&data_json, &"effectiveDate".into(), &JsValue::from(Date::now())).unwrap();
    
    let proposer_id = "4EfA9Jrvv3nnCFdSf7fad59851iiTRZ6Wcu6YVJ4iSeF";
    
    let info = create_group_state_transition_info(
        0,
        Some("proposal-complex".to_string()),
        Some(true)
    );
...
async fn test_group_info_edge_cases() {
    ...
    let info_max = create_group_state_transition_info(
        u16::MAX,
        Some("max-position".to_string()),
        Some(true)
    );
    assert!(info_max.is_object());
    
    // Test with empty string action ID
    let info_empty = create_group_state_transition_info(
        0,
        Some("".to_string()),
        Some(false)
    );
    ...
    let info_long = create_group_state_transition_info(
        0,
        Some(long_id),
        None
    );
