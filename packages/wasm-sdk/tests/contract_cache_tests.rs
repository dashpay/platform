...
async fn test_basic_caching() {
    ...
    // Cache the contract
    let contract_id = cache.cache_contract(&contract_bytes)
    ...
    // Get cached contract
    let cached_bytes = cache.get_cached_contract(&contract_id)
...
async fn test_cache_stats() {
    ...
    for i in 0..5 {
        ...
        cache.cache_contract(&contract_bytes).expect("Failed to cache");
    }
...
async fn test_clear_cache() {
    ...
    for i in 0..3 {
        let contract_bytes = create_test_contract_bytes(i, 1);
        let id = cache.cache_contract(&contract_bytes).expect("Failed to cache");
...
async fn test_remove_contract() {
    ...
    let id1 = cache.cache_contract(&contract1_bytes).expect("Failed to cache");
    let id2 = cache.cache_contract(&contract2_bytes).expect("Failed to cache");
...
async fn test_contract_metadata() {
    ...
    let contract_bytes = create_test_contract_bytes(1, 1);
    let contract_id = cache.cache_contract(&contract_bytes).expect("Failed to cache");
...
async fn test_preload_suggestions() {
    ...
    for _ in 0..5 {
        cache.get_cached_contract(&contract_id).expect("Failed to get");
    }
...
async fn test_cache_performance() {
    ...
    assert!(cache_time < 100.0, "Caching took too long: {}ms", cache_time);
    ...
    assert!(retrieve_time < 10.0, "Retrieval took too long: {}ms", retrieve_time);
}