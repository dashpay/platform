...
use wasm_sdk::{
    epoch::{
        calculate_epoch_blocks, estimate_next_epoch_time, get_current_epoch, get_current_evonodes,
        get_current_quorum, get_epoch_by_index, get_epoch_for_block_height, get_epoch_stats,
        get_evonode_by_pro_tx_hash, get_evonodes_for_epoch, get_validator_set_changes,
    },
    sdk::WasmSdk,
    start,
};
...
async fn test_epoch_creation() {
    ...
    let current_epoch = get_current_epoch(&sdk)
        .await
    ...
    let epoch_obj = current_epoch
        .to_object()
        .expect("Failed to convert epoch to object");
...
async fn test_epoch_by_index() {
    ...
    for index in [0, 1, 10, 100] {
        let epoch = get_epoch_by_index(&sdk, index)
            .await
...
async fn test_evonodes_retrieval() {
    ...
    let evonodes_js = get_current_evonodes(&sdk)
        .await
...
async fn test_evonode_by_pro_tx_hash() {
    ...
    let evonode = get_evonode_by_pro_tx_hash(&sdk, pro_tx_hash.clone())
        .await
    ...
    assert!(get_evonode_by_pro_tx_hash(&sdk, invalid_hash)
        .await
        .is_err());
}
...
async fn test_current_quorum() {
    ...
    let quorum_js = get_current_quorum(&sdk)
        .await
        .expect("Failed to get current quorum");

    let quorum = quorum_js
        .dyn_ref::<Object>()
    ...
    let total_members = Reflect::get(quorum, &"totalMembers".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
    let threshold = Reflect::get(quorum, &"threshold".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
...
async fn test_estimate_next_epoch_time() {
    ...
    let estimate_js = estimate_next_epoch_time(&sdk, current_block)
        .await
        .expect("Failed to estimate next epoch time");

    let estimate = estimate_js
        .dyn_ref::<Object>()
    ...
    let blocks_remaining = Reflect::get(estimate, &"blocksRemaining".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
    let minutes_remaining = Reflect::get(estimate, &"minutesRemaining".into())
        .unwrap()
        .as_f64()
        .unwrap();
...
async fn test_epoch_for_block_height() {
    ...
    for (block_height, expected_epoch) in [
        ...
        let epoch = get_epoch_for_block_height(&sdk, block_height)
            .await
...
async fn test_validator_set_changes() {
    ...
    // Test valid range
    let changes_js = get_validator_set_changes(&sdk, 1, 3)
        .await
        .expect("Failed to get validator set changes");

    let changes = changes_js
        .dyn_ref::<Object>()
    ...
    let from_epoch = Reflect::get(changes, &"fromEpoch".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
    let to_epoch = Reflect::get(changes, &"toEpoch".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
...
async fn test_epoch_stats() {
    ...
    let stats_js = get_epoch_stats(&sdk, 5)
        .await
    ...
    let total_nodes = Reflect::get(stats, &"totalEvonodes".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
    let hpmn_count = Reflect::get(stats, &"hpmnCount".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
    let regular_count = Reflect::get(stats, &"regularNodeCount".into())
        .unwrap()
        .as_f64()
        .unwrap() as u32;
...
async fn test_epoch_fee_multiplier_patterns() {
    ...
    for epoch_idx in 0..25 {
        let epoch = get_epoch_by_index(&sdk, epoch_idx)
            .await
...
async fn test_network_specific_evonodes() {
    ...
    for (network, min_nodes) in [("mainnet", 100), ("testnet", 50), ("devnet", 10)] {
        ...
        let evonodes_js = get_evonodes_for_epoch(&sdk, 0)
            .await
