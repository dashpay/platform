...
use wasm_sdk::{
    nonce::{
        check_identity_contract_nonce_cache, check_identity_nonce_cache,
        clear_identity_contract_nonce_cache, clear_identity_nonce_cache,
        increment_identity_contract_nonce_cache, increment_identity_nonce_cache,
        update_identity_contract_nonce_cache, update_identity_nonce_cache, NonceOptions,
    },
    start,
};
...
async fn test_identity_nonce_cache_lifecycle() {
    ...
    // Check empty cache
    let cached = check_identity_nonce_cache(TEST_IDENTITY_ID).expect("Failed to check cache");
    assert!(cached.is_none());

    // Update cache
    let test_nonce = 42u64;
    update_identity_nonce_cache(TEST_IDENTITY_ID, test_nonce).expect("Failed to update cache");
...
async fn test_identity_nonce_increment() {
    ...
    // Set initial nonce
    update_identity_nonce_cache(TEST_IDENTITY_ID, 10).expect("Failed to update cache");

    // Increment by default (1)
    let new_nonce =
        increment_identity_nonce_cache(TEST_IDENTITY_ID, None).expect("Failed to increment");
    assert_eq!(new_nonce, 11);

    // Increment by custom amount
    let new_nonce =
        increment_identity_nonce_cache(TEST_IDENTITY_ID, Some(5)).expect("Failed to increment");
...
async fn test_identity_contract_nonce_increment() {
    ...
    let new_nonce =
        increment_identity_contract_nonce_cache(TEST_IDENTITY_ID, TEST_CONTRACT_ID, None)
            .expect("Failed to increment");
    assert_eq!(new_nonce, 51);

    // Increment by custom amount
    let new_nonce =
        increment_identity_contract_nonce_cache(TEST_IDENTITY_ID, TEST_CONTRACT_ID, Some(10))
            .expect("Failed to increment");
...
async fn test_cache_staleness() {
    ...
    // Update cache
    update_identity_nonce_cache(TEST_IDENTITY_ID, 123).expect("Failed to update cache");
...
async fn test_saturating_increment() {
    ...
    update_identity_nonce_cache(TEST_IDENTITY_ID, near_max).expect("Failed to update");

    // Increment should saturate, not overflow
    let new_nonce =
        increment_identity_nonce_cache(TEST_IDENTITY_ID, Some(20)).expect("Failed to increment");
...
async fn test_cache_clear_independence() {
    ...
    assert!(check_identity_nonce_cache(TEST_IDENTITY_ID)
        .unwrap()
        .is_none());
    ...
    assert!(
        check_identity_contract_nonce_cache(TEST_IDENTITY_ID, TEST_CONTRACT_ID)
            .unwrap()
            .is_none()
    );
}
...
async fn test_concurrent_increments() {
    ...
    let final_nonce = check_identity_nonce_cache(TEST_IDENTITY_ID)
        .unwrap()
        .unwrap();
