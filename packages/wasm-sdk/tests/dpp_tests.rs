...
use wasm_sdk::{
    dpp::{
        create_state_transition, generate_document_id, generate_entropy, validate_public_keys,
        DataContractWasm, IdentityWasm,
    },
    start,
};
...
async fn test_identity_wasm_creation() {
    ...
    let identity = IdentityWasm::new(TEST_PLATFORM_VERSION).expect("Failed to create IdentityWasm");
...
async fn test_identity_public_keys() {
    ...
    let mut identity =
        IdentityWasm::new(TEST_PLATFORM_VERSION).expect("Failed to create IdentityWasm");
    ...
    // Set public keys
    let new_revision = identity
        .set_public_keys(keys.into())
...
async fn test_data_contract_creation() {
    ...
    js_sys::Reflect::set(
        &raw_contract,
        &"$schema".into(),
        &"https://schema.dash.org/dpp-0-4-0/meta/data-contract".into(),
    )
    .unwrap();
...
async fn test_generate_document_id() {
    ...
    let doc_id = generate_document_id(contract_id, owner_id, document_type, &entropy)
