...
use wasm_sdk::{
    request_settings::{execute_with_retry, RequestSettings, RequestSettingsBuilder, RetryHandler},
    start,
};
...
async fn test_retry_handler_error_codes() {
    ...
    for code in &["NETWORK_ERROR", "TIMEOUT", "UNAVAILABLE"] {
        ...
        assert!(
            handler.should_retry(&error.into()),
            "Should retry on {}",
            code
        );
    }
...
async fn test_request_settings_builder_without_retries() {
    ...
    let settings = RequestSettingsBuilder::new().without_retries().build();
