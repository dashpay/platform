...
async fn test_operation_tracking() {
    ...
    // Start an operation
    monitor.start_operation("test_op_1".to_string(), "TestOperation".to_string())
        .expect("Should start operation");
    
    // Check active operations count
    let active_count = monitor.get_active_operations_count()
        .expect("Should get active operations count");
    assert_eq!(active_count, 1);
    
    // End the operation
    monitor.end_operation("test_op_1".to_string(), true, None)
    ...
    let active_count_after = monitor.get_active_operations_count()
        .expect("Should get active operations count");
...
async fn test_operation_with_error() {
    ...
    monitor.start_operation("error_op".to_string(), "ErrorOperation".to_string())
        .expect("Should start operation");
    ...
    let metric_obj = metric.dyn_ref::<Object>()
        .expect("Should be an object");
    
    let success = Reflect::get(metric_obj, &"success".into())
        .expect("Should have success field");
    assert_eq!(success.as_bool(), Some(false));
    
    let error_msg = Reflect::get(metric_obj, &"errorMessage".into())
        .expect("Should have error message");
    assert_eq!(error_msg.as_string(), Some("Test error message".to_string()));
}
...
fn test_operation_metadata() {
    ...
    monitor.start_operation("metadata_op".to_string(), "MetadataOperation".to_string())
        .expect("Should start operation");
    
    // Add metadata
    monitor.add_operation_metadata(
    ...
    monitor.end_operation("metadata_op".to_string(), true, None)
        .expect("Should end operation");
    ...
    let metadata = Reflect::get(metric_obj, &"metadata".into())
        .expect("Should have metadata");
    let metadata_obj = metadata.dyn_ref::<Object>()
        .expect("Metadata should be an object");
    
    let value1 = Reflect::get(metadata_obj, &"key1".into())
...
fn test_metrics_by_operation() {
    ...
    for i in 0..3 {
        ...
        monitor.start_operation(op_id.clone(), "FetchOperation".to_string())
            .expect("Should start operation");
        monitor.end_operation(op_id, true, None)
            .expect("Should end operation");
    }
    
    for i in 0..2 {
        let op_id = format!("broadcast_{}", i);
        monitor.start_operation(op_id.clone(), "BroadcastOperation".to_string())
            .expect("Should start operation");
        monitor.end_operation(op_id, true, None)
            .expect("Should end operation");
    }
    ...
    let fetch_metrics = monitor.get_metrics_by_operation("FetchOperation".to_string())
        .expect("Should get fetch metrics");
    assert_eq!(fetch_metrics.length(), 3);
    
    let broadcast_metrics = monitor.get_metrics_by_operation("BroadcastOperation".to_string())
...
fn test_operation_statistics() {
    ...
    for i in 0..5 {
        ...
        monitor.start_operation(op_id.clone(), "TestOp".to_string())
            .expect("Should start operation");
        
        // Make some operations fail
        let success = i % 2 == 0;
        let error = if success { None } else { Some("Error".to_string()) };
        
        monitor.end_operation(op_id, success, error)
            .expect("Should end operation");
    }
    ...
    let test_op_stats = Reflect::get(stats_obj, &"TestOp".into())
        .expect("Should have TestOp stats");
    let test_op_obj = test_op_stats.dyn_ref::<Object>()
        .expect("TestOp stats should be an object");
    
    let count = Reflect::get(test_op_obj, &"count".into())
        .expect("Should have count");
    assert_eq!(count.as_f64(), Some(5.0));
    
    let success_count = Reflect::get(test_op_obj, &"successCount".into())
        .expect("Should have success count");
    assert_eq!(success_count.as_f64(), Some(3.0));
    
    let error_count = Reflect::get(test_op_obj, &"errorCount".into())
        .expect("Should have error count");
    assert_eq!(error_count.as_f64(), Some(2.0));
    
    let success_rate = Reflect::get(test_op_obj, &"successRate".into())
        .expect("Should have success rate");
...
fn test_disabled_monitor() {
    ...
    monitor.start_operation("op1".to_string(), "TestOp".to_string())
        .expect("Should not error when disabled");
    monitor.end_operation("op1".to_string(), true, None)
        .expect("Should not error when disabled");
    
    let metrics = monitor.get_metrics()
        .expect("Should get empty metrics");
...
fn test_global_monitor_initialization() {
    initialize_monitoring(true, Some(100))
        .expect("Should initialize global monitoring");
...
fn test_resource_usage() {
    let usage = get_resource_usage()
        .expect("Should get resource usage");
    
    let usage_obj = usage.dyn_ref::<Object>()
    ...
    if Reflect::has(usage_obj, &"memory".into()).unwrap() {
        let memory = Reflect::get(usage_obj, &"memory".into())
...
fn test_performance_metrics_object() {
    ...
    monitor.start_operation("perf_test".to_string(), "PerfTest".to_string())
        .expect("Should start operation");
    ...
    monitor.end_operation("perf_test".to_string(), true, None)
        .expect("Should end operation");
    ...
    let duration = Reflect::get(metric_obj, &"duration".into())
