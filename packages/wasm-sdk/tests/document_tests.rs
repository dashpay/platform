...
async fn test_document_batch_builder() {
    ...
    let delete_result = batch.add_delete_document(
        &contract_id,
        document_type,
        &test_document_id()
    );
    ...
    assert!(!state_transition.unwrap().is_empty(), "State transition should not be empty");
}
...
async fn test_fetch_documents() {
    ...
    // Test basic fetch
    let result = fetch_documents(&sdk, &contract_id, document_type, where_clause.into(), None).await;
    ...
    assert!(result_with_options.is_ok(), "Should fetch documents with options");
}
...
async fn test_document_transitions() {
    ...
    assert!(transfer_result.is_ok(), "Should create transfer document transition");
    ...
    assert!(purchase_result.is_ok(), "Should create purchase document transition");
}
...
async fn test_complex_document_query() {
    ...
    assert_eq!(order_by_clauses.length(), 2, "Should have 2 order by clauses");
}