...
impl AssetLockProof {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<AssetLockProof, JsError> {
        let (inner, _): (DppAssetLockProof, _) = bincode::decode_from_slice(bytes, bincode::config::standard())
            .map_err(|e| JsError::new(&format!("Failed to deserialize asset lock proof: {}", e)))?;
    ...
    pub fn to_json(&self) -> Result<JsValue, JsError> {
        let value = self.inner.to_raw_object()
    ...
    pub fn to_object(&self) -> Result<JsValue, JsError> {
        ...
        match &self.inner {
            ...
            DppAssetLockProof::Chain(proof) => {
                Reflect::set(&obj, &"coreChainLockedHeight".into(), &proof.core_chain_locked_height.into())
    ...
    pub fn get_identity_id(&self) -> Result<String, JsError> {
        let identifier = self.inner.create_identifier()
...
pub fn validate_asset_lock_proof(
    ...
    if let Some(id_str) = identity_id {
        let expected_identifier = Identifier::from_string(
            &id_str,
            platform_value::string_encoding::Encoding::Base58,
        )
        .map_err(|e| JsError::new(&format!("Invalid identity ID: {}", e)))?;
        
        let proof_identifier = proof.inner.create_identifier()
            .map_err(|e| JsError::new(&format!("Failed to create identifier: {}", e)))?;
...
pub fn calculate_credits_from_proof(
    ...
    match &proof.inner {
        DppAssetLockProof::Instant(instant_proof) => {
            let output = instant_proof.output()
        ...
        DppAssetLockProof::Chain(_) => {
            ...
            Err(JsError::new("Cannot calculate credits from chain proof without transaction"))
        }
    }
}
...
pub fn create_instant_proof_from_parts(
    ...
    let tx_bytes = if let Some(tx_str) = transaction.as_string() {
        ...
        return Err(JsError::new("Transaction must be a hex string or Uint8Array"));
    };
    ...
    let lock_bytes = if let Some(lock_str) = instant_lock.as_string() {
        ...
        return Err(JsError::new("Instant lock must be a hex string or Uint8Array"));
    };
