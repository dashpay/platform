...
impl ContractVersion {
    ...
    pub fn to_object(&self) -> Result<JsValue, JsError> {
        ...
        Reflect::set(&obj, &"documentTypesCount".into(), &self.document_types_count.into())
...
pub async fn fetch_contract_history(
    ...
    let response = client.raw_request("/platform/v1/contract/history", &request).await?;
...
pub async fn fetch_contract_versions(
    sdk: &WasmSdk,
    contract_id: &str,
) -> Result<Array, JsError> {
    let _identifier = Identifier::from_string(
    ...
    let response = client.raw_request("/platform/v1/contract/versions", &request).await?;
...
pub async fn get_schema_changes(
    ...
    if version_diff > 0 {
        ...
        let field_changes = vec![
            ("profile", "email", "field_added", None, Some("{ type: 'string', format: 'email' }")),
            ("profile", "avatar", "field_added", None, Some("{ type: 'string', contentMediaType: 'image/*' }")),
        ];
...
pub async fn fetch_contract_at_version(
    ...
    let response = client.raw_request("/platform/v1/contract/version", &request).await?;
...
pub async fn check_contract_updates(
    ...
    let contract_response = client.get_data_contract(contract_id.to_string(), false).await?;
...
pub async fn get_migration_guide(
    ...
    for i in 0..schema_changes.length() {
        ...
        if let Some(change_type) = Reflect::get(&change, &"changeType".into()).ok().and_then(|v| v.as_string()) {
            let doc_type = Reflect::get(&change, &"documentType".into()).ok().and_then(|v| v.as_string()).unwrap_or_default();
            let field_name = Reflect::get(&change, &"fieldName".into()).ok().and_then(|v| v.as_string());
            ...
            match change_type.as_str() {
                ...
                "field_removed" => {
                    if let Some(field) = field_name {
                        ...
                        breaking_changes.push(&format!("Removed field '{}' from '{}'", field, doc_type).into());
                    }
                },
                "field_type_changed" => {
                    if let Some(field) = field_name {
                        steps.push(&format!("Migrate '{}' field in '{}' documents to new type format", field, doc_type).into());
                        warnings.push(&format!("Type change for field '{}' may require data transformation", field).into());
                    }
                },
                "field_required_changed" => {
                    if let Some(field) = field_name {
                        let new_val = Reflect::get(&change, &"newValue".into()).ok().and_then(|v| v.as_string()).unwrap_or_default();
                        if new_val.contains("required: true") {
                            steps.push(&format!("Ensure all '{}' documents have '{}' field before migration", doc_type, field).into());
                            warnings.push(&format!("Field '{}' will become required", field).into());
                        }
                    }
                },
                "index_added" => {
                    if let Some(field) = field_name {
                        ...
                        if new_val.contains("unique: true") {
                            steps.push(&format!("Check for duplicate values in '{}' field of '{}' documents", field, doc_type).into());
                            warnings.push(&format!("Unique constraint will be enforced on '{}' field", field).into());
                        } else {
                ...
                "document_type_added" => {
                    steps.push(&format!("New document type '{}' will be available", doc_type).into());
                },
                "document_type_removed" => {
                    warnings.push(&format!("Document type '{}' will be removed - backup existing documents", doc_type).into());
    ...
    if steps.length() > 0 {
        ...
        steps.push(&format!("{}. Update application code to handle schema changes", steps.length() + 1).into());
        steps.push(&format!("{}. Test thoroughly in staging environment before production deployment", steps.length() + 1).into());
    }
    ...
    Reflect::set(&metadata, &"totalChanges".into(), &schema_changes.length().into())
        .map_err(|_| JsError::new("Failed to set total changes"))?;
    Reflect::set(&metadata, &"hasBreakingChanges".into(), &(breaking_changes.length() > 0).into())
...
pub async fn monitor_contract_updates(
    ...
    Reflect::set(&handle, &"contractId".into(), &identifier.to_string(platform_value::string_encoding::Encoding::Base58).into())
        .map_err(|_| JsError::new("Failed to set contract ID"))?;
    ...
    spawn_local({
        ...
        async move {
            match check_contract_updates(&sdk_inner, &id_inner, current_version).await {
                ...
                Err(e) => {
                    web_sys::console::error_1(&JsValue::from_str(&format!("Contract update check error: {:?}", e)));
                }
            }
        }
    });
            ...
            match check_contract_updates(&sdk_inner, &id_inner, tracked_version).await {
                Ok(has_update) => {
                    if has_update {
                        let update_info = Object::new();
                        let _ = Reflect::set(&update_info, &"hasUpdate".into(), &true.into());
                        let _ = Reflect::set(&update_info, &"currentVersion".into(), &tracked_version.into());
...
fn parse_contract_version(data: &serde_json::Value) -> Result<JsValue, JsError> {
    let version = ContractVersion {
        ...
        created_at: data.get("createdAt")
            .and_then(|v| v.as_u64())
            .unwrap_or(0),
        document_types_count: data.get("documentTypesCount")
            .and_then(|v| v.as_u64())
            .unwrap_or(0) as u32,
        total_documents: data.get("totalDocuments")
            .and_then(|v| v.as_u64())
            .unwrap_or(0),
    };
