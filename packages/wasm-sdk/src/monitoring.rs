...
use js_sys::{Array, Date, Map, Object, Reflect};
...
pub async fn track_operation(
    ...
    // Start tracking
    let monitor_guard = GLOBAL_MONITOR
        .lock()
        .map_err(|_| JsError::new("Failed to lock global monitor"))?;
...
pub async fn perform_health_check(sdk: &crate::sdk::WasmSdk) -> Result<HealthCheckResult, JsError> {
    let checks = Map::new();
    ...
    match check_dapi_connectivity(sdk).await {
        Ok(true) => {
            ...
            Reflect::set(
                &dapi_check,
                &"message".into(),
                &"DAPI connection successful".into(),
            )
    ...
    Reflect::set(
        &memory_check,
        &"message".into(),
        &"Memory monitoring available through browser DevTools".into(),
    )
    .map_err(|_| JsError::new("Failed to set message"))?;
...
pub fn get_resource_usage() -> Result<JsValue, JsError> {
    ...
    // We'll create a placeholder for now
    {
        ...
        Reflect::set(
            &memory_obj,
            &"message".into(),
            &"Memory API not available in web-sys".into(),
        )
