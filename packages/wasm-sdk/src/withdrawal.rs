...
pub async fn get_withdrawal_status(
    ...
    let withdrawal_condition =
        js_sys::Array::of3(&"withdrawalId".into(), &"==".into(), &withdrawal_id.into());
...
pub async fn broadcast_withdrawal(
    ...
    if success {
        ...
        Reflect::set(
            &response,
            &"message".into(),
            &"Withdrawal broadcast successfully".into(),
        )
...
pub async fn estimate_withdrawal_time(
    ...
    let total_minutes =
        ((base_time_minutes as f64 + queue_delay) * network_congestion_factor) as u32;
    ...
    Reflect::set(
        &response,
        &"currentQueueLength".into(),
        &estimated_queue_length.into(),
    )
    .map_err(|_| JsError::new("Failed to set queue length"))?;
    Reflect::set(
        &response,
        &"networkCongestion".into(),
        &network_congestion_factor.into(),
    )
...
fn create_output_script_from_address(address: &str) -> Result<Vec<u8>, JsError> {
    ...
    // Parse the address
    let addr =
        Address::from_str(address).map_err(|e| JsError::new(&format!("Invalid address: {}", e)))?;
