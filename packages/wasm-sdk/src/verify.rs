...
pub async fn verify_identity_by_id(
    ...
    let identity_id_bytes = platform_value::Identifier::from_string(
        identity_id,
        Encoding::Base58,
    )
    ...
    let platform_version = PlatformVersion::get(platform_version)
        .map_err(|e| wasm_bindgen::JsError::new(&format!("Failed to get platform version: {}", e)))?;
    
    let proof_vec = proof.to_vec();
    let identity_id_array: [u8; 32] = identity_id_bytes.to_buffer()
        .try_into()
        .map_err(|_| wasm_bindgen::JsError::new("Invalid identity ID length"))?;
    
    let (_root_hash, identity_option) = wasm_drive_verify::native::verify_full_identity_by_identity_id(
        &proof_vec,
        is_proof_subset,
        identity_id_array,
        &platform_version,
...
pub async fn verify_data_contract_by_id(
    ...
    let contract_id_bytes = platform_value::Identifier::from_string(
        contract_id,
        Encoding::Base58,
    )
    ...
    let proof_vec = proof.to_vec();
    let contract_id_array: [u8; 32] = contract_id_bytes.to_buffer()
        .try_into()
...
pub fn verify_data_contract_proof(
    ...
    let contract_id_array: [u8; 32] = contract_id.try_into()
        ...
        if let Some(order_item_array) = order_item.dyn_ref::<js_sys::Array>() {
            if order_item_array.length() >= 2 {
                ...
                match direction.as_str() {
                    "asc" | "desc" => {},
                    _ => return Err(JsError::new(&format!("Unknown sort direction: {}", direction))),
                };
