...
pub fn validate_state_transition(
    ...
    if bytes.len() > 16 * 1024 * 1024 {
        ...
        return Err(JsError::new(
            "State transition exceeds maximum size of 16MB",
        ));
    }
    ...
    Reflect::set(
        &result,
        &"valid".into(),
        &(validation_errors.length() == 0).into(),
    )
...
pub fn process_broadcast_response(
    ...
    Err(JsError::new(
        "Unable to parse broadcast response: unsupported format",
    ))
}
    ...
    if let Ok(response_str) = String::from_utf8(bytes.clone()) {
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&response_str) {
            ...
            if let Some(result_data) = json.get("result").or_else(|| json.get("data")) {
                ...
                let js_data = serde_wasm_bindgen::to_value(result_data).unwrap_or(JsValue::NULL);
            ...
            if let Some(metadata) = json.get("metadata") {
                let js_metadata = serde_wasm_bindgen::to_value(metadata).unwrap_or(JsValue::NULL);
