...
impl WasmSdk {
    ...
    pub async fn document_create(
        ...
        // Convert to StateTransition and broadcast (using working broadcast method)
        let state_transition: StateTransition = state_transition.into();
        state_transition
            .broadcast(&sdk, None)
            .await
            .map_err(|e| JsValue::from_str(&format!("Failed to broadcast transition: {}", e)))?;

        // Return success result (similar to document_delete pattern)
        Self::build_js_result_object(
            "DocumentCreated",
            &document.id().to_string(Encoding::Base58),
            vec![("created", JsValue::from_bool(true))],
        )
    }
