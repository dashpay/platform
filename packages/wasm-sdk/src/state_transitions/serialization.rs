...
pub fn get_state_transition_type(bytes: &Uint8Array) -> Result<StateTransitionTypeWasm, JsError> {
    ...
    Ok(match state_transition {
        ...
        StateTransition::IdentityCreditWithdrawal(_) => StateTransitionTypeWasm::IdentityCreditWithdrawal,
        StateTransition::IdentityUpdate(_) => StateTransitionTypeWasm::IdentityUpdate,
        StateTransition::IdentityCreditTransfer(_) => StateTransitionTypeWasm::IdentityCreditTransfer,
...
pub fn get_modified_data_ids(bytes: &Uint8Array) -> Result<JsValue, JsError> {
    ...
    match &state_transition {
        StateTransition::DataContractCreate(st) => {
            let contract_id = st.data_contract().id().to_string(platform_value::string_encoding::Encoding::Base58);
        ...
        StateTransition::IdentityCreate(st) => {
            let identity_id = st.identity_id().to_string(platform_value::string_encoding::Encoding::Base58);
...
fn state_transition_to_js_object(state_transition: &StateTransition) -> Result<JsValue, JsError> {
    ...
    match state_transition {
        StateTransition::IdentityCreate(st) => {
            ...
            Reflect::set(&obj, &"publicKeysCount".into(), &(st.public_keys().len() as u32).into())
