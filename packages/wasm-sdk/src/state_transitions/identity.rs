...
use dpp::identity::identity_public_key::{v0::IdentityPublicKeyV0, IdentityPublicKey};
...
use web_sys::js_sys::{Array, Number, Object, Reflect, Uint8Array};
...
pub fn create_identity(
    ...
    let public_keys_in_creation: Vec<IdentityPublicKeyInCreation> =
...
impl IdentityTransitionBuilder {
    ...
    pub fn build_create_transition(
        ...
        let public_keys_in_creation: Vec<IdentityPublicKeyInCreation> = self
            .add_public_keys
            .into_iter()
            .map(|key| key.into())
            .collect();
...
fn parse_public_keys_in_creation_from_js(
    js_array: &JsValue,
) -> Result<Vec<IdentityPublicKeyInCreation>, JsError> {
    let array = js_array
        .dyn_ref::<Array>()
        .ok_or_else(|| JsError::new("Expected an array of public keys"))?;
...
fn parse_public_key_from_js(js_obj: &JsValue) -> Result<IdentityPublicKey, JsError> {
    ...
    let security_level_num = Reflect::get(obj, &"securityLevel".into())
        ...
        .as_f64()
        .ok_or_else(|| JsError::new("'securityLevel' must be a number"))?
        as u8;
    ...
    // Get data
    let data_value =
        Reflect::get(obj, &"data".into()).map_err(|_| JsError::new("Missing 'data' field"))?;
...
fn parse_public_key_in_creation_from_js(
    js_obj: &JsValue,
) -> Result<IdentityPublicKeyInCreation, JsError> {
    let obj = js_obj
        .dyn_ref::<Object>()
        .ok_or_else(|| JsError::new("Expected a public key object"))?;
...
pub fn validate_identity_public_keys(public_keys: JsValue) -> Result<JsValue, JsError> {
    ...
    let has_auth_key = keys.iter().any(|key| match key {
    ...
    let has_master_key = keys.iter().any(|key| match key {
        IdentityPublicKey::V0(v0) => v0.security_level == SecurityLevel::MASTER,
    });

    if !has_master_key {
        return Err(JsError::new(
            "At least one master security level key is required",
        ));
    }
