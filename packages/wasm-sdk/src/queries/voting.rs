...
impl WasmSdk {
    ...
    pub async fn get_contested_resource_voters_for_identity(
        ...
        let start_at_identifier_info = if let Some(info_str) = start_at_voter_info {
            ...
            if let (Some(start_id), Some(included)) = (
                ...
                info.get("startIdentifierIncluded"),
            ) {
    ...
    pub async fn get_vote_polls_by_end_date(
        ...
        let _ = (self, start_time_info, end_time_info, limit, order_ascending);
    ...
    pub async fn get_contested_resources_with_proof_info(
        ...
        let metadata = response
            .inner
            .metadata()
            .map_err(|e| WasmSdkError::generic(format!("Failed to get metadata: {:?}", e)))?;

        let proof = response
            .inner
            .proof()
            .map_err(|e| WasmSdkError::generic(format!("Failed to get proof: {:?}", e)))?;
            ...
            version: Some(get_contested_resource_vote_state_request::Version::V0(
                GetContestedResourceVoteStateRequestV0 {
                    ...
                    allow_include_locked_and_abstaining_vote_tally:
                        allow_include_locked_and_abstaining_vote_tally.unwrap_or(false),
                    start_at_identifier_info,
                    count,
                    prove: true, // Always true for proof info version
                },
