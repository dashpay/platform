...
use dpp::serialization::{
    PlatformDeserializable, PlatformLimitDeserializableFromVersionedStructure,
};
...
pub fn deserialize_get_identity_response(response_bytes: &Uint8Array) -> Result<JsValue, JsError> {
    ...
    if let Ok(json_response) = serde_json::from_slice::<serde_json::Value>(&bytes) {
        ...
        if let Some(identity_data) = json_response.get("identity") {
            return serde_wasm_bindgen::to_value(identity_data).map_err(|e| {
                JsError::new(&format!("Failed to convert identity to JS value: {}", e))
            });
        }
    }
    ...
    match Identity::deserialize_from_bytes_no_limit(&bytes) {
        Ok(identity) => {
            ...
            serde_wasm_bindgen::to_value(&identity_json).map_err(|e| {
                JsError::new(&format!("Failed to convert identity to JS value: {}", e))
            })
        }
...
pub fn deserialize_get_data_contract_response(
    ...
    if let Ok(json_response) = serde_json::from_slice::<serde_json::Value>(&bytes) {
        ...
        if let Some(contract_data) = json_response.get("dataContract") {
            return serde_wasm_bindgen::to_value(contract_data).map_err(|e| {
                JsError::new(&format!(
                    "Failed to convert data contract to JS value: {}",
                    e
                ))
            });
        }
    ...
    match DataContract::versioned_limit_deserialize(&bytes, &platform_version) {
        Ok(contract) => {
            ...
            serde_wasm_bindgen::to_value(&contract_json).map_err(|e| {
                JsError::new(&format!(
                    "Failed to convert data contract to JS value: {}",
                    e
                ))
...
pub fn deserialize_broadcast_response(response_bytes: &Uint8Array) -> Result<JsValue, JsError> {
    let bytes = response_bytes.to_vec();
