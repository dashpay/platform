...
pub trait Fetch {
    ...
    async fn fetch_data_contract(&self, id: String, options: Option<FetchOptions>) -> Result<DataContractWasm, JsError>;
    
    /// Fetch a document by ID
    async fn fetch_document(&self, id: String, contract_id: String, document_type: String, options: Option<FetchOptions>) -> Result<JsValue, JsError>;
}
        ...
        if let Some(response_obj) = response.dyn_ref::<js_sys::Object>() {
            ...
            if prove {
                ...
                if let Some(proof_str) = proof_value.as_string() {
                    ...
                    match verify_full_identity_by_identity_id(&proof_array, false, &identity_id_array, 1) {
                        Ok(result) => {
                            ...
                            let identity: Identity = serde_json::from_str(&identity_json)
                                .map_err(|e| JsError::new(&format!("Failed to parse verified identity: {}", e)))?;
                            
                            return Ok(IdentityWasm::from(identity));
                        }
                        Err(e) => {
                            return Err(JsError::new(&format!("Proof verification failed: {:?}", e)));
                        }
                    }
                }
            }
...
pub async fn fetch_document(
    ...
    sdk.fetch_document(document_id, contract_id, document_type, options).await
}
