...
describe('GetStatusResponse', () => {
  ...
  beforeEach(async () => {
    ...
    const version = new VersionStatus(
      statusFixture.version.software.dapi,
      statusFixture.version.software.drive,
      statusFixture.version.software.tenderdash,
      statusFixture.version.protocol.tenderdash.p2p,
    ...
    const node = new NodeStatus(
      Buffer.from(statusFixture.node.id).toString('hex'),
      Buffer.from(statusFixture.node.proTxHash).toString('hex'),
    );

    const chain = new ChainStatus(
      statusFixture.chain.catchingUp,
      Buffer.from(statusFixture.chain.latestBlockHash).toString('hex'),
      Buffer.from(statusFixture.chain.latestAppHash).toString('hex'),
      BigInt(statusFixture.chain.latestBlockHeight),
      Buffer.from(statusFixture.chain.earliestBlockHash).toString('hex'),
      Buffer.from(statusFixture.chain.earliestAppHash).toString('hex'),
      BigInt(statusFixture.chain.earliestBlockHeight),
      BigInt(statusFixture.chain.maxPeerBlockHeight),
      statusFixture.chain.coreChainLockedHeight,
    );

    const network = new NetworkStatus(
      statusFixture.network.chainId,
      statusFixture.network.peersCount,
      statusFixture.network.listening,
    );

    const stateSync = new StateSyncStatus(
      BigInt(statusFixture.stateSync.totalSyncedTime),
      BigInt(statusFixture.stateSync.remainingTime),
      statusFixture.stateSync.totalSnapshots,
      BigInt(statusFixture.stateSync.chunkProcessAverageTime),
      BigInt(statusFixture.stateSync.snapshotHeight),
      BigInt(statusFixture.stateSync.snapshotChunksCount),
      BigInt(statusFixture.stateSync.backfilledBlocks),
      BigInt(statusFixture.stateSync.backfillBlocksTotal),
    );

    const time = new TimeStatus(
      BigInt(statusFixture.time.local),
      BigInt(statusFixture.time.block),
      BigInt(statusFixture.time.genesis),
      statusFixture.time.epoch,
    );
    ...
    getStatusResponse = new GetStatusResponseClass(
      version,
      node,
      chain,
      network,
  ...
  it('should return status', () => {
    ...
    const nodeStatus = getStatusResponse.getNodeStatus();
    const chainStatus = getStatusResponse.getChainStatus();
    const networkStatus = getStatusResponse.getNetworkStatus();
    const stateSyncStatus = getStatusResponse.getStateSyncStatus();
