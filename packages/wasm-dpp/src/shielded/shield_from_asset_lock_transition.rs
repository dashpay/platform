...
impl ShieldFromAssetLockTransitionWasm {
    ...
    pub fn get_asset_lock_proof(&self) -> Result<JsValue, JsValue> {
        let proof = match &self.0 {
            ShieldFromAssetLockTransition::V0(v0) => &v0.asset_lock_proof,
        };
        serde_wasm_bindgen::to_value(proof).map_err(|e| JsValue::from(e.to_string()))
    }
    ...
    pub fn get_signature(&self) -> Buffer {
        let sig = match &self.0 {
            ShieldFromAssetLockTransition::V0(v0) => &v0.signature,
        };
        Buffer::from_bytes(sig.as_slice())
    }
    ...
    pub fn from_buffer(buffer: Vec<u8>) -> Result<ShieldFromAssetLockTransitionWasm, JsValue> {
        let state_transition: StateTransition =
            PlatformDeserializable::deserialize_from_bytes(&buffer).with_js_error()?;
        match state_transition {
            StateTransition::ShieldFromAssetLock(st) => Ok(st.into()),
