...
use crate::{
    ...
    buffer::Buffer,
    errors::{RustConversionError, from_dpp_err},
    with_js_error,
};
...
impl From<ChainAssetLockProof> for ChainAssetLockProofWasm {
    fn from(v: ChainAssetLockProof) -> Self { ChainAssetLockProofWasm(v) }
}

impl Into<ChainAssetLockProof> for ChainAssetLockProofWasm {
    fn into(self) -> ChainAssetLockProof { self.0 }
}
...
impl ChainAssetLockProofWasm {
    ...
    pub fn new(raw_parameters: JsValue) -> Result<ChainAssetLockProofWasm, JsValue> {
        ...
        let out_point: [u8; 36] = parameters.out_point.try_into().map_err(|_| {
            RustConversionError::Error(
                String::from(
                    "outPoint must be a 36 byte array"
                )
            ).to_js_value()
        })?;

        let chain_asset_lock_proof = ChainAssetLockProof::new(
            parameters.core_chain_locked_height,
            out_point,
        );
    ...
    pub fn to_json(&self) -> Result<JsValue, JsValue> {
        ...
        let out_point_base64 = string_encoding::encode(
            self.0.out_point(),
            Encoding::Base64
        );
    ...
    pub fn to_object(&self) -> Result<JsValue, JsValue> {
        let asset_lock_json = serde_json::to_value(
            self.0.clone()
        ).map_err(|e| from_dpp_err(e.into()))?;

        let asset_lock_json_string = serde_json::to_string(
            &asset_lock_json
        ).map_err(|e| from_dpp_err(e.into()))?;
