...
impl IdentityTopUpTransitionWasm {
    ...
    pub fn to_object(&self, options: JsValue) -> Result<JsValue, JsValue> {
        let opts: to_object::ToObjectOptions = if options.is_object() {
            with_js_error!(serde_wasm_bindgen::from_value(options))?
        } else {
            Default::default()
        };

        let object = to_object::to_object_struct(&self.0, opts);
    ...
    pub fn to_json(&self) -> Result<JsValue, JsValue> {
        let object = to_object::to_object_struct(&self.0, Default::default());
        let js_object = js_sys::Object::new();
