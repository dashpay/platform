...
impl InvalidStateTransitionErrorWasm {
    ...
    pub fn value_of(&self) -> Result<JsValue, JsError> {
        let errors = serde_json::to_value(&self.errors).map_err(|_| JsError::new("Can't serialize consensus errors to json"))?;
        let ser = serde_wasm_bindgen::Serializer::json_compatible();
        let val = errors.serialize(&ser).map_err(|_| JsError::new("Can't serialize consensus errors to json"));
        val
    }
}
