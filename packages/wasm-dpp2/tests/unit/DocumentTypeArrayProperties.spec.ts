...
describe('DataContract: typed array properties (v14)', () => {
  describe('documentTypeArrayProperties()', () => {
    it('should report every typed array with its element type and bounds', () => {
      const contract = buildContract(schemas);

      expect(contract.documentTypeArrayProperties('charter')).to.deep.equal([
        {
          path: 'reasons',
    ...
    it('should return an empty array for a document type declaring none', () => {
      ...
      expect(contract.documentTypeArrayProperties('plain')).to.deep.equal([]);
    });

    it('should throw for an unknown document type', () => {
      const contract = buildContract(schemas);

      expect(() => contract.documentTypeArrayProperties('doesNotExist')).to.throw(/not found/);
    });
  ...
  describe('documentArrayProperties', () => {
    it('should key typed arrays by document type and omit types declaring none', () => {
      const contract = buildContract(schemas);
      const map = contract.documentArrayProperties as Map<string, unknown[]>;

      expect([...map.keys()]).to.deep.equal(['charter']);
      expect(map.get('charter')).to.deep.equal(contract.documentTypeArrayProperties('charter'));
    });

    it('should be empty for a contract declaring no typed arrays at all', () => {
      const contract = buildContract({ plain: schemas.plain });

      expect((contract.documentArrayProperties as Map<string, unknown[]>).size).to.equal(0);
    });
  });
});
