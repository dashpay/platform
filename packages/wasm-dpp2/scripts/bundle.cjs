...
// Copy base type declarations and append additional exports
const baseDts = fs.readFileSync(dtsPath, 'utf8');
const additionalDts = `
// Namespace export for convenient access to all WASM bindings
...
`;
fs.writeFileSync(path.join(distDir, 'dpp.d.ts'), baseDts + additionalDts);

// Create type declaration for compressed version that re-exports from dpp.d.ts
const compressedDts = `// Re-export all types from dpp.d.ts for the compressed version
export * from './dpp.js';
export { default } from './dpp.js';
export { wasm } from './dpp.js';
export { initWasm } from './dpp.js';
`;
fs.writeFileSync(path.join(distDir, 'dpp.compressed.d.ts'), compressedDts);
...
console.log('Wrote dist/dpp.compressed.d.ts');
