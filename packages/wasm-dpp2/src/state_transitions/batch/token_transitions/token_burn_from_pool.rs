...
use dpp::state_transition::batch_transition::token_burn_from_pool_transition::TokenBurnFromPoolTransitionV0;
...
const TOKEN_BURN_FROM_POOL_OPTIONS_TS: &str = r#"
/**
 * Options for constructing a TokenBurnFromPoolTransition. Destroys `amount` tokens held in the token's shielded pool by spending the given notes; the signer must be authorized to burn.
 */
export interface TokenBurnFromPoolTransitionOptions {
...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(typescript_type = "TokenBurnFromPoolTransitionOptions")]
    pub type TokenBurnFromPoolTransitionOptionsJs;
}

/// Non-WASM-instance fields extracted from the constructor options via serde.
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct TokenBurnFromPoolTransitionSimpleFields {
    amount: u64,
    anchor: Vec<u8>,
    proof: Vec<u8>,
...
impl From<TokenBurnFromPoolTransition> for TokenBurnFromPoolTransitionWasm {
    fn from(transition: TokenBurnFromPoolTransition) -> Self {
        Self(transition)
    }
}

impl From<TokenBurnFromPoolTransitionWasm> for TokenBurnFromPoolTransition {
    fn from(transition: TokenBurnFromPoolTransitionWasm) -> Self {
        transition.0
    }
}
...
impl TokenBurnFromPoolTransitionWasm {
    ...
    pub fn constructor(
        options: TokenBurnFromPoolTransitionOptionsJs,
    ) -> WasmDppResult<TokenBurnFromPoolTransitionWasm> {
        ...
        let fields: TokenBurnFromPoolTransitionSimpleFields =
        ...
        Ok(TokenBurnFromPoolTransitionWasm(
...
impl_wasm_type_info!(TokenBurnFromPoolTransitionWasm, TokenBurnFromPoolTransition);
