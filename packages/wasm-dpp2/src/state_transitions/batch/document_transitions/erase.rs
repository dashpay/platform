...
use crate::state_transitions::batch::generators::generate_erase_transition;
...
use dpp::state_transition::batch_transition::DocumentEraseTransition;
use wasm_bindgen::prelude::wasm_bindgen;

#[wasm_bindgen(typescript_custom_section)]
const DOCUMENT_ERASE_OPTIONS_TS: &str = r#"
export interface DocumentEraseTransitionOptions {
    document: Document;
...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(typescript_type = "DocumentEraseTransitionOptions")]
    pub type DocumentEraseTransitionOptionsJs;
}

#[wasm_bindgen(js_name = "DocumentEraseTransition")]
pub struct DocumentEraseTransitionWasm(DocumentEraseTransition);

impl From<DocumentEraseTransition> for DocumentEraseTransitionWasm {
    fn from(document_erase_transition: DocumentEraseTransition) -> Self {
        DocumentEraseTransitionWasm(document_erase_transition)
    }
}

#[wasm_bindgen(js_class = DocumentEraseTransition)]
impl DocumentEraseTransitionWasm {
    #[wasm_bindgen(constructor)]
    pub fn constructor(
        options: DocumentEraseTransitionOptionsJs,
    ) -> WasmDppResult<DocumentEraseTransitionWasm> {
        ...
        let rs_erase_transition = generate_erase_transition(
            &document,
        ...
        Ok(DocumentEraseTransitionWasm(rs_erase_transition))
    }
    ...
    pub fn from_document_transition(
        ...
        transition.erase_transition()
    }
}

impl From<DocumentEraseTransitionWasm> for DocumentEraseTransition {
    fn from(document_erase_transition: DocumentEraseTransitionWasm) -> Self {
        document_erase_transition.0
    }
}

impl_wasm_type_info!(DocumentEraseTransitionWasm, DocumentEraseTransition);
