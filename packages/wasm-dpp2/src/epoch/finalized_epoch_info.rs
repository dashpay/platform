...
impl FinalizedEpochInfoWasm {
    ...
    pub fn from_json(js_value: JsValue) -> WasmDppResult<FinalizedEpochInfoWasm> {
        let json_value: JsonValue = serde_wasm_bindgen::from_value(js_value)
            .map_err(|e| WasmDppError::serialization(e.to_string()))?;
        let info: FinalizedEpochInfo = serde_json::from_value(json_value)
            .map_err(|e| WasmDppError::serialization(e.to_string()))?;
        Ok(FinalizedEpochInfoWasm(info))
    }
    ...
    pub fn from_object(js_value: JsValue) -> WasmDppResult<FinalizedEpochInfoWasm> {
        let info: FinalizedEpochInfo = serde_wasm_bindgen::from_value(js_value)
            .map_err(|e| WasmDppError::serialization(e.to_string()))?;
        Ok(FinalizedEpochInfoWasm(info))
    }
}
