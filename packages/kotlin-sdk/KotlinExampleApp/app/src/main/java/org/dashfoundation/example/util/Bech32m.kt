...
/**
 * Minimal bech32m (BIP-350) encoder/decoder — port of the Swift SDK's
 * `Bech32m` helper (`Sources/SwiftDashSDK/Address/PlatformAddressInfo.swift`).
 * Used by the example app to display shielded (Orchard) receive addresses
 * and to detect bech32m Platform / Orchard recipients in the send flow;
 * the authoritative parse for a *send* still happens on the Rust side
 * (`PlatformAddress::from_bech32m_string`, `OrchardAddress::from_raw_bytes`).
 */
object Bech32m {
