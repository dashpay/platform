...
/**
 * Top up an identity's credit balance by building + broadcasting a **new
 * Core asset lock** (ID-05) — the same funding mechanism as identity
 * registration (`CreateIdentityScreen`), distinct from
    ...
    Scaffold(
        ...
        Column(
            ...
            SubmitButton(
                ...
                scope.launch {
                    try {
                        // Single FFI call — Rust builds the asset lock, waits
                        // for IS/CL, and submits the IdentityTopUp transition.
                        // Matches CreateIdentityScreen's accountIndex = 0.
                        val balance = mgr.identityCredits.topUpFromCore(
