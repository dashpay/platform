...
describe('subscribeToBlockHeadersWithChainLocksHandlerFactory', () => {
  ...
  describe('getHistoricalBlockHeaders and cache', () => {
    const blockHash = Buffer.from('00000bafbc94add76cb75e2ec92894837288a481e5c005f6563d91623bf8bc2c', 'hex');
    ...
    beforeEach(() => {
      ...
      getHistoricalBlockHeadersIteratorMock = getHistoricalBlockHeadersIteratorFactory(coreAPIMock);
    ...
    it('should call for rpc when nothing in the cache', async () => {
      coreAPIMock.getBlockStats.resolves({ height: 1 });
    ...
    it('should use cache when miss something in the tail', async () => {
      ...
      expect(coreAPIMock.getBlockHeaders).to.be
        .calledOnceWithExactly(BlockHeader.fromRawBlock(differentFakeBlockHeaderHex).hash, 3);
    ...
    it('should call for rpc if theres no cache', async () => {
      coreAPIMock.getBlockStats.resolves({ height: 1 });
      coreAPIMock.getBestBlockHeight.resolves(3);
      coreAPIMock.getBlockHash.resolves(blockHash.toString('hex'));
      coreAPIMock.getBlockHeaders.resolves([fakeBlockHeaderHex, fakeBlockHeaderHex,
      ...
      expect(coreAPIMock.getBlockHeaders.callCount).to.be.equal(1);
      expect(coreAPIMock.getBlockHeaders).to.be
        .calledOnceWithExactly(blockHash.toString('hex'), 3);
    ...
    it('should use cache when missing in the middle', async () => {
      ...
      expect(coreAPIMock.getBlockHeaders.callCount).to.be.equal(1);
      expect(coreAPIMock.getBlockHeaders).to.be
        .calledOnceWithExactly(BlockHeader.fromRawBlock(differentFakeBlockHeaderHex).hash, 4);
