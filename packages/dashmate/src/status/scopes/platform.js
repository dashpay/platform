...
const getMasternodeState = async (scope, config, createRpcClient, getConnectionHost) => {
...
const handleTenderdash = async (scope, config, dockerCompose, getConnectionHost, masternodeReady) => {
...
function getPlatformScopeFactory(dockerCompose,
  ...
  async function getPlatformScope(config) {
    ...
    const result = await Promise.allSettled([
      handleTenderdash(scope, config, dockerCompose, getConnectionHost, masternodeReady),
      handleDrive(scope, config, dockerCompose, masternodeReady),
    ]);
