...
impl<C> Platform<C> {
    ...
    /// owes it (its reward shares and its own proposer payout). Every `AddToIdentityBalance` is
    /// converted against the state before the batch, so two of them for one identity (two
    /// masternodes naming the same `payToId`, or a proposer that is also a `payToId`) each
    /// computed its balance from the same previous one. Generation 0 built one operation per
    ...
    ///
    /// The payouts join the batch as the identity credits they are instead of grove operations
    /// converted here: the block's `apply_drive_operations` converts them against the same state
    /// and routes the credits that repay a recipient's debt to the processing fee pool of the
    /// block's epoch, after the batch's own fee distribution. A plain grove batch cannot carry them, and generation 0,
