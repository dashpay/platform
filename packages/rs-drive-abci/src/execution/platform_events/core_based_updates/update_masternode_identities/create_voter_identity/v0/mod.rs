    ...
    C: CoreRPCLike,
{
    pub(super) fn create_voter_identity_v0(
        ...
        let voting_identifier = Self::get_voter_identifier(pro_tx_hash, voting_key, platform_version)?;
        let mut identity = Identity::create_basic_identity(voting_identifier, platform_version)?;
        identity.add_public_keys([Self::get_voter_identity_key(*voting_key, 0, platform_version)?]);
        Ok(identity)
    }
