    ...
    C: CoreRPCLike,
{
    ...
    ///
    /// v1 (protocol v15, state sync): identical to v0 except that
    /// `validator_set_update` runs BEFORE the root hash is computed (it only mutates the
    /// in-memory block platform state, never grovedb, so the move cannot change the root
    /// hash or the rotation outcome), and the reduced platform state — including the
    /// post-rotation next validator set — is then written into the replicated grovedb
    /// state immediately before the root hash, so it is covered by this block's app hash
    /// and a state-synced node can reconstruct the full platform state from it.
    ///
    /// # Arguments
    ///
    ...
    pub(super) fn run_block_proposal_v1(
        &self,
    ...
    ) -> Result<ValidationResult<block_execution_outcome::v0::BlockExecutionOutcome, Error>, Error>
    {
        ...
        // next_validator_set_quorum_hash) and never touches grovedb, so the rotation
        // outcome and the root hash are unaffected by the move; it must come first so
        // the reduced platform state written below carries the post-rotation state.
