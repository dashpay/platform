...
mod tests {
    ...
    #[test]
    fn data_contract_update_check_tx_latest_protocol_version_delta() {
        let platform_config = PlatformConfig {
            testing_configs: PlatformTestConfig {
                disable_instant_lock_signature_verification: true,
                ..Default::default()
        ...
        // The latest protocol version defaults to the delta-based form: only
        // the new document type goes on the wire.
        let update_transition = DataContractUpdateTransition::from_contract_update(
            dashpay_created_contract.data_contract(),
            &modified_dashpay_contract,
            2,
            platform_version,
        )
        .expect("expected a state transition");
        ...
        let mut update_contract_state_transition: StateTransition = update_transition.into();
        ...
        assert_eq!(
            update_processing_result.aggregated_fees().processing_fee,
            // from protocol version 14 the contract's version item is stored beside the contract;
            // the delta still pays 24 billion credits less than the full-contract update above
            3002932650
        );
