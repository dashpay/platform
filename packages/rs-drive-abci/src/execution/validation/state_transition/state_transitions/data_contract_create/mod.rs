...
impl StateTransitionActionTransformerV0 for DataContractCreateTransition {
    fn transform_into_action<C: CoreRPCLike>(
        ...
        match platform_version.drive_abci.validation.state_transitions.contract_create_state_transition.transform_into_action {
...
impl StateTransitionValidationV0 for DataContractCreateTransition {
    fn validate_structure(
        ...
        match platform_version.drive_abci.validation.state_transitions.contract_create_state_transition.structure {
    ...
    fn validate_identity_and_signatures(
        ...
        let platform_version = PlatformVersion::get(protocol_version)?;
        match platform_version.drive_abci.validation.state_transitions.contract_create_state_transition.identity_signatures {
            0 => self.validate_identity_and_signatures_v0(drive, transaction),
            version => Err(Error::Execution(ExecutionError::UnknownVersionMismatch {
                method: "data contract create transition: validate_identity_and_signatures".to_string(),
                known_versions: vec![0],
                received: version,
            }))
        }
    ...
    fn validate_state<C: CoreRPCLike>(
        ...
        let platform_version = PlatformVersion::get(platform.state.current_protocol_version_in_consensus())?;
        match platform_version.drive_abci.validation.state_transitions.contract_create_state_transition.state {
