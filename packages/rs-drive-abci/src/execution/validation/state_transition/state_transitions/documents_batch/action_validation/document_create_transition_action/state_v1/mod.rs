...
use crate::execution::validation::state_transition::documents_batch::state::v0::fetch_documents::{fetch_document_with_id, has_contested_document_with_document_id};
use crate::platform_types::platform::PlatformStateRef;

pub(super) trait DocumentCreateTransitionActionStateValidationV1 {
...
impl DocumentCreateTransitionActionStateValidationV1 for DocumentCreateTransitionAction {
    fn validate_state_v1(
        &self,
        platform: &PlatformStateRef,
        owner_id: Identifier,
        ...
        if let Some((contested_document_resource_vote_poll, _)) = self.prefunded_voting_balance() {
            // -->> Introduced in V1 <<--
            // We should check to see if a contested document already exists in the state with the same id
            // This could happen if we used the same entropy for multiple different contests
            let (contested_document_already_exists, fee_result) =
                has_contested_document_with_document_id(
                    ...
                    contested_document_resource_vote_poll.document_type()?,
                    self.base().id(),
