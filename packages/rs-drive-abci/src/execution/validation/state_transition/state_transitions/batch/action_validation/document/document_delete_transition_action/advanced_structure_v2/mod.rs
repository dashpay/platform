...
pub(in crate::execution::validation::state_transition::state_transitions::batch::action_validation) trait DocumentDeleteTransitionActionStructureValidationV2 {
    fn validate_structure_v2(&self) -> Result<SimpleConsensusValidationResult, Error>;
}
impl DocumentDeleteTransitionActionStructureValidationV2 for DocumentDeleteTransitionAction {
    /// From protocol 15 a keep-history type takes part in the document
    /// lifecycle: a delete removes the document from ordinary reads and leaves
    /// its revisions readable, so v2 no longer refuses a delete because the
    /// type keeps history. What it refuses instead is a keep-history type that
    /// carries a contested index, which the parser rejects for new contracts
    /// but which a contract stored before protocol 15 could still declare. A
    /// contested resource is awarded outside transition validation, at an id
    /// derived from the winner rather than from the contested values, so that
    /// award can land on an id whose retained history already exists.
    fn validate_structure_v2(&self) -> Result<SimpleConsensusValidationResult, Error> {
