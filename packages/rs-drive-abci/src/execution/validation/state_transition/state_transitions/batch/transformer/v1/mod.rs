...
use drive::state_transition_action::batch::v1::{
    BatchTransitionActionV1, BatchedTransitionActionV1,
};

use crate::execution::validation::state_transition::batch::state::v0::fetch_documents::fetch_documents_for_transitions_of_any_format_knowing_contract_and_document_type;
...
pub(in crate::execution::validation::state_transition::state_transitions::batch) trait BatchTransitionTransformerV1
{
    ...
    ) -> Result<ConsensusValidationResult<BatchTransitionActionV1>, Error>;
}
...
trait BatchTransitionInternalTransformerV1 {
    ...
    fn transform_document_transitions_within_contract_v1(
        ...
        owner_id: Identifier,
        document_transitions: &BTreeMap<&String, Vec<DocumentTransitionRefV1<'_>>>,
        user_fee_increase: UserFeeIncrease,
        execution_context: &mut StateTransitionExecutionContext,
        transaction: TransactionArg,
        platform_version: &PlatformVersion,
    ) -> Result<ConsensusValidationResult<Vec<BatchedTransitionActionV1>>, Error>;
    #[allow(clippy::too_many_arguments)]
    fn transform_document_transitions_within_document_type_v1(
        ...
        owner_id: Identifier,
        document_transitions: &[DocumentTransitionRefV1<'_>],
        user_fee_increase: UserFeeIncrease,
        execution_context: &mut StateTransitionExecutionContext,
        transaction: TransactionArg,
        platform_version: &PlatformVersion,
    ...
    fn transform_document_transition_v1(
        ...
        block_info: &BlockInfo,
        data_contract_fetch_info: Arc<DataContractFetchInfo>,
        transition: DocumentTransitionRefV1<'_>,
    ...
    fn find_replaced_document_v1<'a>(
        document_transition: DocumentTransitionRefV1<'_>,
...
impl BatchTransitionTransformerV1 for BatchTransition {
    fn try_into_action_v1(
        ...
        let mut document_transitions_by_contracts_and_types: BTreeMap<
            &Identifier,
            BTreeMap<&String, Vec<DocumentTransitionRefV1>>,
        > = BTreeMap::new();
        ...
        let validation_result_documents = document_transitions_by_contracts_and_types
            ...
            .collect::<Result<Vec<ConsensusValidationResult<Vec<BatchedTransitionActionV1>>>, Error>>(
            )?;
        ...
        if validation_result.has_data() {
            ...
            let batch_transition_action = BatchTransitionActionV1 {
                owner_id,
                transitions,
                user_fee_increase,
            };
...
impl BatchTransitionInternalTransformerV1 for BatchTransition {
    ...
    fn transform_token_transitions_within_contract_v1(
        ...
        let validation_result = token_transitions
            ...
            .map(|result| result.map(|result| result.map(BatchedTransitionActionV1::Batched)))
            .collect::<Result<Vec<ConsensusValidationResult<BatchedTransitionActionV1>>, Error>>(
            )?;
        let validation_result =
    ...
    fn transform_document_transitions_within_document_type_v1(
        ...
        let replace_and_transfer_transitions = document_transitions
            ...
            .filter(|transition| {
                matches!(
                    transition,
                    DocumentTransitionRefV1::Replace(_)
                        | DocumentTransitionRefV1::Transfer(_)
                        | DocumentTransitionRefV1::Purchase(_)
                        | DocumentTransitionRefV1::UpdatePrice(_)
                )
            })
            .copied()
            .collect::<Vec<_>>();
    ...
    /// mismatch, missing target document, etc.) emits a nonce-bump action so
    /// the submitter pays for validation work that has already run.
    ///
    ...
    fn transform_document_transition_v1<'a>(
        ...
        match transition {
            DocumentTransitionRefV1::Create(document_create_transition) => {
                ...
                execution_context
                    .add_operation(ValidationOperation::PrecalculatedOperation(fee_result));
                Ok(document_create_action.map(BatchedTransitionActionV1::Batched))
            }
            DocumentTransitionRefV1::Replace(document_replace_transition) => {
                let validation_result =
                ...
                Ok(document_replace_action.map(BatchedTransitionActionV1::Batched))
            }
            DocumentTransitionRefV1::Delete(document_delete_transition) => {
                ...
                Ok(batched_action.map(BatchedTransitionActionV1::Batched))
            }
            DocumentTransitionRefV1::IndexOnlyDelete(document_index_only_delete_transition) => {
            ...
            DocumentTransitionRefV1::Erase(document_erase_transition) => {
            ...
            DocumentTransitionRefV1::Transfer(document_transfer_transition) => {
                let validation_result =
                    Self::find_replaced_document_v1(transition, replaced_documents);
                ...
                Ok(document_transfer_action.map(BatchedTransitionActionV1::Batched))
            }
            DocumentTransitionRefV1::UpdatePrice(document_update_price_transition) => {
                let validation_result =
                    Self::find_replaced_document_v1(transition, replaced_documents);
                ...
                Ok(document_update_price_action.map(BatchedTransitionActionV1::Batched))
            }
            DocumentTransitionRefV1::Purchase(document_purchase_transition) => {
                let validation_result =
                    Self::find_replaced_document_v1(transition, replaced_documents);
                ...
                Ok(document_purchase_action.map(BatchedTransitionActionV1::Batched))
            }
        }
    }
