...
use crate::execution::validation::state_transition::batch::action_validation::token::token_shielded_pool_common::{
    charge_drive_operations, validate_minimum_token_pool_notes, validate_token_not_paused, validate_token_pool_anchor_exists,
...
impl TokenUnshieldTransitionActionStateValidationV0 for TokenUnshieldTransitionAction {
    ...
    /// side of the transfer checks (exists, paused token; a pooled token can never freeze an
    /// account, so no frozen check is read or billed) plus
