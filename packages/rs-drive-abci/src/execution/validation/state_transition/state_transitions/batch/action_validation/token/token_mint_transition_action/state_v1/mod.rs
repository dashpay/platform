...
pub(in crate::execution::validation::state_transition::state_transitions::batch::action_validation) trait TokenMintTransitionActionStateValidationV1 {
...
impl TokenMintTransitionActionStateValidationV1 for TokenMintTransitionAction {
    fn validate_state_v1(
        &self,
        platform: &PlatformStateRef,
        owner_id: Identifier,
        ...
        // configures no max supply. v0 checked only a configured max supply, so a mint past
        ...
        if token_total_supply
            .checked_add(self.mint_amount())
            .is_none_or(|total_supply_after_mint| total_supply_after_mint > max_supply)
        {
            return Ok(SimpleConsensusValidationResult::new_with_error(
