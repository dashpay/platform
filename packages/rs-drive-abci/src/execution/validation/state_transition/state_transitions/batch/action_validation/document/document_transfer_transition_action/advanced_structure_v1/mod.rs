...
pub(in crate::execution::validation::state_transition::state_transitions::batch::action_validation) trait DocumentTransferTransitionActionStructureValidationV1 {
    fn validate_structure_v1(
        &self,
        platform_version: &PlatformVersion,
    ) -> Result<SimpleConsensusValidationResult, Error>;
}
impl DocumentTransferTransitionActionStructureValidationV1 for DocumentTransferTransitionAction {
    fn validate_structure_v1(
        &self,
        platform_version: &PlatformVersion,
    ) -> Result<SimpleConsensusValidationResult, Error> {
        ...
        // -->> Introduced in V1 <<--
        // The document changes owner: a `distinctFrom: $ownerId` property of the stored
        // document must differ from the new owner, which the action already carries on
        // the document. The data was schema-validated when it was written, so every
        // value compared is a 32-byte identifier.
        document_type
