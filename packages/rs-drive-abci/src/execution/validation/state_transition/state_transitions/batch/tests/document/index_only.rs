...
//! auto-selection of the V1 (delete-by-values) variant for indexOnly
//! types, and the structure gate pairing each delete variant with its
//! storage mode.
...
mod index_only_tests {
    ...
    /// The structure gate pairs delete variants with storage modes: a V0
    /// (by-id) delete on an indexOnly type is refused. (The default path
    /// auto-selects V1 — this test forces V0 through the method feature
    /// version override.)
    #[tokio::test]
    async fn test_v0_delete_on_index_only_type_is_refused() {
        let platform_version = PlatformVersion::latest();
        let mut platform = TestPlatformBuilder::new()
            .build_with_mock_rpc()
            .set_genesis_state();
        ...
        let v0_delete = BatchTransition::new_document_deletion_transition_from_document(
            alice_like,
            like_type,
            &alice_key,
            4,
