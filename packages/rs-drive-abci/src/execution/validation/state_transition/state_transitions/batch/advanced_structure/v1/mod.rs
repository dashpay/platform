...
impl DocumentsBatchStateTransitionStructureValidationV1 for BatchTransition {
    fn validate_advanced_structure_from_state_v1(
        ...
        if let Some(dpp::identity::contract_bounds::ContractBounds::Scoped(scope)) =
            signing_key.contract_bounds()
        {
            use dpp::identity::contract_bounds::authentication_scope::permissions;
            use drive::state_transition_action::batch::batched_transition::document_transition::document_base_transition_action::DocumentBaseTransitionActionAccessorsV0;
            let unauthorized = self
                .transitions_iter()
                .any(|member| !scope.allows_transition(member));
            let unauthorized_payment = !scope.allows(permissions::DOCUMENT_TOKEN_PAYMENT)
                && action.transitions().iter().any(|member| {
                    matches!(member, BatchedTransitionAction::DocumentAction(doc)
                        if doc.base().token_cost().is_some_and(|(_, _, amount)| amount > 0))
                });
            if unauthorized || unauthorized_payment {
                ...
                return Ok(ConsensusValidationResult::new_with_data_and_errors(
                    ...
                    vec![dpp::consensus::signature::ScopedKeyOutOfScopeError::new(
                        signing_key.id(),
                    )
                    .into()],
                ));
