...
use dpp::state_transition::batch_transition::batched_transition::{BatchedTransitionRefV1, DocumentTransitionRefV1};
use dpp::state_transition::batch_transition::document_base_transition::document_base_transition_trait::DocumentBaseTransitionAccessors;
use dpp::validation::ConsensusValidationResult;

use dpp::version::PlatformVersion;

use drive::state_transition_action::batch::v1::{BatchTransitionActionV1, BatchedTransitionActionV1};
...
use crate::execution::validation::state_transition::state_transitions::batch::action_validation::document::document_erase_transition_action::DocumentEraseTransitionActionValidation;
...
pub(in crate::execution::validation::state_transition::state_transitions::batch) trait DocumentsBatchStateTransitionStructureValidationV1
{
    fn validate_advanced_structure_from_state_v1(
        ...
        network: Network,
        action: &BatchTransitionActionV1,
...
impl DocumentsBatchStateTransitionStructureValidationV1 for BatchTransition {
    fn validate_advanced_structure_from_state_v1(
        ...
        if !security_levels.contains(&signing_key.security_level()) {
            ...
            let first_transition = self.first_transition_v1().ok_or(Error::Execution(ExecutionError::CorruptedCodeExecution("There must be at least one state transition as this is already verified in basic validation")))?;

            let bump_action = StateTransitionAction::BumpIdentityDataContractNonceAction(
                BumpIdentityDataContractNonceAction::from_batched_transition_ref_v1(
                    first_transition,
                    self.owner_id(),
        ...
        for transition in self.transitions_iter_v1() {
            if let BatchedTransitionRefV1::Document(DocumentTransitionRefV1::Create(
                create_transition,
            )) = transition
            {
                // Validate the ID
        ...
        for transition in action.transitions() {
            match transition {
                BatchedTransitionActionV1::DocumentErase(erase_action) => {
                    let result = erase_action.validate_structure(platform_version)?;
                    if !result.is_valid() {
                        let bump_action = StateTransitionAction::BumpIdentityDataContractNonceAction(
