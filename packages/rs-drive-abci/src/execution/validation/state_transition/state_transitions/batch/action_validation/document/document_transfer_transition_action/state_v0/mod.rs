...
pub(in crate::execution::validation::state_transition::state_transitions::batch::action_validation) trait DocumentTransferTransitionActionStateValidationV0 {
...
impl DocumentTransferTransitionActionStateValidationV0 for DocumentTransferTransitionAction {
    fn validate_state_v0(
        ...
        // generation serves: only a document type parsed from the `ttl` keyword, which
        // earlier versions do not read, has a time to live to check. From 14, a document
        // whose time to live has passed is no longer transferred.
