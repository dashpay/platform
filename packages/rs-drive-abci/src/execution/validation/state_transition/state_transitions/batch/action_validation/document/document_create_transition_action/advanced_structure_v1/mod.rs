    ...
    fn validate_structure_v1(
        ...
        if !(network == Network::Testnet && block_info.epoch.index < 2080) {
            ...
            match (expected_vote_poll, self.prefunded_voting_balance()) {
                ...
                // -->> End Introduced in V1 <<--
                (None, None) => {}
                // A document type's contested index only ever describes a contested document
                // resource poll; the identity contender kind is opened by other means.
                (Some(VotePoll::IdentityContenderVotePoll(_)), _) => {
                    return Err(Error::Execution(ExecutionError::CorruptedCodeExecution(
                        "a contested index resolves to a contested document resource vote poll",
                    )));
                }
            }
        }
