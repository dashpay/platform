...
use drive::state_transition_action::batch::batched_transition::token_transition::token_claim_to_pool_transition_action::{
    TokenClaimToPoolTransitionAction, TokenClaimToPoolTransitionActionAccessorsV0,
};

pub(in crate::execution::validation::state_transition::state_transitions::batch::action_validation) trait TokenClaimToPoolTransitionActionStateValidationV0 {
...
impl TokenClaimToPoolTransitionActionStateValidationV0 for TokenClaimToPoolTransitionAction {
    /// The claim itself (who may claim, how much, up to which moment) was resolved when the action
    /// was built; here the max supply is checked and the outputs-only bundle must carry exactly
    /// the resolved amount.
    fn validate_state_v0(
        &self,
        platform: &PlatformStateRef,
