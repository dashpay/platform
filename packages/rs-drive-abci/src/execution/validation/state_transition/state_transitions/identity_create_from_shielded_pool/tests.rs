...
#[test]
fn should_validate_scoped_keys_through_shielded_creation_dispatch() {
    ...
    for (case, id, document_type, time_ms, error_code) in [
        ("valid", contract.id(), "contactRequest", 99, None),
        ("expired", contract.id(), "contactRequest", 100, Some(10535)),
        (
            "unknown contract",
        ...
        let scope = AuthenticationScope::V0(AuthenticationScopeV0 {
            contracts: vec![ContractScope {
                id,
                document_types: Some(vec![document_type.into()]),
            }],
