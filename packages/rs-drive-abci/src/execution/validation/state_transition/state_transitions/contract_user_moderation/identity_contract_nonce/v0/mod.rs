...
pub(in crate::execution::validation::state_transition::state_transitions) trait ContractUserModerationStateTransitionIdentityContractNonceV0
{
...
impl ContractUserModerationStateTransitionIdentityContractNonceV0
    for ContractUserModerationTransition
{
    /// The moderator's nonce for the contract, as a contract update's: read with its fee and
    /// checked against the transition's.
    fn validate_identity_contract_nonce_v0(
        &self,
        platform: &PlatformStateRef,
        block_info: &BlockInfo,
