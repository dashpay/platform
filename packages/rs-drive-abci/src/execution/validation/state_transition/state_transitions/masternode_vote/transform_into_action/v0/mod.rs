...
impl MasternodeVoteStateTransitionTransformIntoActionValidationV0 for MasternodeVoteTransition {
    fn transform_into_action_v0<C>(
        ...
        if validation_mode != ValidationMode::NoValidation {
            ...
            let Vote::ResourceVote(resource_vote) = self.vote();
            let maybe_existing_resource_vote_choice = match resource_vote.vote_poll() {
                ...
                VotePoll::IdentityContenderVotePoll(vote_poll) => {
                    // An identity contender vote poll has no Lock choice
                    if !resource_vote.vote_poll().allows_lock_vote()
                        && resource_vote.resource_vote_choice() == ResourceVoteChoice::Lock
                    {
                ...
                maybe_existing_resource_vote_choice
            {
                if existing_resource_vote_choice == resource_vote.resource_vote_choice() {
                    ...
                    return Ok(ConsensusValidationResult::new_with_error(
                        ConsensusError::StateError(StateError::MasternodeVoteAlreadyPresentError(
                        ...
                        .votes_allowed_per_masternode
                {
                    // We are submitting a vote for something we already have
                    return Ok(ConsensusValidationResult::new_with_error(
                        ConsensusError::StateError(StateError::MasternodeVotedTooManyTimesError(
