...
impl MasternodeVoteTransitionBalanceValidationV0 for MasternodeVoteTransition {
    fn validate_advanced_minimum_balance_pre_check_v0(
        ...
        let balance_id = vote.specialized_balance_id()?.ok_or(Error::Execution(ExecutionError::CorruptedCodeExecution("In this version there should always be a specialized balance id")))?;
        let maybe_balance = drive.fetch_prefunded_specialized_balance(balance_id.to_buffer(), tx, platform_version)?;
        ...
        let Some(balance) = maybe_balance else {
            ...
            return Ok(ConsensusValidationResult::new_with_error(PrefundedSpecializedBalanceNotFoundError::new(balance_id).into()));
        };
        if balance < platform_version.fee_version.state_transition_min_fees.masternode_vote {
            return Ok(ConsensusValidationResult::new_with_error(
                PrefundedSpecializedBalanceInsufficientError::new(balance_id, balance, platform_version.fee_version.state_transition_min_fees.masternode_vote)
                    .into(),
            ));
        }

        Ok(ConsensusValidationResult::new_with_data(BTreeMap::from([(balance_id, balance)])))
    }
}
