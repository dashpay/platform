...
impl Commit {
    ...
    /// Return quorum hash as a static array of size 32
    pub fn quorum_hash(&self) -> Result<[u8; 32], AbciError> {
        let h: [u8; 32] = self.inner.quorum_hash[..]
            .try_into()
            .map_err(|e| AbciError::BadRequestDataSize(format!("invalid quorum hash: {:?}", e)))?;
        Ok(h)
    }
    ...
    pub fn verify_signature(
        ...
        let quorum_hash = match self.quorum_hash() {
            Ok(h) => h,
