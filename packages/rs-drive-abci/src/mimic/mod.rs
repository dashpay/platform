        ...
        if !options.independent_process_proposal_verification {
            ...
            *block_execution_context_guard = original_block_execution_context;
            ...
            //we are in init chain
            {
                ...
                // Drain per-transition savepoints left by process_raw_state_transitions_v1
                // (protocol v14+) so we land on the post-init-chain state, matching the
                // genesis-path drain in prepare_proposal/process_proposal. The root-hash
                // assertion below verifies the landing point.
