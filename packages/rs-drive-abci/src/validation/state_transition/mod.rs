...
use dpp::state_transition::StateTransitionAction::{DataContractCreateAction, DataContractUpdateAction, DocumentsBatchAction, IdentityCreateAction, IdentityCreditWithdrawalAction, IdentityTopUpAction, IdentityUpdateAction};
use dpp::validation::{SimpleValidationResult, ValidationResult};
use drive::drive::Drive;
use crate::platform::Platform;

pub trait StateTransitionValidation<C> {
    fn validate_all(&self, platform: &Platform<C>) -> Result<ValidationResult<StateTransitionAction>, ProtocolError> {
        let result = self.validate_type()?;
        if !result.is_valid() {
            return Ok(ValidationResult::<StateTransitionAction>::new_with_errors(result.errors))
        }
