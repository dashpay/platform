...
mod tests {
    ...
    fn setup_history_test_with_n_updates(mut data_contract: DataContract, platform: &TempPlatform<MockCoreRPCLike>, n: u64) {
        ...
        insert_n_contract_updates(&data_contract, &platform, n);
    }

    fn assert_property_exists(data_contract: &DataContract, property: &str) {
    ...
    pub mod fetch_contract_with_history {
        ...
        pub fn should_fetch_10_latest_contract_without_offset_and_limit_and_start_date_0() {
            ...
            let contract_created_at = 1000;

            let contract_id = *data_contract.id.as_bytes();
            setup_history_test_with_n_updates(data_contract, &platform, total_updates);
            ...
            let contract_history = platform
                .drive
                .fetch_contract_with_history(contract_id, None, start_at_date, limit, offset)
                .expect("to get contract history");

            let keys = contract_history.keys().copied().collect::<Vec<u64>>();
            let contracts = contract_history.values().collect::<Vec<&DataContract>>();

            let expected_offset: usize = 10;
            // We've created contract at time 1000 and applied 20 updates, so the latest update is at 21000
            let latest_update: u64 = total_updates as u64 * 1000 + 1000;
            let expected_first_result_key: u64 = latest_update - expected_offset as u64 * 1000 + 1000;

            assert_eq!(contract_history.len(), expected_offset);

            for (i, (key, contract)) in contract_history.iter().enumerate() {
                let expected_key: u64 = expected_first_result_key + i as u64 * 1000;
                assert_eq!(key, &expected_key);

                let prop_index = i + total_updates as usize - expected_offset;
                assert_property_exists(contracts[i], format!("newProp{}", prop_index).as_str());
            }
        }
        ...
        pub fn should_fetch_with_limit_without_offset() {
            ...
            // Fetch from time 0 with a limit and without offset
            let contract_history = platform
                .drive
                .fetch_contract_with_history(contract_id, None, start_at_date, limit, offset)
                .expect("to get contract history");
            ...
            assert_eq!(contract_history.len(), limit.unwrap() as usize);

            let expected_offset: usize = limit.unwrap() as usize;
            let latest_update: u64 = total_updates as u64 * 1000 + 1000;
            let expected_first_result_key: u64 = latest_update - expected_offset as u64 * 1000 + 1000;
        ...
        pub fn should_fetch_without_limit_with_offset() {
            ...
            assert_eq!(contract_history.len(), expected_limit);

            let oldest_result_offset: usize = expected_limit + offset.unwrap() as usize;
            let latest_update: u64 = total_updates as u64 * 1000 + 1000;
            let expected_first_result_key: u64 = latest_update - oldest_result_offset as u64 * 1000 + 1000;

            for (i, (key, contract)) in contract_history.iter().enumerate() {
                let expected_key: u64 = expected_first_result_key + i as u64 * 1000;
                assert_eq!(key, &expected_key);

                let prop_index = i + total_updates as usize - oldest_result_offset;
                assert_property_exists(contracts[i], format!("newProp{}", prop_index).as_str());
            }
        }
        ...
        pub fn should_fetch_with_non_zero_start_date() {
            ...
            let oldest_result_offset: usize = expected_limit + expected_offset as usize;
            let latest_update: u64 = total_updates as u64 * 1000 + 1000;
            let expected_first_result_key: u64 = latest_update - oldest_result_offset as u64 * 1000 + 1000;
        ...
        pub fn should_fail_with_limit_higher_than_10() {
            ...
            assert!(matches!(error, Error::Drive(DriveError::InvalidContractHistoryFetchLimit(
                11,
            ))));
        }
        ...
        pub fn should_fetch_empty_with_start_date_after_latest_update() {
            ...
            assert!(contract_history.is_empty());
        }

        #[test]
        pub fn should_fail_with_non_existent_contract_id() {
            let TestData {
                raw_state_transition,
                mut data_contract,
                mut platform,
