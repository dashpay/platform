...
mod tests {
    ...
    #[test]
    fn test_query_identity_contract_nonce_with_none_version_returns_decoding_error() {
        let (platform, state, version) = setup_platform(None, Network::Testnet, None);

        let request = GetIdentityContractNonceRequest { version: None };

        let result = platform
            .query_identity_contract_nonce(request, &state, version)
            .expect("expected query to succeed");

        assert!(matches!(
            result.errors.as_slice(),
            [QueryError::DecodingError(msg)] if msg.contains("could not decode identity contract nonce query")
        ));
    }

    #[test]
    fn test_query_identity_contract_nonce_with_valid_v0_version() {
        let (platform, state, version) = setup_platform(None, Network::Testnet, None);

        let request = GetIdentityContractNonceRequest {
            version: Some(RequestVersion::V0(GetIdentityContractNonceRequestV0 {
