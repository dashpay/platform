...
use dapi_grpc::platform::v0::get_identity_by_non_unique_public_key_hash_response::get_identity_by_non_unique_public_key_hash_response_v0::IdentityResponse;
...
impl<C> Platform<C> {
    pub(super) fn query_identity_by_non_unique_public_key_hash_v0(
        ...
        let start_after = if let Some(start_after) = start_after {
            Some(check_validation_result_with_data!(Bytes32::from_vec(start_after)
                .map(|bytes| bytes.0)
                .map_err(|_| QueryError::InvalidArgument(
                    "public key hash must be 20 bytes long".to_string()
                ))))
        ...
        let response = if prove {
            let proof = self.drive.prove_full_identity_by_non_unique_public_key_hash(
                public_key_hash,
                start_after,
                None,
                platform_version,
            ...
            GetIdentityByNonUniquePublicKeyHashResponseV0 {
                result: Some(get_identity_by_non_unique_public_key_hash_response_v0::Result::Proof(
                    self.response_proof_v0(platform_state, proof),
                )),
                metadata: Some(self.response_metadata_v0(platform_state)),
            }
        } else {
            let maybe_identity = self.drive.fetch_full_identity_by_non_unique_public_key_hash(
                public_key_hash,
                start_after,
                None,
                platform_version,
            ...
            let serialized_identity =  maybe_identity.map(|identity|
...
mod tests {
    ...
    fn test_identity_not_found() {
        ...
        assert_eq!(result.data, Some(GetIdentityByNonUniquePublicKeyHashResponseV0 {
            metadata: Some(ResponseMetadata { height: 0, core_chain_locked_height: 0, epoch: 0, time_ms: 0, protocol_version: 9, chain_id: "chain_id".to_string() }),
            result: Some(get_identity_by_non_unique_public_key_hash_response_v0::Result::Identity(IdentityResponse {
                identity: None,
            })),
        }));
    }
