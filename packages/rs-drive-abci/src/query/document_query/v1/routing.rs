...
/// nothing to skip on either `prove` setting: grovedb counts the
/// skipped region from the subtree aggregates rather than walking it,
/// and both executors take that descent. Only the proved result
