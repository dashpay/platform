...
//! `dash_platform_queries::documents::proto_conversions`, shared
//! with the client-side `DocumentQuery::try_from_request` so the
//! bytes the server decodes and the bytes the proof verifier decodes
//! cannot drift. This module only maps the shared crate's neutral
//! [`DecodeError`] onto this crate's [`QueryError`] surface:
...
use dash_platform_queries::documents::proto_conversions::{self as shared, DecodeError};
