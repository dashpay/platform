...
mod composite_trust_boundary {
    ...
    fn should_verify_the_composite_composition_through_from_proof() {
        ...
        let drive_query: DriveDocumentQuery = (&query).try_into().expect("rich query converts");
        let (from_drive, _, _) =
            <CompositeDocuments as FromProof<DriveDocumentQuery>>::maybe_from_proof_with_metadata(
                drive_query,
                response.clone(),
                Network::Testnet,
                platform_version(),
        ...
        assert_eq!(from_drive, verified);
        assert_eq!(from_borrowed, verified);
        let verified = verified.expect("a proven page is Some, even when empty");
