...
impl<C> Platform<C> {
    pub(super) fn query_contested_resource_vote_status_v0(
        &self,
        GetContestedResourceVoteStateRequestV0 { contract_id, document_type_name, index_name, index_values, include_documents, start_at_identifier_info, prove }: GetContestedResourceVoteStateRequestV0,
        ...
        let index = check_validation_result_with_data!(document_type.find_contested_index().ok_or(QueryError::InvalidArgument(format!(
        ...
        let index = check_validation_result_with_data!(document_type
            .inc (document_type_name.as_str())
            .map_err(|_| QueryError::InvalidArgument(format!(
                "document type {} not found for contract {}",
                document_type_name, contract_id
            ))));
        ...
        let response = if prove {
            ...
            GetContestedResourceVoteStateResponseV0 {
                result: Some(get_contested_resource_vote_status_response_v0::Result::ContestedResourceVoters(balance)),
                metadata: Some(self.response_metadata_v0(platform_state)),
            }
        };
