...
mod tests {
    ...
    fn test_patch_version() {
        ...
        impl Drop for PatchCleanup {
            fn drop(&mut self) {
                cleanup_version_patches();
            }
        }

        let _cleanup = PatchCleanup;

        fn patch_1_5_test(mut platform_version: PlatformVersion) -> PlatformVersion {
            platform_version
                .drive_abci
                .query
                .document_query
        ...
        fn patch_1_10_test(mut platform_version: PlatformVersion) -> PlatformVersion {
            platform_version.drive_abci.query.document_query.max_version = 10;

            platform_version
        }

        fn patch_2_30_test(mut platform_version: PlatformVersion) -> PlatformVersion {
