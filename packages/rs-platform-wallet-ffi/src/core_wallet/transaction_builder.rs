...
impl CoreAccountTypeFFI {
    ...
    ///
    /// UTXO-reservation reconciliation (`reservations.rs`) is defined only over
    /// `StandardAccountType` (BIP44/BIP32) — the send funding source — so
    /// CoinJoin has no equivalent there and broadcasts without reconciliation.
...
///
/// # Safety
/// `builder` must be a valid, non-destroyed pointer; `wallet` a valid core-wallet handle.
#[no_mangle]
pub unsafe extern "C" fn core_wallet_tx_builder_set_funding(
    ...
    let wallet = unwrap_option_or_return!(CORE_WALLET_STORAGE.with_item(wallet, |w| w.clone()));
