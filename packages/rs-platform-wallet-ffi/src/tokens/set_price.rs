...
//! `price_per_token == 0` clears the schedule (disabling direct
//! purchase); any non-zero value is lifted into
//! `TokenPricingSchedule::SinglePrice(price)` on the Rust side.
//!
//! Set Price is group-capable; callers pass the same flat
//! `(group_info_kind, position, action_id, action_is_proposer)` tuple
//! used by Burn / Freeze / etc.
//!
//! The richer tiered-price schedule (`SetPrices`) is intentionally not
//! surfaced here — a future FFI / Swift wrapper can add it without
...
/// Set the direct-purchase price for the token at `token_position` on
/// `token_contract_id`. `price_per_token == 0` disables direct
/// purchase by clearing the schedule.
///
/// # Safety
...
pub unsafe extern "C" fn platform_wallet_token_set_price(
                ...
                Err(e) => {
                    if !out_error.is_null() {
                        *out_error = PlatformWalletFFIError::new(
                            ...
                            format!("token_set_price failed: {e}"),
                        );
                    }
                    PlatformWalletFFIResult::ErrorWalletOperation
                }
