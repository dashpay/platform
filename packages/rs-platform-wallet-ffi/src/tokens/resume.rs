//! FFI binding for `IdentityWallet::token_resume_with_external_signer`.
//!
//! Resume is an emergency action and is group-capable; same shape as
//! Pause. There is no `amount` and no target identity — the action
//! targets the token itself, re-enabling all operations after a
//! preceding Pause.
...
/// Resume all operations for the token at `token_position` on
/// `token_contract_id`.
///
/// # Safety
...
pub unsafe extern "C" fn platform_wallet_token_resume(
    ...
    PLATFORM_WALLET_STORAGE
        .with_item(wallet_handle, |wallet| {
            ...
            let result = block_on_worker(async move {
                ...
                identity_wallet
                    .token_resume_with_external_signer(
                        id,
                        contract_id,
                        token_position,
                        public_note_str,
