//! C-ABI callback handles backing the
...
//! module exposes those two operations as paired vtable + handle
//! types so a Rust-owned derivation loop can call into Swift for
//! each one.
//!
...
//!   wallet's mnemonic per `dash_sdk_derive_and_persist_identity_keys`
//!   invocation).
...
//! nothing in common with how the mnemonic is fetched.
//!
...
//! prompts can take seconds), Keychain reads and writes are
//! sub-millisecond, so both callbacks here are fully synchronous —
//! no completion-callback ceremony, no `tokio::time::timeout`.
...
//! # Lifetime / cleanup
//!
//! The Swift side calls `dash_sdk_mnemonic_resolver_create` /
//! `dash_sdk_identity_key_persister_create`, which return opaque
//! `*mut MnemonicResolverHandle` / `*mut IdentityKeyPersisterHandle`
//! pointers. Pass them to
//! `dash_sdk_derive_and_persist_identity_keys`. Then call the
//! matching `_destroy` function — that fires the supplied destructor
//! callback (typically the `Unmanaged.fromOpaque(...).release()`
//! pattern Swift uses for its `KeychainSigner` ctx) so the Swift
//! object can drop.
...
// ---------------------------------------------------------------------------
// Mnemonic resolver — Rust → Swift "fetch BIP-39 mnemonic for wallet_id"
// ---------------------------------------------------------------------------
...
/// of-two cap keeps the inline Rust stack buffer (used in
/// [`crate::dash_sdk_derive_and_persist_identity_keys`]) cheap to
/// zero-on-drop.
...
pub mod mnemonic_resolver_result {
    ...
    /// Mnemonic exceeded [`MNEMONIC_RESOLVER_BUFFER_CAPACITY`].
...
/// into [`crate::dash_sdk_derive_and_persist_identity_keys`].
#[repr(C)]
pub struct MnemonicResolverHandle {
    pub(crate) ctx: *mut c_void,
    pub(crate) vtable: *mut MnemonicResolverVTable,
}
...
/// Destroy a previously-created persister handle. Safe on null.
///
/// # Safety
/// `handle` must have been produced by
...
mod tests {
    ...
    unsafe extern "C" fn fake_resolver_destroy(_ctx: *mut c_void) {
        RESOLVER_DESTROYED.store(true, Ordering::SeqCst);
    }
    ...
    #[test]
    fn resolver_create_destroy_roundtrip() {
        unsafe {
            let h = dash_sdk_mnemonic_resolver_create(
                std::ptr::null_mut(),
                fake_resolve,
                fake_resolver_destroy,
            );
            assert!(!h.is_null());
            assert!(!(*h).vtable.is_null());

            // Drive the resolver once to confirm the wired-up callback fires.
            let mut buf = [0u8; MNEMONIC_RESOLVER_BUFFER_CAPACITY];
            let mut len: usize = 0;
            let rc = ((*(*h).vtable).resolve)(
                (*h).ctx,
                [0u8; 32].as_ptr(),
            ...
            assert_eq!(rc, mnemonic_resolver_result::SUCCESS);
            assert!(len > 0);

            dash_sdk_mnemonic_resolver_destroy(h);
        }
        assert!(RESOLVER_CALLED.load(Ordering::SeqCst));
        assert!(RESOLVER_DESTROYED.load(Ordering::SeqCst));
    }
    ...
    #[test]
    fn destroy_handles_null() {
        unsafe {
            // Should be no-ops.
            dash_sdk_mnemonic_resolver_destroy(std::ptr::null_mut());
