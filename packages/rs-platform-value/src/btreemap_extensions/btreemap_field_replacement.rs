...
/// The members a `list[]` or `list[3]` component addresses under `map`: every
/// element, or the one at the index. `None` when the map has no such list,
/// which is an absent optional property and nothing to replace.
fn array_members<'a>(
    map: &'a mut Vec<(Value, Value)>,
    list_name: &str,
    index: Option<usize>,
) -> Result<Option<Vec<&'a mut Value>>, Error> {
    let Some(array_value) = map.get_optional_key_mut(list_name) else {
        return Ok(None);
    };
    let array = array_value.to_array_mut()?;
    match index {
        Some(index) => match array.get_mut(index) {
            Some(member) => Ok(Some(vec![member])),
            None => Err(Error::StructureError(format!(
                "element at position {index} in array does not exist"
            ))),
        },
        None => Ok(Some(array.iter_mut().collect())),
    }
}
        ...
        let next_values = current_values
            ...
            .map(|current_value| {
                if current_value.is_map() {
                    ...
                    // `list[]` names the members of a list: the values to replace when it
                    // ends the path, the values to descend into otherwise
                    if let Some((list_name, index)) = is_array_path(path_component)? {
                        let Some(members) = array_members(map, list_name, index)? else {
                            return Ok(None);
                        };
...
impl BTreeValueMapReplacementPathHelper for BTreeMap<String, Value> {
    fn replace_at_path(
        ...
        // `list[]` as the first component: the members of a top-level list
        if let Some((list_name, index)) = is_array_path(first_path_component)? {
            let Some(array_value) = self.get_mut(list_name) else {
                return Ok(());
            };
            let array = array_value.to_array_mut()?;
            let members: Vec<&mut Value> = match index {
                Some(index) => match array.get_mut(index) {
...
mod tests {
    ...
    #[test]
    fn replace_at_path_top_level_list_members() {
        let mut map = BTreeMap::new();
        map.insert(
            "ids".to_string(),
    ...
    #[test]
    fn replace_at_path_nested_list_members() {
        ...
        // a list of maps: descend into the members
    ...
    #[test]
    fn replace_at_path_absent_list_is_nothing_to_replace() {
        let mut map = BTreeMap::new();
