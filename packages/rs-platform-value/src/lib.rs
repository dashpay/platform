...
use bincode::de::{Decoder, UntrustedDecoder};
...
pub(crate) trait ValueLeafReader<D: Decoder> {
    ...
    /// Called before each element is pushed into container storage. Readers that pre-size their
    /// storage from the declared length need nothing here; readers that start containers empty
    /// grow them one fallible reservation at a time so a hostile length cannot force a large
    /// allocation.
    fn reserve_element<T>(&mut self, _storage: &mut Vec<T>) -> Result<(), Self::Error> {
        Ok(())
    }
                ...
                ValueDecodeFrame::Map {
                    ...
                    if pending_key.is_none() {
                        *pending_key = Some(value);
                        ...
                        leaves.reserve_element(entries)?;
...
/// The leaf reader behind the [`DecodeUntrusted`] impl. Same wire grammar and depth limit as
/// [`NativeLeaves`], but nothing is sized from a declared length: containers start empty and grow
/// one fallible reservation at a time, and byte leaves go through bincode's own untrusted decoders.
struct UntrustedLeaves;

impl<D: UntrustedDecoder> ValueLeafReader<D> for UntrustedLeaves {
    type Error = DecodeError;
    ...
    fn reserve_element<T>(&mut self, storage: &mut Vec<T>) -> Result<(), DecodeError> {
        storage
            .try_reserve(1)
    ...
    fn bytes(&mut self, decoder: &mut D) -> Result<Vec<u8>, DecodeError> {
        DecodeUntrusted::decode_untrusted(decoder)
    }
...
impl<Context> DecodeUntrusted<Context> for Value {
    fn decode_untrusted<D: UntrustedDecoder<Context = Context>>(
        decoder: &mut D,
    ) -> Result<Self, DecodeError> {
        decode_value_with(decoder, &mut UntrustedLeaves)
    }
}
