...
use platform_wallet_sqlite::migrations as mig;
...
fn tc027_smoke_insert_every_table() {
    ...
    let cases: &[(&str, &str, &[&dyn rusqlite::ToSql])] = &[
        ...
        (
            ...
            "INSERT INTO core_derived_addresses (wallet_id, account_type, address, derivation_path, used) VALUES (?1, 'Standard', 'addr', '', 0)",
            &[&wallet_id.as_slice()],
        ),
        (
            "core_sync_state",
...
fn tc028_idempotent_reopen() {
    ...
    drop(persister);
    let cfg = platform_wallet_sqlite::SqlitePersisterConfig::new(&path);
    let _p2 = platform_wallet_sqlite::SqlitePersister::open(cfg).expect("reopen");
    drop(tmp);
}
