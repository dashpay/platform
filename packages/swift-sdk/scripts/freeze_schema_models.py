...
`--check` is what a reviewer or CI runs: the frozen files are a pure
function of `FREEZES` and the repository history, so a clean check proves
nobody edited a frozen copy by hand.
"""
...
def block_end(lines, start):
    ...
    Braces on `//` comment lines are ignored so a `// { ... }` note inside
    a body cannot truncate the copy. Block comments are not parsed, so one
    is refused rather than risk a silently truncated copy.
    """
    depth = 0
    opened = False
    for i in range(start, len(lines)):
        ...
        if stripped.startswith("//"):
            continue
        if "/*" in lines[i]:
            raise SystemExit(f"line {i + 1}: block comments are not supported")
