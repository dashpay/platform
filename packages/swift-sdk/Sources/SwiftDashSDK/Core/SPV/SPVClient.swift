...
class SPVClient: @unchecked Sendable {
    ...
    init(network: Network = DashSDKNetwork(rawValue: 1), dataDir: String?, startHeight: UInt32, eventHandlers: SPVEventHandlers) throws {
        ...
        // Convert event handlers to FFI callbacks
        self.spvEventHandlers = eventHandlers
        let ffiEventCallbacks = eventHandlers.intoFFIEventCallbacks()

        // Create client
        let client = dash_spv_ffi_client_new(configPtr, ffiEventCallbacks)
