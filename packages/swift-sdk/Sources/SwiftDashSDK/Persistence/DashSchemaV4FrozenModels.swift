...
// Historical V4 definitions. Preserve stored properties, defaults, indexes and
// relationship topology exactly. Constructors only support migration fixtures;
// application behavior belongs to the live models. All relationship targets are
// nested here so SwiftData cannot pull a live entity into the historical schema.
extension DashSchemaV4 {
    ...
    final class PersistentDPNSName {
        ...
        init(
            ...
            self.label = label
            self.normalizedLabel = label.lowercased().replacingOccurrences(of: "o", with: "0").replacingOccurrences(of: "i", with: "1").replacingOccurrences(of: "l", with: "1")
            self.parentDomainName = parentDomainName
            self.normalizedParentDomainName = parentDomainName.lowercased().replacingOccurrences(of: "o", with: "0").replacingOccurrences(of: "i", with: "1").replacingOccurrences(of: "l", with: "1")
    ...
    final class PersistentTxo {
        ...
        init(
            ...
            var outpoint = transaction.txid
            var outputIndex = vout.littleEndian
            withUnsafeBytes(of: &outputIndex) { outpoint.append(contentsOf: $0) }
