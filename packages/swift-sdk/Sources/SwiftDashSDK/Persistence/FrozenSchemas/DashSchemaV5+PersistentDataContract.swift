...
// `PersistentDataContract` exactly as schema DashSchemaV5 registered it, generated by
...
extension DashSchemaV5.PersistentDataContract {
    static func predicate(contractId: String) -> Predicate<DashSchemaV5.PersistentDataContract> {
        guard let idData = Data.identifier(fromBase58: contractId) else {
            return #Predicate<DashSchemaV5.PersistentDataContract> { _ in false }
        }
        return #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.id == idData
        }
    }

    static func predicate(ownerId: Data) -> Predicate<DashSchemaV5.PersistentDataContract> {
        #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.ownerId == ownerId
        }
    }

    static func predicate(name: String) -> Predicate<DashSchemaV5.PersistentDataContract> {
        #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.name.localizedStandardContains(name)
        }
    }

    static var contractsWithTokensPredicate: Predicate<DashSchemaV5.PersistentDataContract> {
        #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.hasTokens == true
        }
    }

    static func predicate(keyword: String) -> Predicate<DashSchemaV5.PersistentDataContract> {
        #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.keywordRelations.contains { $0.keyword == keyword }
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV5.PersistentDataContract> {
        #Predicate<DashSchemaV5.PersistentDataContract> { contract in
            contract.lastSyncedAt == nil || contract.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV5.PersistentDataContract> {
    ...
    static func contractsWithTokensPredicate(network: Network) -> Predicate<DashSchemaV5.PersistentDataContract> {
        let target = network.rawValue
        return #Predicate<DashSchemaV5.PersistentDataContract> { contract in
