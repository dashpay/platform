...
// `PersistentIdentity` exactly as schema DashSchemaV2 registered it, generated by
...
extension DashSchemaV2.PersistentIdentity {
    static func predicate(identityId: Data) -> Predicate<DashSchemaV2.PersistentIdentity> {
        #Predicate<DashSchemaV2.PersistentIdentity> { identity in
            identity.identityId == identityId
        }
    }

    static var walletOwnedIdentitiesPredicate: Predicate<DashSchemaV2.PersistentIdentity> {
        #Predicate<DashSchemaV2.PersistentIdentity> { identity in
            identity.wallet != nil
        }
    }

    static func predicate(type: IdentityType) -> Predicate<DashSchemaV2.PersistentIdentity> {
        let typeString = type.rawValue
        return #Predicate<DashSchemaV2.PersistentIdentity> { identity in
            identity.identityType == typeString
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV2.PersistentIdentity> {
        #Predicate<DashSchemaV2.PersistentIdentity> { identity in
            identity.lastSyncedAt == nil || identity.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV2.PersistentIdentity> {
    ...
    static func walletOwnedIdentitiesPredicate(network: Network) -> Predicate<DashSchemaV2.PersistentIdentity> {
        let target = network.rawValue
        return #Predicate<DashSchemaV2.PersistentIdentity> { identity in
    ...
    static func fetch(
        ...
        identityId: Data
    ) -> DashSchemaV2.PersistentIdentity? {
        let target = identityId
        let descriptor = FetchDescriptor<DashSchemaV2.PersistentIdentity>(
