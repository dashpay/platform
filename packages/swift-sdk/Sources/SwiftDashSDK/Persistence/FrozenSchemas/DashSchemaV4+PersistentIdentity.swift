...
// `PersistentIdentity` exactly as schema DashSchemaV4 registered it, generated by
...
extension DashSchemaV4.PersistentIdentity {
    static func predicate(identityId: Data) -> Predicate<DashSchemaV4.PersistentIdentity> {
        #Predicate<DashSchemaV4.PersistentIdentity> { identity in
            identity.identityId == identityId
        }
    }

    static var walletOwnedIdentitiesPredicate: Predicate<DashSchemaV4.PersistentIdentity> {
        #Predicate<DashSchemaV4.PersistentIdentity> { identity in
            identity.wallet != nil
        }
    }

    static func predicate(type: IdentityType) -> Predicate<DashSchemaV4.PersistentIdentity> {
        let typeString = type.rawValue
        return #Predicate<DashSchemaV4.PersistentIdentity> { identity in
            identity.identityType == typeString
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV4.PersistentIdentity> {
        #Predicate<DashSchemaV4.PersistentIdentity> { identity in
            identity.lastSyncedAt == nil || identity.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV4.PersistentIdentity> {
    ...
    static func walletOwnedIdentitiesPredicate(network: Network) -> Predicate<DashSchemaV4.PersistentIdentity> {
        let target = network.rawValue
        return #Predicate<DashSchemaV4.PersistentIdentity> { identity in
    ...
    static func fetch(
        ...
        identityId: Data
    ) -> DashSchemaV4.PersistentIdentity? {
        let target = identityId
        let descriptor = FetchDescriptor<DashSchemaV4.PersistentIdentity>(
