...
// `PersistentDataContract` exactly as schema DashSchemaV2 registered it, generated by
...
extension DashSchemaV2.PersistentDataContract {
    static func predicate(contractId: String) -> Predicate<DashSchemaV2.PersistentDataContract> {
        guard let idData = Data.identifier(fromBase58: contractId) else {
            return #Predicate<DashSchemaV2.PersistentDataContract> { _ in false }
        }
        return #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.id == idData
        }
    }

    static func predicate(ownerId: Data) -> Predicate<DashSchemaV2.PersistentDataContract> {
        #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.ownerId == ownerId
        }
    }

    static func predicate(name: String) -> Predicate<DashSchemaV2.PersistentDataContract> {
        #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.name.localizedStandardContains(name)
        }
    }

    static var contractsWithTokensPredicate: Predicate<DashSchemaV2.PersistentDataContract> {
        #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.hasTokens == true
        }
    }

    static func predicate(keyword: String) -> Predicate<DashSchemaV2.PersistentDataContract> {
        #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.keywordRelations.contains { $0.keyword == keyword }
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV2.PersistentDataContract> {
        #Predicate<DashSchemaV2.PersistentDataContract> { contract in
            contract.lastSyncedAt == nil || contract.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV2.PersistentDataContract> {
    ...
    static func contractsWithTokensPredicate(network: Network) -> Predicate<DashSchemaV2.PersistentDataContract> {
        let target = network.rawValue
        return #Predicate<DashSchemaV2.PersistentDataContract> { contract in
