...
// `PersistentDataContract` exactly as schema DashSchemaV4 registered it, generated by
...
extension DashSchemaV4.PersistentDataContract {
    static func predicate(contractId: String) -> Predicate<DashSchemaV4.PersistentDataContract> {
        guard let idData = Data.identifier(fromBase58: contractId) else {
            return #Predicate<DashSchemaV4.PersistentDataContract> { _ in false }
        }
        return #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.id == idData
        }
    }

    static func predicate(ownerId: Data) -> Predicate<DashSchemaV4.PersistentDataContract> {
        #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.ownerId == ownerId
        }
    }

    static func predicate(name: String) -> Predicate<DashSchemaV4.PersistentDataContract> {
        #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.name.localizedStandardContains(name)
        }
    }

    static var contractsWithTokensPredicate: Predicate<DashSchemaV4.PersistentDataContract> {
        #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.hasTokens == true
        }
    }

    static func predicate(keyword: String) -> Predicate<DashSchemaV4.PersistentDataContract> {
        #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.keywordRelations.contains { $0.keyword == keyword }
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV4.PersistentDataContract> {
        #Predicate<DashSchemaV4.PersistentDataContract> { contract in
            contract.lastSyncedAt == nil || contract.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV4.PersistentDataContract> {
    ...
    static func contractsWithTokensPredicate(network: Network) -> Predicate<DashSchemaV4.PersistentDataContract> {
        let target = network.rawValue
        return #Predicate<DashSchemaV4.PersistentDataContract> { contract in
