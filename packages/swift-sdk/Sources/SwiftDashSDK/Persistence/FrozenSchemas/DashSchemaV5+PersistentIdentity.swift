...
// `PersistentIdentity` exactly as schema DashSchemaV5 registered it, generated by
...
extension DashSchemaV5.PersistentIdentity {
    static func predicate(identityId: Data) -> Predicate<DashSchemaV5.PersistentIdentity> {
        #Predicate<DashSchemaV5.PersistentIdentity> { identity in
            identity.identityId == identityId
        }
    }

    static var walletOwnedIdentitiesPredicate: Predicate<DashSchemaV5.PersistentIdentity> {
        #Predicate<DashSchemaV5.PersistentIdentity> { identity in
            identity.wallet != nil
        }
    }

    static func predicate(type: IdentityType) -> Predicate<DashSchemaV5.PersistentIdentity> {
        let typeString = type.rawValue
        return #Predicate<DashSchemaV5.PersistentIdentity> { identity in
            identity.identityType == typeString
        }
    }

    static func needsSyncPredicate(olderThan date: Date) -> Predicate<DashSchemaV5.PersistentIdentity> {
        #Predicate<DashSchemaV5.PersistentIdentity> { identity in
            identity.lastSyncedAt == nil || identity.lastSyncedAt! < date
        }
    }

    static func predicate(network: Network) -> Predicate<DashSchemaV5.PersistentIdentity> {
    ...
    static func walletOwnedIdentitiesPredicate(network: Network) -> Predicate<DashSchemaV5.PersistentIdentity> {
        let target = network.rawValue
        return #Predicate<DashSchemaV5.PersistentIdentity> { identity in
    ...
    static func fetch(
        ...
        identityId: Data
    ) -> DashSchemaV5.PersistentIdentity? {
        let target = identityId
        let descriptor = FetchDescriptor<DashSchemaV5.PersistentIdentity>(
