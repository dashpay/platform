    ...
    func persistIdentities(
        ...
        for entry in upserts {
            ...
            if let existing = try? backgroundContext.fetch(descriptor).first {
                ...
                // Rows born from the changeset path start observed
                // (`isLocal: false`): the wallet-linkage attach at the
                // bottom of this loop promotes wallet-owned ones, and
                // key-import flows (LoadIdentityView) promote
                // walletless identities the user holds keys for.
