...
public final class CoreTransactionBuilder {
    ...
    /// `buildSigned` consumes and frees the builder on the Rust side; once
    /// consumed, `deinit` must not destroy it again.
    ...
    public func buildSigned(
        ...
        // The builder is freed by the FFI on every path — never destroy it again.
        consumed = true
        try result.check()
