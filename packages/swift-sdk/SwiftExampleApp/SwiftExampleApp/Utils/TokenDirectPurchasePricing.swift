...
///
/// Prices are `u64` credits on-chain. Drive saturates an overflowing
/// `singlePrice` multiplication to `UInt64.max`, while `setPrices` uses checked
/// multiplication and rejects overflow. A zero price is valid. Token amounts
/// are additionally bounded by DPP's u48 distribution parameter.
enum TokenDirectPurchasePricing: Equatable {
    ...
    /// minimum sale amount, an amount outside DPP's u48 domain, or a tiered
    /// total that overflows `UInt64`.
    ///
    ...
    func cost(forAmount amount: UInt64) -> UInt64? {
        guard amount > 0, amount <= Self.maximumTokenAmount else { return nil }

        let perToken: UInt64
        let saturatesOnOverflow: Bool
        switch self {
        case let .singlePrice(price):
            perToken = price
            saturatesOnOverflow = true
        case let .setPrices(tiers):
            ...
            perToken = tierPrice
            saturatesOnOverflow = false
        }

        let (total, overflow) = perToken.multipliedReportingOverflow(by: amount)
        if overflow {
            return saturatesOnOverflow ? UInt64.max : nil
        }
        return total
    }

    /// DPP `MAX_DISTRIBUTION_PARAM` (`2^48 - 1`).
    private static let maximumTokenAmount: UInt64 = (1 << 48) - 1
