    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          TextField("Enter bech32m or hex address", text: $viewModel.addressInput)
          ...
          HStack {
            Button("Use Bech32m Test") {
              viewModel.addressInput = GetAddressInfoViewModel.testBech32m
            }
            .font(.caption)
            ...
            Button("Use Hex Test") {
              viewModel.addressInput = GetAddressInfoViewModel.testAddressHex
            }
            .font(.caption)
            .foregroundColor(.blue)
          }
          ...
          if !viewModel.addressInput.isEmpty {
            VStack(alignment: .leading, spacing: 4) {
              HStack {
                Image(systemName: viewModel.detectedFormat.icon)
                  .foregroundColor(viewModel.detectedFormat.color)
                Text(viewModel.detectedFormat.description)
                  .font(.caption)
                  .foregroundColor(.secondary)
              }
        ...
        Button {
          ...
          HStack {
            ...
            Text(viewModel.isLoading ? "Fetching..." : "Fetch Address Info")
              .fontWeight(.semibold)
          }
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Query balance and nonce for multiple Platform addresses. Enter one address per line. Supports bech32m and hex formats (can be mixed)."
          )
          .font(.body)
          .foregroundColor(.secondary)
        }
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          TextEditor(text: $viewModel.addressesText)
            .frame(height: 120)
            .overlay(
          ...
          HStack {
            Button("Use Bech32m Test") {
              viewModel.addressesText = GetAddressesInfosViewModel.testBech32mAddresses
            }
            .font(.caption)
            .foregroundColor(.blue)
            ...
            Button("Use Hex Test") {
              viewModel.addressesText = GetAddressesInfosViewModel.testHexAddresses
            }
            .font(.caption)
            .foregroundColor(.blue)
          }
        ...
        Button {
          ...
          HStack {
            ...
            Text(viewModel.isLoading ? "Fetching..." : "Fetch Addresses Infos")
              .fontWeight(.semibold)
          }
          .frame(maxWidth: .infinity)
          .padding()
              ...
              VStack(alignment: .leading, spacing: 12) {
                ...
                ForEach(Array(result.infos.values), id: \.addressHex) { info in
                  ...
                  VStack(alignment: .leading, spacing: 4) {
                    HStack {
                      Image(
                        systemName: info.isFound ? "checkmark.circle.fill" : "questionmark.circle"
                      )
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Fetch the trunk state of the address tree. This returns addresses at the top levels of the tree and leaf boundaries for subtrees that need further queries."
          )
          .font(.body)
          .foregroundColor(.secondary)
              ...
              VStack(alignment: .leading, spacing: 16) {
                ...
                if !state.leafBoundaries.isEmpty {
                  VStack(alignment: .leading, spacing: 8) {
                    ...
                    ForEach(Array(state.leafBoundaries.enumerated()), id: \.offset) {
                      index, boundary in
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Query a specific branch of the address tree. Use leaf boundary info from a trunk state query."
          )
          .font(.body)
          .foregroundColor(.secondary)
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Fetch all address balance changes that occurred since the specified block height. Useful for syncing wallet balances after initial sync."
          )
          .font(.body)
          .foregroundColor(.secondary)
        }
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Fetch compacted (merged) address balance changes since a block height. Compacted changes merge multiple blocks into ranges for efficient syncing."
          )
          .font(.body)
          .foregroundColor(.secondary)

          Text(
            "BlockAwareCreditOperation preserves per-block granularity for partial sync scenarios."
          )
          .font(.caption)
          .foregroundColor(.orange)
        }
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Transfer credits between Platform addresses. This creates a state transition on-chain."
          )
          .font(.body)
          .foregroundColor(.secondary)

          Text(
            "⚠️ This is an on-chain transaction. Requires valid addresses with funds and correct private keys."
          )
          .font(.caption)
          .foregroundColor(.orange)
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Address (hex, 42 chars)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("00...(21 bytes = 42 hex chars)", text: $viewModel.inputAddressHex)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .autocapitalization(.none)
              .disableAutocorrection(true)
              .monospaced()
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Amount (credits)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("e.g., 1000000000", text: $viewModel.inputAmount)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .keyboardType(.numberPad)
            if let amount = UInt64(viewModel.inputAmount), amount > 0 {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Private Key (hex, 64 chars)")
              .font(.subheadline)
              .fontWeight(.medium)
            SecureField("32-byte private key as hex", text: $viewModel.inputPrivateKeyHex)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .autocapitalization(.none)
              .disableAutocorrection(true)
              .monospaced()
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Address (hex, 42 chars)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("00...(21 bytes = 42 hex chars)", text: $viewModel.outputAddressHex)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .autocapitalization(.none)
              .disableAutocorrection(true)
              .monospaced()
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Amount (credits)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("e.g., 500000000", text: $viewModel.outputAmount)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .keyboardType(.numberPad)
            if let amount = UInt64(viewModel.outputAmount), amount > 0 {
              Text("\(formatDash(amount)) DASH")
                .font(.caption)
                .foregroundColor(.blue)
            }
        ...
        VStack(alignment: .leading, spacing: 4) {
          ...
          Text(
            "Network fees will be deducted from the input amount. The output amount should be less than input to cover fees."
          )
          .font(.caption)
          .foregroundColor(.secondary)
        }
        ...
        Button {
          ...
          HStack {
            ...
            Text(viewModel.isLoading ? "Transferring..." : "Execute Transfer")
              .fontWeight(.semibold)
          }
          .frame(maxWidth: .infinity)
          .padding()
          .background(viewModel.isLoading || !viewModel.isFormValid ? Color.gray : Color.orange)
          .foregroundColor(.white)
          .cornerRadius(10)
        }
        .disabled(
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text(
            "Withdraw credits from Platform addresses to a Dash Core (L1) address. This creates a state transition on-chain."
          )
          .font(.body)
          .foregroundColor(.secondary)

          Text(
            "⚠️ This is an on-chain transaction. Requires valid Platform addresses with funds and correct private keys."
          )
          .font(.caption)
          .foregroundColor(.orange)
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Dash Core Address (base58)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("e.g., yP8A3...", text: $viewModel.coreAddress)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .autocapitalization(.none)
              .disableAutocorrection(true)
        ...
        VStack(alignment: .leading, spacing: 12) {
          Toggle("Use Change Address", isOn: $viewModel.useChangeAddress)
            .font(.headline)

          if viewModel.useChangeAddress {
            VStack(alignment: .leading, spacing: 4) {
              Text("Change Address (hex, 42 chars)")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("00...(21 bytes = 42 hex chars)", text: $viewModel.changeAddressHex)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .autocapitalization(.none)
                .disableAutocorrection(true)
                .monospaced()
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Core Fee Per Byte")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("e.g., 1", text: $viewModel.coreFeePerByte)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .keyboardType(.numberPad)
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Pooling Strategy")
              .font(.subheadline)
              .fontWeight(.medium)
            Picker("Pooling", selection: $viewModel.poolingStrategy) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          ...
          Text(
            "Network fees will be deducted from the input amount. Change (if any) will be sent to the change address if provided, otherwise it's lost."
          )
          .font(.caption)
          .foregroundColor(.secondary)
        }
        ...
        Button {
          ...
          HStack {
            ...
            Text(viewModel.isLoading ? "Withdrawing..." : "Execute Withdrawal")
              .fontWeight(.semibold)
          }
          .frame(maxWidth: .infinity)
          .padding()
          .background(viewModel.isLoading || !viewModel.isFormValid ? Color.gray : Color.purple)
          .foregroundColor(.white)
          .cornerRadius(10)
        }
        .disabled(
    ...
    ScrollView {
      VStack(spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          Picker("Proof Type", selection: $viewModel.proofType) {
            Text("Instant Lock").tag(
              Addresses.AssetLockProofType.instant as Addresses.AssetLockProofType)
            Text("Chain Lock").tag(
              Addresses.AssetLockProofType.chain as Addresses.AssetLockProofType)
          }
          .pickerStyle(SegmentedPickerStyle())
        }
        .padding()
        ...
        if viewModel.proofType == Addresses.AssetLockProofType.instant {
          VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text("Instant Lock (hex)")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("Instant lock bytes as hex", text: $viewModel.instantLockHex)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .autocapitalization(.none)
                .disableAutocorrection(true)
                .monospaced()
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text("Transaction (hex)")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("Transaction bytes as hex", text: $viewModel.transactionHex)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .autocapitalization(.none)
                .disableAutocorrection(true)
                .monospaced()
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text("Output Index")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("0", text: $viewModel.outputIndex)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .keyboardType(.numberPad)
            }
          }
          ...
          VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text("Core Chain Locked Height")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("Block height", text: $viewModel.coreChainLockedHeight)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .keyboardType(.numberPad)
            }
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text("Out Point (hex, 72 chars)")
                .font(.subheadline)
                .fontWeight(.medium)
              TextField("36 bytes = 72 hex chars (32 txid + 4 vout)", text: $viewModel.outPointHex)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .autocapitalization(.none)
                .disableAutocorrection(true)
                .monospaced()
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Amount (credits, optional)")
              .font(.subheadline)
              .fontWeight(.medium)
            TextField("0 = SDK calculates", text: $viewModel.outputAmount)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .keyboardType(.numberPad)
            if let amount = UInt64(viewModel.outputAmount), amount > 0 {
              Text("\(formatDash(amount)) DASH")
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text("Private Key (hex, 64 chars)")
              .font(.subheadline)
              .fontWeight(.medium)
            SecureField("32-byte private key as hex", text: $viewModel.assetLockPrivateKeyHex)
              .textFieldStyle(RoundedBorderTextFieldStyle())
              .autocapitalization(.none)
              .disableAutocorrection(true)
              .monospaced()
        ...
        Button {
          ...
          HStack {
            ...
            Text(viewModel.isLoading ? "Topping Up..." : "Execute Top Up")
              .fontWeight(.semibold)
          }
          .frame(maxWidth: .infinity)
          .padding()
...
struct CreateIdentityFromAddressesView: View {
  ...
  @State private var result:
    (identityHandle: OpaquePointer, addressInfos: PlatformAddressInfosResult)?
  @State private var errorMessage: String?
  @State private var showResult = false
