...
/// Form for freezing another identity's token balance.
///
/// Inputs: target identity (via `RecipientPickerView`), optional public
/// note. Surfaces a group-action banner whenever the caller's
/// authorization to freeze is gated on a `MainGroup` / `Group:<position>`
/// rule — in that case the submission is sent as a group-action proposal
/// (`GroupActionMode.propose`).
struct TokenFreezeActionView: View {
        ...
        Form {
            ...
            if let groupPosition = inferredGroupPosition {
                Section("Group action") {
                    Label(
                        "This freeze requires a group action.",
    ...
    /// If the freeze rule on this token is gated on a group, return the
    /// group position the caller should propose under. Otherwise nil
    /// (single-signer freeze). Mirrors the Burn / Mint resolver.
    private var inferredGroupPosition: Int? {
        guard let rule = token.freezeRules else { return nil }
    ...
    private func submit() {
        guard
            ...
            submitError = .init(message: "Pick an identity to freeze.")
            return
        }
