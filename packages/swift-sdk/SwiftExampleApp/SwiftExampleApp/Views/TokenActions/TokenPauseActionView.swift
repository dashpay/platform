...
/// Form for pausing all operations on a token.
///
/// Pause is an emergency action — it targets the token itself, so the
...
struct TokenPauseActionView: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                Text("Pausing the token prevents transfers, mints, burns, and other operations until it is resumed.")
                    .font(.subheadline)
            }
            ...
            if let groupPosition = inferredGroupPosition {
                Section("Group action") {
                    Label(
                        "This pause requires a group action.",
    ...
    /// If the emergency-action rule on this token is gated on a group,
    /// return the group position the caller should propose under.
    /// Otherwise nil (single-signer pause). Mirrors the Freeze /
    /// Unfreeze resolver but reads `emergencyActionRules`.
                ...
                await MainActor.run {
                    ...
                    if case .none = groupAction {
                        ...
                        do {
                            ...
                            print("⚠️ TokenPauseActionView: failed to persist isPaused flip: \(error)")
                        }
                    }
                    self.dismiss()
                }
