...
struct AddAccountView: View {
    ...
    private func createAccount() {
        ...
        Task {
            do {
                // Get the underlying wallet from the manager
                guard let walletManager = walletService.walletManager else {
                    throw NSError(domain: "AddAccountView", code: 1, userInfo: [NSLocalizedDescriptionKey: "Wallet manager not available"])
                }

                // Add the account via the wallet manager
                try await walletManager.addAccount(
                    to: wallet,
