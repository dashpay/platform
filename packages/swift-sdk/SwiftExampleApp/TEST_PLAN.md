...
| SH-11 | Create identity from shielded pool (Type 20) | Cross | Common | ✅ |  | `CreateIdentityView` → funding source **Shielded balance** (fixed denominations 0.03 / 0.1 / 0.25 / 0.5 / 1.0 DASH as of v13, gated on the bound pool's balance) → `IdentityRegistrationController` (`.shieldedPool`) → `shieldedIdentityCreateFromPool` → `platform_wallet_manager_shielded_identity_create_from_pool`. Requires a synced shielded pool with sufficient balance. |
