...
| DOC-16 | Erase a deleted keep-history document's revisions | Platform | Thorough | 🧪 |  | Production UI: **Browse Documents** → document → **⋯** → **Erase History…** (shown for a deletable `documentsKeepHistory` type) → `EraseDocumentView` → `platform_wallet_document_erase` (`IdentityWallet::erase_document_with_signer`, keychain signer, 8 MB worker stack). Needs a type with `canBeErased: true` and a document already deleted via `DOC-04`; the first erase must be signed by the owner, later ones by any identity, each removing up to 100 revisions. Check progress with **Platform Queries → Get Document History** (`dash_sdk_document_fetch_history`: `lifecycle.state` DELETED → ERASING → ABSENT, `remaining_revisions`). Not driven on a device; no such type exists on testnet yet. *(Settings builder → Document Erase / `dash_sdk_document_erase_and_wait` remains as a test-signer alternative.)* |
