...
| DOC-16 | Erase a deleted keep-history document's revisions | Platform | Thorough | 🧪 |  | Primary route: **Settings → Platform State Transitions → Document Erase** (`TransitionDetailView` → `dash_sdk_document_erase_and_wait`), which takes the deleted document's id as text: an in-app delete (`DOC-04`) removes the local row, so the Browse Documents route below cannot start after it. Secondary route: **Browse Documents** → document → **⋯** → **Erase History…** (shown when the type's schema sets `canBeErased`) → `EraseDocumentView` → `platform_wallet_document_erase` (`IdentityWallet::erase_document_with_signer`), for a row whose document was deleted elsewhere; the sheet stays open for further chunks and removes the row only once the lifecycle reports ABSENT. Both routes read the lifecycle before and after each broadcast (`SDK.documentGetLifecycle` over `dash_sdk_document_fetch_history`) and show that, because the erase result itself only proves the document absent from ordinary reads, which it already was: a still-DELETED lifecycle with the same retained count is reported as "Not established", ERASING with the remaining count as progress, ABSENT as complete. Needs a type with `canBeErased: true` and a document already deleted; the first erase must be signed by the owner, later ones by any identity, each removing up to 100 revisions. Not driven on a device; no such type exists on testnet yet. |
...
| SH-11 | Create identity from shielded pool (Type 20) | Cross | Common | ✅ |  | `CreateIdentityView` → funding source **Shielded balance** (fixed denominations gated on the network's reported protocol version — 0.1 / 0.3 / 0.5 / 1.0 DASH pre-v13, 0.03 / 0.1 / 0.25 / 0.5 / 1.0 DASH from v13 — and on the bound pool's balance) → `IdentityRegistrationController` (`.shieldedPool`) → `shieldedIdentityCreateFromPool` → `platform_wallet_manager_shielded_identity_create_from_pool`. Requires a synced shielded pool with sufficient balance. |
...
- **Document** — `DOC-01..15`
- **Token** — `TOK-01..20`
- **Shielded** — `SH-01..17`, `CORE-21`
- **DashPay** — `DP-01..19` (`DP-12..19` = shipped invitation create, claim, persistence, and reclaim UI; funded testnet acceptance is not automated)
- **System / Diagnostics** — `SYS-01..08`

**By tag (cross-cutting, the Tags column):**

- **multiwallet** — `CORE-14..23`, `ID-14`, `ID-15`, `TOK-17`, `DPNS-08`, `DP-11`, `DP-14`, `DP-19`, `DOC-15`, `SH-14`, `SH-15`, `SH-16`, `SYS-07`, `SYS-08`
