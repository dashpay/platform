...
final class StateTransitionTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    // Load environment variables (EnvLoader is @MainActor); run on main and return values
    func loadEnvVars() -> (testId: String?, key1: String?, key3: String?) {
      MainActor.assumeIsolated {
        EnvLoader.loadEnvFile()
        return (
          EnvLoader.get("TEST_IDENTITY_ID"),
          EnvLoader.get("TEST_KEY_1_PRIVATE"),
          EnvLoader.get("TEST_KEY_3_PRIVATE")
        )
      }
    }
    let envVars: (testId: String?, key1: String?, key3: String?) =
      Thread.isMainThread
      ? loadEnvVars()
      : DispatchQueue.main.sync(execute: loadEnvVars)
    guard let id = envVars.testId else {
      throw XCTSkip(
        "TEST_IDENTITY_ID not found in environment. Please copy .env.example to .env and add your test credentials."
      )
    }
    testIdentityId = id
    guard let k1 = envVars.key1, let k3 = envVars.key3 else {
      throw XCTSkip(
        "TEST_KEY_1_PRIVATE or TEST_KEY_3_PRIVATE not found in environment. Please copy .env.example to .env and add your test credentials."
      )
    }
    key1Private = try decodePrivateKey(from: k1)
    key3Private = try decodePrivateKey(from: k3)
  ...
  func testIdentityCreditTransferDebug() async throws {
    ...
    print(
      "Checking key3Private: \(key3Private != nil ? "present (\(key3Private.count) bytes)" : "nil")"
    )
  ...
  func testTransferCredits() async throws {
    ...
    do {
      ...
      // Try to fetch identity first (identityGet is @MainActor)
      print("4. Fetching sender identity...")
      do {
        let id = testIdentityId
        let sdkRef = sdk
        let balanceOpt = try await Task { @MainActor in
          let identity = try await sdkRef.identityGet(identityId: id)
          print("   ✅ Identity fetched: \(identity)")
          return identity["balance"] as? UInt64
        }.value
        if let balance = balanceOpt {
      ...
      do {
        ...
        let id = testIdentityId
        let sdkRef = sdk
        let balance = try await Task { @MainActor in
          let identity = try await sdkRef.identityGet(identityId: id)
          return (identity["balance"] as? UInt64) ?? 0
        }.value
  ...
  func testIdentityCreditWithdrawal() async throws {
    ...
    // Create DPPIdentity
    guard let id = testIdentityId, let sdkRef = sdk,
      let idData = Data.identifier(fromBase58: id)
    else {
      throw XCTSkip("Invalid identity ID format or missing SDK")
    }

    let balance = try await Task { @MainActor in
      let identityDict = try await sdkRef.identityGet(identityId: id)
      return (identityDict["balance"] as? UInt64) ?? 0
    }.value
  ...
  func testMinimalTransferFFI() async throws {
    ...
    let transferResult = transferResultPtr.assumingMemoryBound(
      to: DashSDKTransferCreditsResult.self
    ).pointee
    ...
    dash_sdk_transfer_credits_result_free(
  ...
  func testFetchIdentityBalance() async throws {
    ...
    guard let id = testIdentityId, let sdkRef = sdk else {
      XCTFail("Test identity or SDK not set")
      return
    }
    let balance = try await Task { @MainActor in
      let identity = try await sdkRef.identityGet(identityId: id)
