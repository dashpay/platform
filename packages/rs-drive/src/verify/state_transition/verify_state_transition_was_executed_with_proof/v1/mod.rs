...
impl Drive {
    ...
    pub(super) fn verify_state_transition_was_executed_with_proof_v1(
        ...
        let (root_hash, result) = match state_transition {
            ...
            StateTransition::Batch(documents_batch_transition) => {
                if documents_batch_transition.transitions_len_v1() > 1 {
                ...
                let Some(transition) = documents_batch_transition.first_transition_v1() else {
                ...
                match transition {
                    BatchedTransitionRefV1::Document(document_transition) => {
                        ...
                        // prover used.
                        {
                            ...
                            if document_type.index_only() {
                                let values = match document_transition {
                                    DocumentTransitionRefV1::Create(create_transition) => {
                                        create_transition.data().clone()
                                    }
                                    // The indexOnlyDelete kind always
                                    // carries its values.
                                    DocumentTransitionRefV1::IndexOnlyDelete(delete_transition) => {
                                ...
                                // the execution.
                                let outcome = if Self::state_transition_proof_binds_execution_v1(
                                    state_transition,
                        ...
                        let contested_status =
                            if let DocumentTransitionRefV1::Create(create_transition) =
                                document_transition
                            {
                        ...
                        match document_transition {
                            ...
                            DocumentTransitionRefV1::Replace(replace_transition) => {
                            ...
                            DocumentTransitionRefV1::Transfer(transfer_transition) => {
                            ...
                            DocumentTransitionRefV1::Delete(delete_transition) => {
                                if document.is_some() {
                            ...
                            DocumentTransitionRefV1::UpdatePrice(update_price_transition) => {
                            ...
                            DocumentTransitionRefV1::Purchase(purchase_transition) => {
                            ...
                            DocumentTransitionRefV1::Erase(erase_transition) => {
            ...
            StateTransition::Unshield(st) => {
                ...
                let (root_hash, proved_key_values) = Self::verify_merged_query_strict_v1(
                    proof,
    ...
    fn state_transition_proof_binds_execution_v1(
        state_transition: &StateTransition,
        known_contracts_provider_fn: &ContractLookupFn,
    ) -> Result<bool, Error> {
        let binds = match state_transition {
            ...
            StateTransition::Batch(batch) => match batch.first_transition_v1() {
                ...
                Some(BatchedTransitionRefV1::Document(document_transition)) => {
                    use dpp::data_contract::document_type::accessors::DocumentTypeV2Getters;
                    let data_contract_id = document_transition.data_contract_id();
                    let contract = known_contracts_provider_fn(&data_contract_id)?.ok_or(
                        Error::Proof(ProofError::UnknownContract(format!(
                    ...
                    if matches!(document_transition, DocumentTransitionRefV1::Erase(_)) {
                        false
                    } else {
                        !contract
                            .document_type_for_name(document_transition.document_type_name())
                ...
                Some(BatchedTransitionRefV1::Token(token_transition)) => {
                    let data_contract_id = token_transition.data_contract_id();
                    let contract = known_contracts_provider_fn(&data_contract_id)?.ok_or(
                        Error::Proof(ProofError::UnknownContract(format!(
                            "unknown contract with id {} in token verification",
    ...
    fn verify_merged_query_strict_v1(
        proof: &[u8],
...
mod tests {
    ...
    fn verify_batch_document_erase_is_affected_state_not_execution_proved() {
        ...
        use dpp::state_transition::batch_transition::batched_transition::{
            BatchedTransitionV1, DocumentTransitionV1,
        };
        use dpp::state_transition::batch_transition::document_base_transition::v0::DocumentBaseTransitionV0;
        use dpp::state_transition::batch_transition::document_base_transition::DocumentBaseTransition;
        ...
        use dpp::state_transition::batch_transition::{BatchTransition, BatchTransitionV2};
        ...
        let batch = |transition: DocumentTransitionV1| {
            StateTransition::Batch(BatchTransition::V2(BatchTransitionV2 {
                owner_id: Default::default(),
                transitions: vec![BatchedTransitionV1::Document(transition)],
                ..Default::default()
            }))
        };
        let contract_arc = Arc::new(contract.clone());
        ...
        let erase_outcome = verify(batch(DocumentTransitionV1::Erase(
        ...
        let delete_outcome = verify(batch(DocumentTransitionV1::Delete(
    ...
    fn classifier_tags_snapshot_only_families_as_affected_state() {
        ...
        for (name, st) in snapshot_only {
            let binds = Drive::state_transition_proof_binds_execution_v1(&st, no_contracts)
