...
impl Drive {
    pub(super) fn verify_token_info_for_identity_id_v0(
        proof: &[u8],
        token_id: [u8; 32],
        identity_id: [u8; 32],
        verify_subset_of_proof: bool,
        platform_version: &PlatformVersion,
    ) -> Result<(RootHash, Option<IdentityTokenInfo>), Error> {
        let path_query = Self::token_info_for_identity_id_query(token_id, identity_id);
...
mod tests {
    ...
    #[test]
    fn should_prove_and_verify_single_token_info_for_frozen_identity() {
        ...
        let path_query = Drive::token_info_for_identity_id_query(token_id.to_buffer(), identity_id);
        let proof = drive
            .grove_get_proved_path_query(&path_query, None, &mut vec![], &platform_version.drive)
            .expect("expected to get proof");

        let (_, info) = Drive::verify_token_info_for_identity_id(
            proof.as_slice(),
            token_id.to_buffer(),
            identity_id,
            false,
    ...
    #[test]
    fn should_prove_and_verify_absent_token_info_for_identity() {
        let drive = setup_drive_with_initial_state_structure(None);
        let platform_version = PlatformVersion::latest();
