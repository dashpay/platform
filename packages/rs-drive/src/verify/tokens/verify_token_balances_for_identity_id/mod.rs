...
mod tests {
    ...
    #[test]
    fn test_verify_token_balances_for_identity_id_unknown_version() {
        ...
        platform_version
            ...
            .verify
            .token
            .verify_token_balances_for_identity_id = 255;
        ...
        assert!(
            matches!(result, Err(Error::Drive(DriveError::UnknownVersionMismatch { method, known_versions, received }))
                if method == "verify_token_balances_for_identity_id" && known_versions == vec![0] && received == 255
            )
        );
    }
}
