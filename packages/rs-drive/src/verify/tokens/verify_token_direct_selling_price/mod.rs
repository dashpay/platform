...
mod tests {
    ...
    #[test]
    fn test_verify_token_direct_selling_price_unknown_version() {
        ...
        platform_version
            ...
            .verify
            .token
            .verify_token_direct_selling_price = 255;

        let result =
            Drive::verify_token_direct_selling_price(&[], [0u8; 32], false, &platform_version);

        assert!(
            matches!(result, Err(Error::Drive(DriveError::UnknownVersionMismatch { method, known_versions, received }))
                if method == "verify_token_direct_selling_price" && known_versions == vec![0] && received == 255
            )
        );
    }
}
