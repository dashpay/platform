...
mod tests {
    ...
    #[test]
    fn test_verify_token_balance_for_identity_id_unknown_version() {
        ...
        platform_version
            ...
            .verify
            .token
            .verify_token_balance_for_identity_id = 255;

        let result = Drive::verify_token_balance_for_identity_id(
            &[],
            [0u8; 32],
            [0u8; 32],
            false,
        ...
        assert!(
            matches!(result, Err(Error::Drive(DriveError::UnknownVersionMismatch { method, known_versions, received }))
                if method == "verify_token_balance_for_identity_id" && known_versions == vec![0] && received == 255
            )
        );
    }
}
