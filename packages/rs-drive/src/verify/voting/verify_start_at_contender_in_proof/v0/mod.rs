...
impl<'a> ResolvedContestedDocumentVotePollVotesDriveQuery<'a> {
    ...
    #[inline(always)]
    pub(super) fn verify_start_at_contender_in_proof_v0(
        &self,
        proof: &[u8],
        is_proof_subset: bool,
        contender_identity_id: [u8; 32],
        platform_version: &PlatformVersion,
    ) -> Result<(RootHash, Option<Contender>), Error> {
        let start_at_document_path = ;
        let start_at_document_key = contender_identity_id.to_vec();
        ...
        match proved_key_values.len() {
            1 => {
                ...
                if path != start_at_document_path {
                    return Err(Error::Proof(ProofError::CorruptedProof(
                        "we did not get back a contender for the correct path".to_string(),
                    )));
                }
                if key != start_at_document_key {
                    return Err(Error::Proof(ProofError::CorruptedProof(
                        "we did not get back a contender for the correct key".to_string(),
                    )));
                }
            ...
            _ => Err(Error::Proof(ProofError::TooManyElements(
                "expected one contender for start at",
            ))),
        }
    }
}
