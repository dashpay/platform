...
mod tests {
    ...
    #[test]
    fn test_verify_vote_poll_votes_proof_unknown_version() {
        ...
        let query = ResolvedContestedDocumentVotePollVotesDriveQuery {
            ...
            contestant_id: Identifier::default(),
            offset: None,
            limit: None,
            start_at: None,
        ...
        platform_version
            ...
            .verify
            .voting
            .verify_vote_poll_votes_proof = 255;

        let result = query.verify_vote_poll_votes_proof(&[], &platform_version);

        assert!(
            matches!(result, Err(Error::Drive(DriveError::UnknownVersionMismatch { method, known_versions, received }))
                if method == "verify_vote_poll_votes_proof" && known_versions == vec![0] && received == 255
            )
        );
    }
}
