...
impl Drive {
    ...
    /// Verifies the proof of recent address balance changes after a given block
    /// height (exclusive start).
    ///
    /// Uses `RangeAfter` for the query, matching the proof generated by
    /// `prove_recent_address_balance_changes_after`.
    ///
    /// # Arguments
    /// - `proof`: A byte slice containing the cryptographic proof.
    /// - `after_block_height`: The block height after which to verify (exclusive).
    /// - `limit`: Optional maximum number of blocks to verify.
    /// - `verify_subset_of_proof`: Whether to verify only a subset of the proof.
    /// - `platform_version`: A reference to the platform version.
    ///
    /// # Returns
    /// - `Ok((RootHash, VerifiedAddressBalanceChangesPerBlock))`: On success.
    /// - `Err(Error)`: If verification fails.
    pub fn verify_recent_address_balance_changes_after(
        proof: &[u8],
        ...
        match platform_version
            ...
            .verify_recent_address_balance_changes
        {
            0 => Self::verify_recent_address_balance_changes_after_v0(
                proof,
                after_block_height,
                limit,
                verify_subset_of_proof,
            ...
            version => Err(Error::Drive(DriveError::UnknownVersionMismatch {
                method: "verify_recent_address_balance_changes_after".to_string(),
                known_versions: vec![0],
                received: version,
            })),
        }
