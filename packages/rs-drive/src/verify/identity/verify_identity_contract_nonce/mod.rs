...
mod tests {
    ...
    #[test]
    fn test_verify_identity_contract_nonce_unknown_version() {
        ...
        platform_version
            ...
            .verify
            .identity
            .verify_identity_contract_nonce = 255;

        let result = Drive::verify_identity_contract_nonce(
            &[],
            [0u8; 32],
            [0u8; 32],
            false,
        ...
        assert!(
            matches!(result, Err(Error::Drive(DriveError::UnknownVersionMismatch { method, known_versions, received }))
                if method == "verify_identity_contract_nonce" && known_versions == vec![0] && received == 255
            )
        );
    }
}
