...
impl Drive {
    /// Generation 1 sees a batch of any wire format through the shell that
    /// knows the erase kind. An erase acts on a document that is already
    /// deleted, so it is proved by the same by-id query as a delete: the proof
    /// shows the document absent.
    pub(super) fn prove_state_transition_v1(
        ...
        let path_query = match state_transition {
            ...
            StateTransition::Batch(st) => {
                ...
                let Some(transition) = st.first_transition_v1() else {
                    return Ok(ProofCreationResult::new_with_error(
                        ProofError::InvalidTransition(
