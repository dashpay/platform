...
impl Drive {
    /// Inserts an item with a sum item at the specified path and key if it doesn't already exist.
    ...
    pub(super) fn batch_insert_item_with_sum_item_if_not_exists_v0<const N: usize>(
        &self,
        ...
        match path_key_element_info {
            PathKeyRefElement((path, key, element)) => {
                if let Element::ItemWithSumItem(..) = &element {
