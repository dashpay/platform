...
impl Drive {
    ...
    ///
    /// v1 first coalesces the current-key alias writes of scoped authentication keys, so an
    /// identity update that registers and revokes keys covering one contract queues a single
    /// operation per alias slot (see [`coalesce_current_key_alias_operations`]).
