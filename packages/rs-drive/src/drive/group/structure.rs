...
fn action(description: &str) -> StructureNode {
    StructureNode::identifier("action", "action_id", "The action id")
        ...
        .children(vec![
            ...
            .describe(
                "Who signed; the sum is the power gathered so \
                 far.",
            )
            .child(
...
pub(crate) fn structure() -> StructureNode {
    ...
    .describe(
        "Multi party actions on tokens: proposed by one \
         member of a contract's group, carried out once \
         enough power has signed.",
    )
    .child(
        StructureNode::identifier("contract", "contract_id", "The data contract id")
            ...
            .source(SOURCE)
            .describe(
                "The groups of one contract. Created with the \
                 contract when it defines groups.",
            )
            .child(
                StructureNode::dynamic(
                    ...
                    KeyMatcher::Len(2),
                    KeyEncoding::U16Be,
                    "The position of the group in the contract",
                )
                .kind(ElementKind::Tree)
                .describe("One group.")
                .children(vec![
                    StructureNode::fixed("info", GROUP_INFO_KEY, "GroupInfo", "GROUP_INFO_KEY")
                        ...
                        .kind(ElementKind::Item)
                        .value(
                            "serialized Group: members, their power, the \
                             power required",
                        )
                    ...
                    .kind(ElementKind::Tree)
                    .describe(
                        "Actions still gathering signatures. The root of \
                         the group layer.",
                    )
                    .child(action(
