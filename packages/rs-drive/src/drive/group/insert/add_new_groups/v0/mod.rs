...
mod tests {
    ...
    #[test]
    fn test_insert_and_fetch_group_v0() {
        ...
        let group = Identity::random_group(5, Some(12345), platform_version)
            .expect("expected a random group");
        ...
        drive
            ...
            .expect("expected to insert group");

        let fetched_group = drive
            .fetch_full_group(group.id().to_buffer(), Some(&transaction), platform_version)
            .expect("should fetch an group")
            .expect("should have an group");

        assert_eq!(group, fetched_group);
    }

    #[test]
    fn test_insert_group_v0() {
