...
pub(crate) fn structure() -> StructureNode {
    ...
    .describe(
        "Groups of contracts, document types and tokens \
         an identity key can be bound to. Key 124 hangs \
         below Versions so no fee bearing transition pays \
         for the extra root node.",
    )
    .children(vec![
        StructureNode::fixed(
            "groups",
        ...
        .source(SOURCE)
        .describe(
            "The forward store: everything about a group \
             under its id.",
        )
        .child(
            StructureNode::identifier("group", "contract_group_id", "The contract group id")
                ...
                .children(vec![
                    ...
                    .value(
                        "bincode ContractGroupInfo: owner, optional name \
                         and description",
                    )
                    ...
                    .kind(ElementKind::Tree)
                    .describe(
                        "Single document types in the group, on one flat \
                         level so paging is a plain range.",
                    )
                    .child(
                        StructureNode::dynamic(
                            "member",
                            "contract_id_and_document_type",
                            KeyMatcher::Any,
                            KeyEncoding::Composite,
                            "The contract id followed by the document type \
                             name",
                        )
                        .kind(ElementKind::Item)
                        .value("empty; the key carries the information")
                    ...
                    .child(
                        StructureNode::dynamic(
                            "member",
                            "contract_id_and_token_position",
                            KeyMatcher::Len(34),
                            KeyEncoding::Composite,
                            "The contract id followed by the token position, \
                             u16 big endian",
                        )
                        .kind(ElementKind::Item)
                        .value("empty; the key carries the information")
        ...
        .source(SOURCE)
        .describe(
            "The backwards index: every group a contract \
             belongs to, under the contract id.",
        )
        .child(
            StructureNode::identifier("contract", "contract_id", "The data contract id")
                ...
                .children(vec![
                    ...
                    .child(group_reference(
                        "contract_groups.groups.group.contracts.contract",
                        "A group holding the contract.",
                    )),
                    StructureNode::fixed(
                        "document_types",
                    ...
                    .lazy()
                    .describe(
                        "Groups single document types of the contract \
                         belong to.",
                    )
                    .child(
                        StructureNode::dynamic(
                        ...
                        .child(group_reference(
                            ...
                            "A group holding the document type.",
                        )),
                    ),
                    StructureNode::fixed(
                        "tokens",
