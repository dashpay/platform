...
impl Drive {
    ...
    pub(super) fn disable_identity_keys_v1(
        &self,
        identity_id: [u8; 32],
        keys_ids: Vec<KeyID>,
        ...
        let batch_operations = self.disable_identity_keys_operations_v1(
            identity_id,
            keys_ids,
            disable_at,
            &block_info.epoch,
    ...
    /// Disables a set of identity keys for a given identity in version 1.
    ///
    ...
    #[allow(clippy::too_many_arguments)]
    #[inline(always)]
    pub(super) fn disable_identity_keys_operations_v1(
        &self,
        identity_id: [u8; 32],
        key_ids: Vec<KeyID>,
        disable_at: TimestampMillis,
        ...
        // v1 reads the stored keys in estimation mode too. v0 substituted a maximal stand-in
        // key without contract bounds, so a scoped key's contract-info reference refreshes
        // (up to one per bounded contract and document type) were missing from the fee
        // estimate that admits the transition.
        ...
        let keys: KeyIDIdentityPublicKeyPairVec = self.fetch_identity_keys_operations(
            key_request,
            transaction,
            &mut drive_operations,
            platform_version,
