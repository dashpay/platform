...
mod tests {
    ...
    mod fetch_identities_balances_partial {
        use super::*;

        #[test]
        fn should_return_only_existing_identities_when_some_missing() {
            let drive = setup_drive_with_initial_state_structure(None);
            let platform_version = PlatformVersion::latest();
            ...
            // Query for the existing identity and a non-existent one
            let ids = vec![identity.id().to_buffer(), [0xab; 32]];

            let balances = drive
                .fetch_identities_balances(&ids, None, platform_version)
                .expect("should fetch balances");

            // Only the existing identity should be returned; non-existent ones are omitted
            assert_eq!(balances.len(), 1);
            let balance = balances
                .get(&identity.id().to_buffer())
                .expect("should contain balance for the existing identity");
            assert_eq!(*balance, identity.balance());
            assert!(
                balances.get(&[0xab; 32]).is_none(),
    ...
    mod fetch_optional_identities_balances {
        ...
        fn should_return_none_for_non_existent_identities() {
            ...
            assert_eq!(
                *balances
                    .get(&identity.id().to_buffer())
                    .expect("expected balance entry for inserted identity"),
                Some(identity.balance())
            );
            assert_eq!(
                *balances
                    .get(&[0xff; 32])
                    .expect("expected entry for non-existent identity"),
                None
            );
        }
    }
