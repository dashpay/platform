...
impl Drive {
    ...
    pub(super) fn prove_full_identity_by_non_unique_public_key_hash_v0(
        ...
        if let Some(identity_id) = identity_ids.first() {
            let query = Self::full_identity_with_non_unique_public_key_hash_query(
                public_key_hash,
                *identity_id,
                after,
            ...
            let query = Self::identity_id_by_non_unique_public_key_hash_query(public_key_hash, after);
            self.grove_get_proved_path_query(
                &query,
                transaction,
                &mut vec![],
...
mod tests {
    ...
    fn should_prove_a_single_identity_with_non_unique_key() {
        ...
        let proof = drive
            .prove_full_identity_by_non_unique_public_key_hash(first_key_hash, None, None, platform_version)
            .expect("should not error when proving an identity");
    ...
    fn should_prove_a_single_identity_with_non_unique_key_when_two_have_same_key() {
        ...
        let key = IdentityPublicKey::random_voting_key_with_rng(3, &mut rng, platform_version).expect("expected key").0;
        ...
        let proof = drive
            .prove_full_identity_by_non_unique_public_key_hash(key_hash, None, None, platform_version)
            .expect("should not error when proving an identity");
        ...
        let proof = drive
            .prove_full_identity_by_non_unique_public_key_hash(key_hash, Some(identity_1.id().to_buffer()), None, platform_version)
            .expect("should not error when proving an identity");
        ...
        let proof = drive
            .prove_full_identity_by_non_unique_public_key_hash(key_hash, Some(identity_2.id().to_buffer()), None, platform_version)
            .expect("should not error when proving an identity");
