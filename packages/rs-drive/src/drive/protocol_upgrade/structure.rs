...
pub(crate) fn structure() -> StructureNode {
    ...
    .describe(
        "The protocol versions proposers ask for. Written \
         once per block at most, which is why new root \
         trees are hung below it.",
    )
    .child(
        StructureNode::fixed(
            "counter",
        ...
        .describe(
            "How many proposers ask for each protocol \
             version.",
        )
        .child(
    ...
    .child(
        StructureNode::fixed(
            "desired",
            &VALIDATOR_DESIRED_VERSIONS,
            "ValidatorDesiredVersions",
            "VALIDATOR_DESIRED_VERSIONS",
        )
        .kind(ElementKind::Tree)
        .source("packages/rs-drive/src/drive/protocol_upgrade/mod.rs")
        .describe(
            "The protocol version each proposer asked for \
             last.",
        )
        .child(
