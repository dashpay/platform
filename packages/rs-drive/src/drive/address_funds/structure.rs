...
pub(crate) fn structure() -> StructureNode {
    ...
    .describe("Credits held by Platform addresses rather than identities.")
    .child(
        StructureNode::fixed("clear", CLEAR_ADDRESS_POOL, "ClearAddressPool", "CLEAR_ADDRESS_POOL")
.ascii()
            .kind(ElementKind::ProvableCountSumTree)
            .source("packages/rs-drive/src/drive/address_funds/queries.rs")
            .describe("Transparent address balances. The count is part of the hashed state, so the number of funded addresses can be proven.")
            .child(
                StructureNode::dynamic("address", "address", KeyMatcher::Any, KeyEncoding::Raw, "The serialized Platform address")
