...
pub(in crate::drive::votes) fn vote_contested_resource_end_date_queries_tree_path<'a>() -> [&'a [u8]; 3] {
    [
        Into::<&[u8; 1]>::into(RootTree::Votes),
        Into::<&[u8; 1]>::into(CONTESTED_RESOURCE_TREE_KEY),
...
pub(in crate::drive::votes) fn vote_contested_resource_identity_votes_tree_path<'a>() -> [&'a [u8]; 3] {
    [
        Into::<&[u8; 1]>::into(RootTree::Votes),
        Into::<&[u8; 1]>::into(CONTESTED_RESOURCE_TREE_KEY),
...
pub(in crate::drive::votes) fn vote_contested_resource_identity_votes_tree_path_for_identity<'a>(identity_id: &[u8; 32]) -> [&'a [u8]; 4] {
    [
        Into::<&[u8; 1]>::into(RootTree::Votes),
        Into::<&[u8; 1]>::into(CONTESTED_RESOURCE_TREE_KEY),
        Into::<&[u8; 1]>::into(IDENTITY_VOTES_TREE_KEY),
...
pub(in crate::drive::votes) fn vote_contested_resource_identity_votes_tree_path_for_identity_vec(identity_id: &[u8; 32]) -> Vec<Vec<u8>> {
    vec![
...
impl TreePath for ContestedDocumentResourceVoteType {
    fn tree_path(&self, contract: &DataContract) -> Result<Vec<&[u8]>, ProtocolError> {
        ...
        let index = document_type.indices().iter().find(|index| &index.name == &self.vote_poll.index_name).ok_or(ProtocolError::VoteError(format!("vote index name {} not found", &self.vote_poll.index_name)))?;
        let mut path = contract_document_type_path(&self.vote_poll.contract_id.as_bytes(), &self.vote_poll.document_type_name).to_vec();
