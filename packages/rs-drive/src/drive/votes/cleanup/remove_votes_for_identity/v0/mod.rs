...
impl Drive {
    ...
    pub(super) fn remove_votes_for_identity_v0(
        ...
        let vote_path = vote_contested_resource_identity_votes_tree_path_for_identity_vec(identity_id.as_bytes());

        let path_query = PathQuery::new(vote_path, SizedQuery::new(Query::new_single_query_item(QueryItem::RangeFull(RangeFull)), Some(512), None));

        let votes_to_remove_elements = self.grove_get_path_query(&path_query, transaction, QueryElementResultType, &mut vec![], &platform_version.drive)?.0.to_elements();
        ...
        for vote_to_remove in votes_to_remove_elements {
            let Element::Item(vote, ..) = vote_to_remove else {
                return Err(Error::Drive(DriveError::CorruptedDriveState(format!("vote {:?} for identity {} is not an item", vote_to_remove, identity_id))));
            };
