...
impl Drive {
    ...
    /// - `id`: A 32-byte array representing the contract ID that the nonce is for.
    ...
    ///
    pub(crate) fn verify_identity_nonce_v0(
        proof: &[u8],
        identity_id: [u8; 32],
        verify_subset_of_proof: bool,
    ) -> Result<(RootHash, Option<u64>), Error> {
        let mut path_query = Self::identity_nonce_query(identity_id);
        ...
        if proved_key_values.len() == 1 {
            ...
            if key != vec![IdentityTreeNonce as u8] {
                return Err(Error::Proof(ProofError::CorruptedProof(
                    "we did not get back an element for the correct key for the identity nonce"
                        .to_string(),
                )));
            }

            let identity_nonce = maybe_element
                .map(|element| {
