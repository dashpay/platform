    ...
    pub fn verify_identity_id_by_public_key_hash(
        ...
        if proved_key_values.len() == 1 {
            ...
            if path != unique_key_hashes_tree_path_vec() {
                return Err(Error::Proof(ProofError::CorruptedProof(
                    "we did not get back an element for the correct path in unique key hashes".to_string(),
                )));
            }
            if key != public_key_hash {
                return Err(Error::Proof(ProofError::CorruptedProof(
                    "we did not get back an element for the correct key in unique key hashes".to_string(),
                )));
            }
