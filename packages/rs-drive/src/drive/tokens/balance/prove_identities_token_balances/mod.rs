...
impl Drive {
    /// Proves the token balances of an identity from the backing store.
    ///
    /// # Arguments
    ///
    ...
    pub fn prove_identities_token_balances(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
    ...
    /// Proves the identity's token balances with associated costs.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// * `DriveError::UnknownVersionMismatch` - If the platform version does not support the requested operation.
    pub fn prove_identities_token_balances_with_costs(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
        ...
        let value = self.prove_identities_token_balances_operations(
            token_id,
            identity_ids,
            transaction,
    ...
    ///
    /// * `DriveError::UnknownVersionMismatch` - If the platform version does not support the requested operation.
    pub fn prove_identities_token_balances_operations(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
        transaction: TransactionArg,
            ...
            .identities_token_balances
        {
            ...
            version => Err(Error::Drive(DriveError::UnknownVersionMismatch {
                method: "prove_identities_token_balances_operations".to_string(),
                known_versions: vec![0],
                received: version,
            })),
        }
