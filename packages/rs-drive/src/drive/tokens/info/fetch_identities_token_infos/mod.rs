...
impl Drive {
    ...
    pub fn fetch_identities_token_infos(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
        ...
        {
            0 => self.fetch_identities_token_infos_v0(
                token_id,
                identity_ids,
                transaction,
                platform_version,
    ...
    /// # Returns
    ///
    /// * `Result<((BTreeMap<[u8; 32], Option<IdentityTokenInfo>>), FeeResult), Error>` - A tuple containing a map of token infos and the associated fee result.
    ///
    /// # Errors
    ///
    /// * `DriveError::UnknownVersionMismatch` - If the platform version does not support the requested operation.
    ...
    pub fn fetch_identities_token_infos_with_costs(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
        ...
        let value = self.fetch_identities_token_infos_operations(
            token_id,
            identity_ids,
            transaction,
    ...
    ///
    /// * `DriveError::UnknownVersionMismatch` - If the platform version does not support the requested operation.
    pub fn fetch_identities_token_infos_operations(
        &self,
        token_id: [u8; 32],
        identity_ids: &[[u8; 32]],
        transaction: TransactionArg,
        ...
        match platform_version
            ...
            .identities_token_infos
        {
            0 => self.fetch_identities_token_infos_operations_v0(
                token_id,
                identity_ids,
                transaction,
                drive_operations,
            ...
            version => Err(Error::Drive(DriveError::UnknownVersionMismatch {
                method: "fetch_identities_token_infos_operations".to_string(),
                known_versions: vec![0],
                received: version,
            })),
        }
