//! Joins through a document reference resolved by a unique index
//! (`refersTo.lookup`, protocol version 14): such a reference's value is not
//! the referenced document's `$id`, so neither a chained query nor a composite
//! by-id join may take it as a join property. Both surfaces refuse it while
//! validating the shape, on the server and in the verifier alike.
...
/// names the owner of a profile through that index: the indexOnly `like` a
/// chained query starts from and the plain `note` a composite page starts from.
fn lookup_join_contract() -> DataContract {
    ...
    .expect("the lookup join contract parses")
}
...
fn assert_refused_as_a_lookup(result: Result<(), Error>) {
    match result {
        Err(Error::Query(error)) => assert!(
            error.to_string().contains("unique index \"byOwner\""),
            "the refusal should name the lookup's index: {error}"
        ),
