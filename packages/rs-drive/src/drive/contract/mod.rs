...
mod tests {
    ...
    fn test_update_contract_errors_on_changing_document_type_keeps_history() {
        ...
        // the V2 schema parser rejects the keep-history + canBeDeleted combination
        // (canBeDeleted's config default is true), so the schema must opt out of
        // delete to reach the intended `ChangingDocumentTypeKeepsHistory` assertion
        // at `update_contract`.
