...
impl Drive {
    /// Takes `identity_id` off the banlist of `contract_id`. The storage refund goes to the
    /// identity the entry's storage flags name.
    ///
    ...
    /// * `identity_id`: The identity to unban.
    ...
    /// The low level operations of [`Drive::remove_contract_ban`]. With layer information the
    ...
    /// * `Err(Error)` when the version is unknown or GroveDB refuses an operation.
    #[allow(clippy::too_many_arguments)]
    pub fn remove_contract_ban_operations(
        &self,
        contract_id: Identifier,
        identity_id: Identifier,
        block_info: &BlockInfo,
        ...
        match platform_version
            ...
            .remove_contract_ban
        {
            0 => self.remove_contract_ban_operations_v0(
                contract_id,
                identity_id,
                block_info,
