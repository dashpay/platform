...
pub(crate) fn structure() -> StructureNode {
    ...
    .describe(
        "The address balance changes of recent blocks, so \
         a wallet can catch up without replaying blocks.",
    )
    .children(vec![
        StructureNode::fixed(
            "compacted",
            COMPACTED_ADDRESS_BALANCES_KEY,
        ...
        .source(SOURCE)
        .describe(
            "Runs of blocks merged into one entry once enough \
             have piled up.",
        )
        .child(
            StructureNode::dynamic(
                ...
                "The first and last block height, each u64 big \
                 endian",
            )
        ...
        StructureNode::fixed(
            "compacted_expiration",
            COMPACTED_ADDRESSES_EXPIRATION_TIME_KEY,
            "CompactedAddressesExpirationTime",
            "COMPACTED_ADDRESSES_EXPIRATION_TIME_KEY",
        )
        .ascii()
        .kind(ElementKind::Tree)
        ...
        .source(SOURCE)
        .describe(
            "One entry per recent block. The count and sum \
        ...
        .child(
            ...
            .value(
                "the block's address balance changes; the sum is \
