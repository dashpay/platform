...
pub(crate) fn structure() -> StructureNode {
    StructureNode::fixed("balances", &[RootTree::Balances as u8], "Balances", "RootTree::Balances")
        .kind(ElementKind::SumTree)
        .source("packages/rs-drive/src/drive/mod.rs")
        .book("data-model/identities.md")
        .describe("The credit balance of every identity. Kept out of the identity tree so balance proofs stay small.")
        .child(
