...
impl Drive {
    ...
    pub(crate) fn add_indices_for_top_index_level_for_contract_operations(
        ...
        for (name, sub_level) in index_level.sub_levels() {
            // Decide property-name tree type. If `sub_level` terminates a
            // `range_countable` index, the property-name tree (the level
            // whose keys are property values) becomes a `ProvableCountTree`
            // so range-count queries on the indexed property can walk the
            // boundary in O(log n). Otherwise it stays `NormalTree`.
            ...
            // The value tree (one per distinct property value, hosting `[0]`
            // terminals + sibling continuations) becomes a `CountTree` when
            // the index is `range_countable` so the property-name
            // `ProvableCountTree`'s aggregate sums per-value counts cleanly.
