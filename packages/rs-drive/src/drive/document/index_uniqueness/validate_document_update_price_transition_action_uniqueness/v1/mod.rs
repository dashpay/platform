...
impl Drive {
    /// Validate that a document update_price transition action would be unique in the state
    #[inline(always)]
    pub(super) fn validate_document_update_price_transition_action_uniqueness_v1(
        &self,
        contract: &DataContract,
        document_type: DocumentTypeRef,
        document_update_price_transition: &DocumentUpdatePriceTransitionAction,
        owner_id: Identifier,
        ...
        let request = UniquenessOfDataRequestV1 {
            ...
            document_type,
            owner_id,
            creator_id: document_update_price_transition.document().creator_id(),
            document_id: document_update_price_transition.base().id(),
