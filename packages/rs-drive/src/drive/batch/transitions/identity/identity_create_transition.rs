use dpp::identity::state_transition::identity_create_transition::{IdentityCreateTransition, IdentityCreateTransitionAction};
...
impl DriveHighLevelOperationConverter for IdentityCreateTransitionAction {
    fn to_high_level_drive_operations(self, epoch: &Epoch) -> Result<Vec<DriveOperation>, Error> {
        let IdentityCreateTransitionAction {
            public_keys, initial_balance_amount, identity_id, ..
        } = self;

        let mut drive_operations = vec![];
        /// We must create the contract
        drive_operations.push(IdentityOperation(IdentityOperationType::AddNewIdentity { identity: Identity {
        ...
        drive_operations.push(SystemOperation(SystemOperationType::AddToSystemCredits { amount: initial_balance_amount }));
