...
impl DocumentCreateTransitionActionV0 {
    ...
    pub fn try_from_document_create_transition_with_contract_lookup(
        ...
        let DocumentCreateTransitionV0 { base, data, prefunded_voting_balances , .. } = value;
        ...
        let prefunded_voting_balances_by_index = prefunded_voting_balances.into_iter().map(|(index_name, credits)| {
            let index = document_type_indexes.get(&index_name).ok_or(ProtocolError::UnknownContestedIndexResolution(format!("index {} not found on document type {}", index_name, document_type.name())))?;
    ...
    pub fn try_from_borrowed_document_create_transition_with_contract_lookup(
        ...
        let prefunded_voting_balances_by_index = prefunded_voting_balances.iter().map(|(index_name, credits)| {
            let index = document_type_indexes.get(index_name).ok_or(ProtocolError::UnknownContestedIndexResolution(format!("index {} not found on document type {}", index_name, document_type.name())))?;
            Ok((index.clone(), *credits))
        }).collect();
