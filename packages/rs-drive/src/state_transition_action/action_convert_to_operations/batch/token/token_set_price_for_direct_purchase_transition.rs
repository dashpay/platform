...
impl DriveHighLevelBatchOperationConverter for TokenSetPriceForDirectPurchaseTransitionAction {
            ...
            .token_set_price_for_direct_purchase_transition
        {
            0 => {
                ...
                if self.base().perform_action() {
                    ops.push(TokenOperation(TokenOperationType::TokenSetPriceForDirectPurchase {
                        token_id: self.token_id(),
                        price: self.price().cloned(),
                    }));

                    let token_configuration = self.base().token_configuration()?;
                    if token_configuration.keeps_history().keeps_direct_pricing_history() {
            ...
            version => Err(Error::Drive(DriveError::UnknownVersionMismatch {
                method: "TokenSetPriceForDirectPurchaseTransitionAction::into_high_level_document_drive_operations"
                    .to_string(),
