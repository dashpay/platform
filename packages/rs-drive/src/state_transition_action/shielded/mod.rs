...
/// One note from an Orchard action: the per-action fields that travel together.
#[derive(Debug, Clone)]
pub struct ShieldedActionNote {
    ...
    pub cmx: [u8; 32],
    /// Value commitment (cv_net). Stored unencrypted alongside the note so a
    /// wallet can recover the value of an outgoing note via OVK decryption.
    pub cv_net: [u8; 32],
...
mod tests {
    ...
    fn test_clone() {
        ...
        assert_eq!(cloned.cv_net, note.cv_net);
