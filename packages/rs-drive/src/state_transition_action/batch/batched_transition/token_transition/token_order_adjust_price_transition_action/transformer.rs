...
/// Implement methods to transform a `TokenOrderAdjustPriceTransition` into a `TokenOrderAdjustPriceTransitionAction`.
impl TokenOrderAdjustPriceTransitionAction {
    /// Transform a `TokenOrderAdjustPriceTransition` into a `TokenOrderAdjustPriceTransitionAction` using the provided data contract lookup.
    ///
    /// # Arguments
    ///
    ...
    /// * `value` - A `TokenOrderAdjustPriceTransition` instance.
    ...
    /// # Returns
    ///
    /// * `Result<(ConsensusValidationResult<BatchedTransitionAction>, FeeResult), Error>` - A `TokenOrderAdjustPriceTransitionAction` if successful, otherwise `ProtocolError`.
    #[allow(clippy::too_many_arguments)]
    pub fn try_from_token_order_adjust_price_transition_with_contract_lookup(
        drive: &Drive,
        owner_id: Identifier,
        value: TokenOrderAdjustPriceTransition,
    ...
    > {
        match value {
            TokenOrderAdjustPriceTransition::V0(v0) => {
                TokenOrderAdjustPriceTransitionActionV0::try_from_token_order_adjust_price_transition_with_contract_lookup(
                    drive,
                    owner_id,
                    v0,
    ...
    /// Transform a borrowed `TokenOrderAdjustPriceTransition` into a `TokenOrderAdjustPriceTransitionAction` using the provided data contract lookup.
    ///
    /// # Arguments
    ///
    /// * `drive` - A reference to the `Drive` instance used for accessing the system.
    /// * `owner_id` - The identifier of the owner initiating the release transition.
    /// * `transaction` - The transaction argument used for state changes.
    /// * `value` - A reference to a `TokenOrderAdjustPriceTransition`.
    /// * `approximate_without_state_for_costs` - A flag indicating whether to approximate state costs without full state.
    /// * `drive_operations` - A mutable reference to the vector of low-level operations that need to be performed.
    /// * `get_data_contract` - A closure that fetches the `DataContractFetchInfo` given a contract ID.
    /// * `platform_version` - The platform version for the context in which the transition is being executed.
    ...
    > {
        match value {
            TokenOrderAdjustPriceTransition::V0(v0) => {
                TokenOrderAdjustPriceTransitionActionV0::try_from_borrowed_token_order_adjust_price_transition_with_contract_lookup(
                    drive,
                    owner_id,
                    v0,
                    approximate_without_state_for_costs,
