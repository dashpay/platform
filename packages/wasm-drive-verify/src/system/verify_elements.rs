...
pub fn verify_elements(
    ...
    for i in 0..path_array.length() {
        ...
        let item_uint8: Uint8Array = item
            .dyn_into()
    ...
    let (root_hash, elements_map) =
        drive::verify::system::verify_elements(&proof_vec, path_vec, keys_vec, platform_version)
    ...
    for (key, element_option) in elements_map {
        ...
        let element_js = match element_option {
            Some(element) => {
                ...
                to_value(&element_json).map_err(|e| {
                    JsValue::from_str(&format!("Failed to convert element to JsValue: {:?}", e))
                })?
            }
            None => JsValue::NULL,
        };
