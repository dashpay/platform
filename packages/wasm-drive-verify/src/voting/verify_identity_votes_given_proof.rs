...
pub fn verify_identity_votes_given_proof_vec(
    ...
    let query: ContestedResourceVotesGivenByIdentityQuery = ciborium::de::from_reader(&query_cbor.to_vec()[..])
    ...
    let (root_hash, votes_vec): (RootHash, Vec<(Identifier, ResourceVote)>) = 
        query.verify_identity_votes_given_proof(
            &proof_vec,
            &contract_lookup_fn,
            platform_version,
        )
...
pub fn verify_identity_votes_given_proof_map(
    ...
    let (root_hash, votes_map): (RootHash, BTreeMap<Identifier, ResourceVote>) = 
        query.verify_identity_votes_given_proof(
            &proof_vec,
            &contract_lookup_fn,
            platform_version,
...
fn create_contract_lookup_fn(contract_lookup: &JsValue) -> Result<ContractLookupFn, JsValue> {
    ...
    let contracts_obj: Object = contract_lookup.clone().dyn_into()
    ...
    for i in 0..keys.length() {
        ...
        let contract_uint8: Uint8Array = contract_bytes_js.dyn_into()
    ...
    let lookup_fn: ContractLookupFn = Arc::new(move |id: &Identifier| {
        contracts_map.get(id).cloned()
    });
