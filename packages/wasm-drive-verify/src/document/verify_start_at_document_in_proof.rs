...
#[wasm_bindgen]
impl VerifyStartAtDocumentInProofResult {
    #[wasm_bindgen(getter)]
...
#[wasm_bindgen(js_name = "verifyStartAtDocumentInProof")]
pub fn verify_start_at_document_in_proof(
    ...
    // Parse document_id
    let document_id_bytes: [u8; 32] = document_id
        .to_vec()
        .try_into()
    ...
    let (root_hash, document_option) = query.verify_start_at_document_in_proof(
        &proof_vec,
        is_proof_subset,
        document_id_bytes,
        platform_version
    ...
    let document_js = match document_option {
        Some(doc) => {
            ...
            to_value(&doc_json)
                .map_err(|e| JsValue::from_str(&format!("Failed to convert document to JsValue: {:?}", e)))?
        }
        None => JsValue::NULL,
    };

    Ok(VerifyStartAtDocumentInProofResult {
