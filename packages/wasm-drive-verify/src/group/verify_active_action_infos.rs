...
pub fn verify_action_infos_in_contract_vec(
    ...
    let start_position = match (start_action_id, start_at_included) {
        (Some(id), Some(included)) => {
            let id_bytes: [u8; 32] = id
                .to_vec()
                .try_into()
                .map_err(|_| JsValue::from_str("Invalid start_action_id length. Expected 32 bytes."))?;
            Some((Identifier::from(id_bytes), StartAtIncluded(included)))
        }
        (Some(_), None) => return Err(JsValue::from_str("start_at_included must be provided when start_action_id is set")),
        (None, Some(_)) => return Err(JsValue::from_str("start_action_id must be provided when start_at_included is set")),
        (None, None) => None,
    };
    ...
    for (id, action) in actions_vec {
        ...
        let action_js = to_value(&action_json)
            .map_err(|e| JsValue::from_str(&format!("Failed to convert action to JsValue: {:?}", e)))?;
