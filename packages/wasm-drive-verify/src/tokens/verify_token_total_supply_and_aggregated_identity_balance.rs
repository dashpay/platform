...
pub fn verify_token_total_supply_and_aggregated_identity_balance(
    ...
    let (root_hash, supply_and_balance_option) = Drive::verify_token_total_supply_and_aggregated_identity_balance(
        &proof_vec,
        token_id_bytes,
    ...
    let result_js = match supply_and_balance_option {
        Some((total_supply, aggregated_balance)) => {
            ...
            Reflect::set(&obj, &JsValue::from_str("totalSupply"), &JsValue::from_f64(total_supply as f64))
                .map_err(|_| JsValue::from_str("Failed to set totalSupply"))?;
            
            Reflect::set(&obj, &JsValue::from_str("aggregatedIdentityBalance"), &JsValue::from_f64(aggregated_balance as f64))
