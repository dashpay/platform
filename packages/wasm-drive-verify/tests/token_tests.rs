...
use wasm_drive_verify::token_verification::*;
...
fn test_verify_token_statuses_empty_array() {
    ...
    let result = verify_token_statuses_vec(&proof, &contract_ids, platform_version);
    // Should succeed with empty results
    assert!(result.is_ok());
}
...
fn test_verify_token_direct_selling_prices_mixed_valid_invalid() {
    ...
    let result = verify_token_direct_selling_prices_vec(&proof, &contract_ids, platform_version);
