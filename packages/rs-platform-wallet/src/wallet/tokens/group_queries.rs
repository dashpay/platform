...
//! (next to `wallet.rs`) rather than in rs-sdk-ffi because querying
//! pending proposals only makes sense in the context of a wallet's
//! `Sdk` reference and an identity's role in a contract's groups —
//! see `swift-sdk/CLAUDE.md`'s "high-level operations go through
//! platform-wallet" rule.
...
use crate::wallet::tokens::TokenWallet;
            ...
            let params = match token_event {
                ...
                TokenEvent::ChangePriceForDirectPurchase(schedule, note) => {
                    let price_per_token = match schedule {
                        Some(
                            dpp::tokens::token_pricing_schedule::TokenPricingSchedule::SinglePrice(
                                p,
                            ),
                        ) => Some(p),
