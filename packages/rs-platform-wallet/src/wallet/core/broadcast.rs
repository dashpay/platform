...
impl<B: TransactionBroadcaster + ?Sized> CoreWallet<B> {
    ...
    /// lock-free shape as the rejection-release path.)
    ///
    ...
    /// a gRPC-style broadcast only awaits the submit call and injects
    ...
    /// dropped. On a gRPC-style path — which returns from
    ...
    /// immediate rebuild; an ambiguous `MaybeSent` keeps it. It takes an
    /// [`AccountTypePreference`] rather than a `StandardAccountType`, so it
...
mod tests {
    ...
    use crate::test_support::{
        funded_wallet_manager, AlwaysOkBroadcaster, RejectFirstBroadcaster, WalletSigner,
    };
    ...
    async fn in_broadcast_pin_blocks_reselection_until_dispatch_returns() {
        ...
        // pipeline, exactly like a gRPC-style path — never produces.
        let still_fenced =
    ...
    /// the gRPC-style shape — the submit call returns without injecting
    ...
    /// gRPC-style shape, so nothing observes the spend), run catch-up far
