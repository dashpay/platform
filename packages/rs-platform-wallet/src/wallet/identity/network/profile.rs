...
impl<B: TransactionBroadcaster + ?Sized> IdentityWallet<B> {
    ...
    ///
    /// # Superseded — prefer [`Self::create_profile_with_external_signer`]
    ///
    ...
    /// external `&S: Signer<IdentityPublicKey>` (typically the iOS
    /// `KeychainSigner`).
    ///
    /// Steps:
    ...
    ///
    /// # Superseded — prefer [`Self::update_profile_with_external_signer`]
    ///
    /// Same rationale as `create_profile` deprecation.
    ///
