...
impl IdentityWallet {
    ...
    ///
    /// # Superseded — prefer [`Self::transfer_credits_to_addresses_with_external_signer`]
    ///
    ...
    ///
    /// Same shape as [`Self::transfer_credits_to_addresses`] but
    /// signing is routed through the supplied
    /// `&S: Signer<IdentityPublicKey>`. Required for
    /// external-signable wallets.
        ...
        S: Signer<IdentityPublicKey> + Send + Sync,
    {
        ...
        // Mirror the local-state bookkeeping in the legacy variant.
        {
            let mut wm = self.wallet_manager.write().await;
            let info_guard = wm.get_wallet_info_mut(&self.wallet_id).ok_or_else(|| {
                crate::error::PlatformWalletError::WalletNotFound(
