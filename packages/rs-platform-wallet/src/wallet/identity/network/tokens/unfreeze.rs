//! Token unfreeze — releases a previously-frozen target identity's
//! balance for a given `(contract, token_position)`. Group-gateable.
...
impl<B: TransactionBroadcaster + ?Sized> IdentityWallet<B> {
    /// Unfreeze with a caller-supplied signer + key.
    #[allow(clippy::too_many_arguments)]
    pub async fn token_unfreeze_with_signer<S: Signer<IdentityPublicKey>>(
        &self,
    ...
    /// Unfreeze with an external signer. Contract fetched internally.
    #[allow(clippy::too_many_arguments)]
    pub async fn token_unfreeze_with_external_signer<S: Signer<IdentityPublicKey>>(
        &self,
        identity_id: Identifier,
        ...
        self.token_unfreeze_with_signer(
            data_contract,
            token_position,
            identity_id,
            frozen_identity_id,
        ...
        .map_err(|e| PlatformWalletError::token_operation_failed("unfreeze", e))
    }
}
