//! Token resume — emergency action that re-enables a paused
...
impl<B: TransactionBroadcaster + ?Sized> IdentityWallet<B> {
    /// Resume a paused token using the wallet's internal signer.
    pub async fn token_resume(
        &self,
    ...
    /// Resume with a caller-supplied signer + key.
    #[allow(clippy::too_many_arguments)]
    pub async fn token_resume_with_signer<S: Signer<IdentityPublicKey>>(
        &self,
        data_contract: Arc<DataContract>,
        token_position: TokenContractPosition,
        identity_id: Identifier,
    ...
    /// Resume with an external signer. Contract fetched internally.
    #[allow(clippy::too_many_arguments)]
    pub async fn token_resume_with_external_signer<S: Signer<IdentityPublicKey>>(
        &self,
        identity_id: Identifier,
