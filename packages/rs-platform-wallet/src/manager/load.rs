...
impl<P: PlatformWalletPersistence + 'static> PlatformWalletManager<P> {
    ...
    pub async fn load_from_persistor(&self) -> Result<(), PlatformWalletError> {
        ...
        // Hydration is a lifecycle transition like registration and
        // removal: each wallet goes live in `wallet_manager` well before
        // it is published into `self.wallets`, and the batch rollback at
        // the bottom unwinds both maps. Held across the whole loop so a
        // concurrent `remove_wallet` of a deterministic id this batch is
        // mid-way through can neither drop a `wallet_manager` entry out
        // from under an unpublished wallet nor detach a generation it
        // never retired. See
