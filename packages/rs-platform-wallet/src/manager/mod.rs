...
pub struct PlatformWalletManager<P: PlatformWalletPersistence + 'static> {
    ...
    /// via `rcu`. Its closure must stay pure map manipulation because it
    /// can run more than once under a concurrent-writer retry, and only
    /// the invocation whose compare-and-swap succeeds is published — so
    /// captured state it writes must be overwritten per attempt, never
    /// accumulated across them. `remove_wallet`'s generation verdict and
    /// the load rollback's reclaimed-id list both rely on exactly that:
    /// each attempt recomputes its answer from the map the closure was
    /// handed, which is the same map the CAS compares against.
    pub(super) wallets:
...
mod tests {
    ...
    /// A `TransactionsSwept` snapshot delivered while a lifecycle write to
    /// the `wallets` map is in flight must still land in the wallet's
    /// lock-free balance atomics. A sweep can be the ONLY event carrying
    /// the corrected (lower) balance — `superseded_by` need not be
    /// wallet-relevant, so no later balance-bearing event is guaranteed —
    /// and the event bus does not retry or coalesce, so dropping this one
    /// snapshot leaves the removed funds on screen indefinitely.
    ///
    /// When the map was a `tokio::sync::RwLock` and the handler used
    /// `try_read()`, this exact delivery-under-contention scenario dropped
    /// the snapshot (the pre-fix form of this test held `wallets.write()`
    /// across the delivery and failed). With the map an `ArcSwap`, the
    /// closest reachable window is a lifecycle writer parked mid-`rcu`;
    /// the handler's `load()` must observe a committed map and apply the
    /// balance immediately, before that writer completes.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn swept_balance_survives_wallets_map_write_contention() {
        ...
        let writer = std::thread::spawn(move || {
            // `rcu` may re-run the closure after a compare-and-swap loss. Wait
            // for the release only once so a retry cannot wedge this test.
        ...
        // Deliver the sweep while the write is in flight.
        let corrected = WalletCoreBalance::new(1_234, 0, 0, 0);
        handler.on_wallet_event(&crate::events::WalletEvent::TransactionsSwept {
            wallet_id,
        ...
        assert_eq!(
            ...
            corrected.confirmed(),
            "the swept balance snapshot was dropped: a lifecycle write to the wallets map \
             was in flight during delivery, and no later event will re-carry the correction"
        );
