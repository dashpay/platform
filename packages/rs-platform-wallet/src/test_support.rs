...
/// Persister probe for lock-scope regression tests. On every `store` whose
/// changeset satisfies the `matches` predicate it records whether the shared
/// wallet-manager write lock was already released — a `try_read` on the
/// manager succeeds iff no writer holds it. Arm it with [`Self::watch`]
/// AFTER any setup that legitimately stores under the guard (identity
/// registration does), and read the verdict with [`Self::seen_unlocked`]:
/// `Some(true)` = a matching store ran with the lock released, `Some(false)`
/// = it ran while a writer still held the lock (the ordering these tests
/// exist to prevent), `None` = no matching store since the last
/// [`Self::reset`]. A `Weak` back-reference avoids a persister <-> manager
/// cycle.
#[cfg(test)]
pub(crate) struct LockProbePersister {
    manager: std::sync::Mutex<Option<std::sync::Weak<RwLock<WalletManager<PlatformWalletInfo>>>>>,
    matches: Box<dyn Fn(&crate::changeset::PlatformWalletChangeSet) -> bool + Send + Sync>,
    seen_unlocked: std::sync::Mutex<Option<bool>>,
}

#[cfg(test)]
impl LockProbePersister {
    pub(crate) fn for_changesets(
        matches: impl Fn(&crate::changeset::PlatformWalletChangeSet) -> bool + Send + Sync + 'static,
    ) -> Self {
        Self {
            manager: std::sync::Mutex::new(None),
            matches: Box::new(matches),
            seen_unlocked: std::sync::Mutex::new(None),
        }
    }

    /// Point the probe at the manager lock the code under test contends on.
    pub(crate) fn watch(
        &self,
        manager: std::sync::Weak<RwLock<WalletManager<PlatformWalletInfo>>>,
    ) {
        *self.manager.lock().expect("probe manager") = Some(manager);
    }

    /// Forget any matching store seen so far (isolate the assertion to the
    /// code path under test).
    pub(crate) fn reset(&self) {
        *self.seen_unlocked.lock().expect("probe verdict") = None;
    }

    pub(crate) fn seen_unlocked(&self) -> Option<bool> {
        *self.seen_unlocked.lock().expect("probe verdict")
    }
}

#[cfg(test)]
impl crate::changeset::PlatformWalletPersistence for LockProbePersister {
    fn store(
        &self,
        _wallet_id: WalletId,
        changeset: crate::changeset::PlatformWalletChangeSet,
    ) -> Result<(), crate::changeset::PersistenceError> {
        if (self.matches)(&changeset) {
            let unlocked = self
                .manager
                .lock()
                ...
                .unwrap_or(false);
            *self.seen_unlocked.lock().expect("probe verdict") = Some(unlocked);
        }
        Ok(())
    }
