...
| AL-001 | Concurrent asset-lock builds from same wallet | P1 | runs in the default `--features e2e` suite (gating is `required-features = ["e2e"]`, not `#[ignore]`; no `#[ignore]` on the test file); RED on devnets with weak IS-lock/ChainLock liveness under N-way concurrent asset-lock load: paloma 2026-06-02 — 2/3 IS-locks missed within the 300 s budget, ChainLock fallback also missed → `FinalityTimeout` (outpoints `0xa3c9c5fb…`/`0xda317344…`, `wait_for_proof` ~16× in mempool; single-build asset lock in the same run got IS-lock in ~0.67 s). Working hypothesis: server-side IS-lock/ChainLock liveness failure under concurrency (not a wallet bug). **OBSERVED — needs re-repro + root-cause before any upstream report; NOT reported** (matches run #544). See the AL-001 detail block. Guards the Found-008 fix only when the chain actually produces proofs. **Found-031 precondition (2026-07-06)**: on THIS branch the step-3 `add_identity_topup_account(.., None)` precondition is unfixed and still fails exactly as Found-031 describes (confirmed failing, `rerun-full.log`) — now understood as a usage error, not a defect (see Found-031; ID-002b already carries the fix). A matching `Some(xpub)` fix compiles clean on an experimental branch but its live run was deferred (shared bank depleted) — not yet confirmed passing, and not yet ported to this branch. Distinct from the IS-lock/ChainLock liveness finding above, which is unaffected. | L |
...
<!-- Found-bug pin accounting: 32 Found-NNN matrix entries (Found-001..018 + Found-021..034; Found-019/020 deleted, Found-006 retired). Full recompute 2026-07-06 against the live tables (baseline Quick index + Found-bug pins) — prior counts here had drifted stale as rows accreted (SH-*, PA-3040, Harness-*, etc.) without a matching recompute. Per-edit running tallies live in git history, not here. -->
Counts by priority: **P0: 17**, **P1: 53** (incl. CR-004 passing-as-regression + the 8 P1 Found-bug pins: Found-024 [retired], Found-025 [reported-fixed by #3650, unverified this session], Found-027 [reframed 2026-07-06 — test-scaffold limitation, not a defect], Found-028, Found-029 [fixed, guarded by SH-007], Found-031 [retired 2026-07-06 — confirmed usage error, not a defect], Found-032 [filed #4029], Found-033), **P2: 70** (incl. 24 P2 Found-bug pins, e.g. Found-013 [filed #4028], Found-021/Found-022 [tracked rust-dashcore#763/#764], Found-034 [tracked rust-dashcore#846]), **DEFERRED: 1** (141 total index entries: 109 baseline Quick-index rows + 32 Found-bug-pin rows).
...
- **Found-031 precondition status (2026-07-06)**: independently of the liveness finding below, AL-001's step-3 precondition (`add_identity_topup_account`, one call per identity slot `0..N`) carries the same bug shape Found-031 describes and is now understood as a **confirmed usage error, not a defect** — see Found-031 and ID-002b, which already carries the fix and passes GREEN. On **this branch** the precondition is still unfixed and fails deterministically with `add_account(.., None)` → `"External signable wallet has no private key"` (confirmed failing, `rerun-full.log:337`, 2026-07-06 14:30). A matching per-slot `Some(xpub)` fix (seed-derived, mirrors ID-002b) compiles clean on an experimental branch (`test/found031-provisioning`) but its live run was deferred (shared bank depleted) — reachability under concurrency is not yet confirmed, and the fix has not been ported to this branch. This is orthogonal to the IS-lock/ChainLock liveness finding immediately below, which is unaffected either way.
...
- **Filed**: dashpay/platform#4028. Confirmed a genuine defect, not a usage error, by a 2026-07-06 adversarial review — there is no alternate correct-usage path (the rustdoc's own `[recover_asset_lock](Self::recover_asset_lock)` link is dangling; no such method exists), and the silent-swallow behaviour is inconsistent with `register_wallet`'s fail-closed handling of the analogous registration-changeset `store` error.
