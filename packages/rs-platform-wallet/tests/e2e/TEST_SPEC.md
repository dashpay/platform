...
|----|-------|----------|--------|------------|
| PA-001 | Multi-output platform-address transfer | P0 | red-by-design (concurrency-only) — Found-026 family: `next_unused_address()` returns a DUPLICATE of `addr_1` under canonical 14-thread churn; deterministic panic at `pa_001_multi_output.rs:124` (`assert_ne!(addr_1, addr_2)`) after the Found-025 chain-confirmed gate clears. Single-thread PASS | S |
| PA-002 | Partial-fund + change handling | P0 | red-by-design (concurrency-only) — Found-026 family: `next_unused_address()` returns a DUPLICATE of `addr_1` under canonical 14-thread churn; deterministic panic at `pa_002_partial_fund.rs:130` (`assert_ne!(addr_1, addr_2)`) after the Found-025 chain-confirmed gate clears. Single-thread PASS | S |
| PA-004 | Sweep-back: drain test wallet, observe bank credit | P0 | green | S |
| PA-003 | Fee scaling: one-output vs. five-output | P1 | red-by-design (concurrency-only) — Found-026 family: `next_unused_address()` returns a DUPLICATE of `addr_src` under canonical 14-thread churn; deterministic panic at `pa_003_fee_scaling.rs:161` (`assert_ne!(addr_src, dest_1)`) after the Found-025 chain-confirmed gate clears. Single-thread PASS — real chain-time fee under single-input isolation; symmetric pre-markers put both shapes on address-funds UPDATE ops; strict + sub-linear + ceiling guards | M |
...
- **Priority**: P0
- **Status**: `red-by-design (concurrency-only)` — Found-026 family. Single-thread PASS (testnet; gated by `cargo test -p platform-wallet --tests` plus operator env vars per `tests/e2e/README.md`). Under the canonical 14-thread v-run (`cargo test --test e2e -- --include-ignored --nocapture --test-threads=14`, proven at SHA `e83a43c7e9`) it deterministically panics at `pa_001_multi_output.rs:124` with `assert_ne!(addr_1, addr_2)` — `next_unused_address()` returned a DUPLICATE of `addr_1`. The Found-025 chain-confirmed gate cleared first; this is the *downstream* bug it unmasked, not a regression. Same root component / concurrency trigger as ID-002 / ID-005 (see Found-026 §3 and the ID-002 same-family justification); not promoted to a new Found-NNN. No production fix (production cursor race, pinned not patched).
...
- **Priority**: P0
- **Status**: `red-by-design (concurrency-only)` — Found-026 family. Single-thread PASS. Under the canonical 14-thread v-run (`cargo test --test e2e -- --include-ignored --nocapture --test-threads=14`, proven at SHA `e83a43c7e9`) it deterministically panics at `pa_002_partial_fund.rs:130` with `assert_ne!(addr_1, addr_2)` — `next_unused_address()` returned a DUPLICATE of `addr_1`. The Found-025 chain-confirmed gate cleared first; this is the *downstream* bug it unmasked, not a regression. Same root component / concurrency trigger as ID-002 / ID-005 (see Found-026 §3 and the ID-002 same-family justification); not promoted to a new Found-NNN. No production fix (production cursor race, pinned not patched). Cross-bank-balance asserts (`bank_pre` / `bank_post` comparison) were dropped — sibling test traffic pollutes the bank balance under parallel execution, making those bounds non-deterministic. The per-address balance invariants (`balances[addr_1]`, `balances[addr_2]`, `fee > 0`) are the real contract and remain.
...
- **Priority**: P1
- **Status**: `red-by-design (concurrency-only)` — Found-026 family. Single-thread PASS. Under the canonical 14-thread v-run (`cargo test --test e2e -- --include-ignored --nocapture --test-threads=14`, proven at SHA `e83a43c7e9`) it deterministically panics at `pa_003_fee_scaling.rs:161` with `assert_ne!(addr_src, dest_1)` — `next_unused_address()` returned a DUPLICATE of `addr_src`. The Found-025 chain-confirmed gate cleared first; this is the *downstream* bug it unmasked, not a regression. Same root component / concurrency trigger as ID-002 / ID-005 (see Found-026 §3 and the ID-002 same-family justification); not promoted to a new Found-NNN. No production fix (production cursor race, pinned not patched). Supersedes the prior V28-303 "not reliably green under concurrency" concession (§ V28-303) and the stale `green` claim. When it runs single-thread it measures the real chain-time fee (`Σ gross outputs − Σ destination balance deltas`) for two self-transfers that draw inputs exclusively from one source address; every destination, including the 1-output `dest_1`, is pre-markered so both shapes hit address-funds UPDATE ops — output count is the sole varied factor. Asserts `fee_5 > fee_1`, sub-linear `fee_5 < 5 × fee_1`, and the `FEE_DELTA_CEILING` linear-schedule tripwire.
