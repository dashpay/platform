...
- **Status**: red-by-design. Pinned by `al_001_concurrent_asset_lock_builds.rs` (precondition loop at step 3: `add_identity_topup_account` fails for every slot 0..N) and `id_002b_asset_lock_top_up.rs` (precondition at step 3). Both tests fail at the account-provisioning step before any network call is made.
...
- **Owner**: `rs-platform-wallet` (this crate)
- **Status**: red-by-design. Pinned by `sh_011_note_selection_convergence.rs` (third iteration of `for _ in 0..NUM_NOTES` loop: `.expect("shield_from_account")` panics with `ShieldedBroadcastFailed("Protocol error: Invalid address nonce for P2PKH(...): expected 2, got 1")`).
