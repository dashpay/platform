...
| PA-005c | Concurrent receive-address reservations and release/reissue | P2 | compile-only / not yet live-verified | S |
...
- **Priority**: P2
- **Status**: blocked at `setup()`, not a defect in this case. Live run attempted 2026-07-23 (solo, `--test-threads` default): fails before reaching any of the case's own logic — `e2e setup failed: Wallet("SDK error: Proof verification error: dash drive: proof: corrupted error: compacted address balance proof contains trailing bytes")`. This is the same systemic proof-verification breakage documented in the 2026-07-23 full-suite rerun (see changelog entry below and `PA-005c`'s sibling changelog note): a client/server version skew from `rs-drive`#4165 (`6588f1f343`, merged into `v4.1-dev` 2026-07-22), which the live testnet DAPI/drive-abci fleet has apparently not yet redeployed. 83/102 live cases in the full suite hit the identical error at the identical layer. PA-005c's own reservation-lifecycle logic has never executed against a live network as a result — only the compile-time and in-process unit/sim verification (see the wiring changelog entry) has run. Re-attempt once the version skew resolves.
