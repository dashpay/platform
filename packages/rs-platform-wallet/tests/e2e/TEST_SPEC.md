...
- **v4.2-dev (2026-09-08, base merge — Found-035 superseded upstream)** — Merged `origin/v4.2-dev` (129 commits) into this branch. One finding changes status as a direct consequence: **Found-035 is SUPERSEDED, not fixed** — upstream PR #4611 deletes registration-time identity discovery outright rather than repairing it, so the branch-local fix (`3e175d2c31`) and its `capture_discovery_master` helper are dropped at the merge. `register_wallet` now performs no identity scan at all; hydration is the host startup sequence's job (`start_wallet_subsystems`, budgeted, master key resolved on demand) or an explicit `PlatformWallet::identity().discover_from_master(..)`. Upstream guards the new contract with `register_wallet_records_no_identity_scan_verdict` in `manager/wallet_lifecycle.rs`. Two upstream API breaks also land: `key_wallet::Mnemonic::from_phrase` is single-argument (auto-detecting wordlist, rust-dashcore#981), and `PlatformWalletManager.wallets` is an `arc_swap::ArcSwap` rather than an `RwLock`; the harness is adapted to both. New upstream machinery this suite now runs against: per-wallet in-broadcast input fences and `PlatformWalletInfo::observed_input_conflicts`, which the concurrent asset-lock cases (AL-001, ID-002b) exercise directly. **Full live testnet rerun on the merged tree (2026-09-09, `8b7a78169d`): 158 passed / 35 failed / 0 ignored** (193 cases, `--test-threads=3 --include-ignored`, 45m39s). Best recorded result on this branch — the prior full run was 100/88/0 (2026-07-23). **No merge-attributable regression.** The 35 reds decompose into four pre-existing buckets: **(1) 30 SH-\* cases**, one shared root cause — `bind_shielded` rejects with `shielded seedless restart requires persistence capabilities ["atomic_changesets", "shielded_viewing_keys"] (missing mask 0x9)`, because the harness wires `NoPlatformPersistence`, which stores nothing and so correctly reports `PersistenceCapabilities::NONE`. A harness gap, not a wallet defect, and structurally identical either side of the merge (the three `SHIELDED_FVK_RESTART` preflights, the `NONE` trait default and `NoPlatformPersistence` are byte-identical at `201756a536` and at the merge). Fix is a storing in-memory persister for the SH cases, not a capability claim `NoPlatformPersistence` cannot honour. **(2) 3 documented red-by-design pins** — Found-021, Found-022, Found-032 (via `pa_007_sync_watermark`), each failing with its own pinned assertion text. **(3) `found_coinjoin_gap_limit_sync_height_analysis`** — diagnostic, reports `BUG NOT REPRODUCED` (balances equal at this cutoff height). **(4) `found_coinjoin_gap_limit_sync`** — SPV capped sync timed out (`cutoff 1491827` not reached within 1200s; last committed filter height 875999), an environment/liveness limit. **AL-001 and ID-002b both PASS**, so the in-broadcast fences and the no-discovery registration are now network-verified against this merge. Full log archived at `/data/artifacts/platform/2026-09-09/e2e-full-run.log`.
...
| Found-035 | (SUPERSEDED upstream) `register_wallet` unconditionally ran `downgrade_to_external_signable()` before its post-registration best-effort `identity().sync()`, so built-in identity discovery always derived via the now-stripped resident-key path and failed on every registration, logging a misleading `WARN` (`"External signable wallet has no private key"`) — the auto-hydrate-on-reimport feature was dead code for every wallet, not merely noisy | P2 | SUPERSEDED (2026-09-08, v4.2-dev merge) — briefly fixed branch-locally by `3e175d2c31` (capture the BIP-32 master xpriv before the downgrade, route discovery through `discover_from_master(..)`), then obsoleted wholesale by upstream PR #4611, which deletes registration-time discovery instead of repairing it: the scan cost two wallet-manager lock waits and a host persistence round per registration and could stretch a wallet import to minutes. `register_wallet` now runs no scan; hydration belongs to `start_wallet_subsystems` or an explicit `discover_from_master(..)`. The branch-local fix and its `capture_discovery_master` helper are gone; upstream's `register_wallet_records_no_identity_scan_verdict` guards the new contract. **Distinct from Found-031** (retired — a confirmed test-helper usage error in `add_account(.., None)`, not a production defect) despite sharing the same `downgrade_to_external_signable()` call site: Found-031 was the suite's own precondition helper misusing a keyless-wallet API; this is registration's own best-effort discovery silently failing in production for every caller. | S |
...
*Note (2026-07-06, superseded 2026-09-08): the `register_wallet` built-in-identity-discovery defect fixed this session (`3e175d2c31` — see Changelog) is pinned as **Found-035** (SUPERSEDED, P2 — upstream PR #4611 deleted the discovery call site in the v4.2-dev merge) — see the row above and the detail entry in §3. It shares its `downgrade_to_external_signable()` call site with the retired Found-031 but is a distinct, unrelated symptom (registration's own best-effort discovery, not the test-suite's `add_account` helper); see Found-035's cross-reference for the disambiguation. Counts below now include this one new FIXED P2 pin.*

Counts by priority: **P0: 17**, **P1: 53** (incl. CR-004 passing-as-regression + the 8 P1 Found-bug pins: Found-024 [retired], Found-025 [reported-fixed by #3650, unverified this session], Found-027 [reframed 2026-07-06 — test-scaffold limitation, not a defect], Found-028, Found-029 [fixed, guarded by SH-007], Found-031 [retired 2026-07-06 — confirmed usage error, not a defect], Found-032 [filed #4029], Found-033), **P2: 71** (incl. 25 P2 Found-bug pins, e.g. Found-013 [filed #4028], Found-021/Found-022 [tracked rust-dashcore#763/#764], Found-034 [tracked rust-dashcore#846], Found-035 [SUPERSEDED 2026-09-08 by upstream #4611]), **DEFERRED: 1** (142 total index entries: 109 baseline Quick-index rows + 33 Found-bug-pin rows).
...
#### Found-035 — `register_wallet` always ran built-in identity discovery through the stripped resident-key path, silently failing on every registration — SUPERSEDED UPSTREAM

- **Priority**: P2 (genuine production defect, but low user impact — see Severity).
- **Severity**: LOW as shipped — discovery was explicitly best-effort; no funds or state loss occurred; callers retain an explicit `discover_from_master` escape hatch, which is now the only supported hydration path. The *code-path* impact was total (100% of `register_wallet` callers hit the broken branch), which is why the pin exists despite the low severity.
- **Owner**: `rs-platform-wallet` (this crate).
- **Status**: **SUPERSEDED UPSTREAM 2026-09-08** (v4.2-dev merge, upstream PR #4611). The underlying observation stands and was never disputed: `register_wallet` (`manager/wallet_lifecycle.rs`) unconditionally called `wallet.downgrade_to_external_signable()` — the same call site Found-031 pins — **before** its own post-registration best-effort `identity().sync()`. That strips the wallet's resident root key, so the sync always derived via the now-keyless resident-key path and failed unconditionally, logging `WARN "External signable wallet has no private key"` on every single registration; the auto-hydrate-on-reimport feature was dead code for every wallet, not merely noisy. This branch briefly *repaired* it (`3e175d2c31`, adversarial-QA-verified: capture the BIP-32 master xpriv before the downgrade, route discovery through `discover_from_master(..)`, 4 unit tests, master zeroized on drop). Upstream chose the opposite remedy and **deleted the call site** instead: the scan cost two wallet-manager lock waits plus a host persistence round on every registration and left a spurious "scan incomplete at index 0" verdict behind — on a host with a persister commit in flight that wait stretched a wallet import to minutes. Reviving it would have made registration strictly more expensive than the version #4611 removed, so the branch-local fix and its `capture_discovery_master` helper were dropped at the merge.
- **Current contract**: `register_wallet` performs no identity scan and records no scan verdict. Identity hydration is the host's job — the startup sequence (`start_wallet_subsystems`: budgeted, master key resolved on demand) or an explicit `PlatformWallet::identity().discover_from_master(..)`. Upstream pins this with `register_wallet_records_no_identity_scan_verdict` in `manager/wallet_lifecycle.rs`, which fails if any discovery runs during registration.
- **Wallet feature exercised**: `manager/wallet_lifecycle.rs` (`register_wallet` → `downgrade_to_external_signable()`, with no discovery tail); `PlatformWallet::identity().discover_from_master(..)` as the explicit hydration path.
- **Defect**: downgrade-then-sync ordering meant the best-effort discovery branch could never succeed via the resident-key path. Two remedies existed — derive from the captured master (this branch's `3e175d2c31`) or remove the branch (upstream #4611). Upstream's won; the master-derived derivation survives as the public `discover_from_master(..)` entry point.
- **Verification**: upstream's `register_wallet_records_no_identity_scan_verdict` (`manager/wallet_lifecycle.rs`) asserts registration persists no identity-scan verdict and leaves no incomplete-scan state — green on the merged tree. The 4 unit tests that guarded the branch-local fix were removed with it.
- **Cross-reference — distinct from Found-031**: shares the `downgrade_to_external_signable()` call site with the retired Found-031, but is an unrelated symptom. Found-031 was a confirmed **test-suite usage error** — this repo's own `add_identity_topup_account` helper misused `add_account(AccountType::IdentityTopUp, None)` on an already-downgraded wallet; no production code ever provisioned that account type, so there was no production defect (see Found-031 above). Found-035 is **registration's own best-effort discovery** — a genuine production code path exercised by every caller of `register_wallet`, silently broken from day one. Do not conflate the two: Found-031 is retired (never a defect); Found-035 was a real defect, resolved upstream by removing the feature rather than repairing it.
- **Filed**: not filed upstream. Fixed same-day in this repo (`3e175d2c31`), then superseded by upstream PR #4611 in the 2026-09-08 v4.2-dev merge.
