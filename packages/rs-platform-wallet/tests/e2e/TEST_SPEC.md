...
| Found-024 | `PlatformAddressWallet::transfer` writes foreign output-address balances to local ledger (no ownership check) | P1 | passing-as-regression. **2026-07-05 testnet run**: confirmed passing — `found_024_transfer_persistence_builder_drops_foreign_address` passes; GREEN guard holds. | S |
...
<!-- Found-bug pin accounting is reconciled by the summary line below (27 Found-NNN matrix entries; 26 live after Found-006 retirement). Per-edit running tallies live in git history, not here. -->
Counts by priority: **P0: 10**, **P1: 32** (incl. CR-004 passing-as-regression + ID-002b + AL-001 + Found-024 + Found-025 + Found-031 + Found-032 + Found-033), **P2: 64** (incl. 24 P2 Found-bug pins), **DEFERRED: 1** (107 total index entries; 77 baseline + 29 Found-bug pins + 1 deferred placeholder).
...
- **Priority**: P1
- **Status**: not implemented (Wave H) — **red-by-design** until Found-027 is fixed. **2026-07-05 testnet run: NOT VALIDATED** — `sh_005_inmemory_witness_split` failed before the case body ran: `Bank under-funded for e2e run (planner) ... Platform short 0 credits` (fund-planner E5 race; see run-conditions note above the Quick index).
...
- **Status**: passing-as-regression. Fix landed at `16636f01c0` (V27-007). Tests PA-004b and PA-009 unblocked; `#[ignore]` removed at the same commit. **2026-07-05 testnet run**: confirmed passing — `found_024_transfer_persistence_builder_drops_foreign_address` passes; GREEN guard holds.
...
#### Found-031 — `register_wallet` calls `downgrade_to_external_signable()` before IdentityTopUp accounts are provisioned, stripping the private key required for HD derivation
- **Priority**: P1 (blocks IdentityTopUp for any wallet that goes through `register_wallet`)
- **Severity**: HIGH (full feature-path failure — no IdentityTopUp account can be added to a registered wallet; the error is always fatal and immediate)
- **Owner**: `rs-platform-wallet` (this crate)
- **Status**: red-by-design. Pinned by `al_001_concurrent_asset_lock_builds.rs` (precondition loop at step 3: `add_identity_topup_account` fails for every slot 0..N) and `id_002b_asset_lock_top_up.rs` (precondition at step 3). Both tests fail at the account-provisioning step before any network call is made. **2026-07-05 testnet run**: `id_002b_asset_lock_funded_top_up` not validated — it failed earlier this run, at shared bank setup (`Bank under-funded for e2e run (planner) ... Platform short 0 credits`, fund-planner E5 race), before reaching the local precondition this pin targets; AL-001 not covered by this run's scope.
- **Wallet feature exercised**: `wallet/wallet_lifecycle.rs:244` (`register_wallet` → `wallet.downgrade_to_external_signable()`); `wallet/identity/` (`add_account(AccountType::IdentityTopUp { registration_index }, None)` → `KeyWallet::add_account` → HD derivation from root xprv).
- **Defect**: `register_wallet` (`wallet_lifecycle.rs:244`) calls `wallet.downgrade_to_external_signable()` after snapshotting the wallet's xpubs, zeroing the in-memory root private key material. Any subsequent `add_account(AccountType::IdentityTopUp { registration_index }, None)` on that wallet requires HD derivation of the new account's xpub from the root extended private key — which is now gone. The call fails immediately with `"Invalid parameter: External signable wallet has no private key"` wrapped into `PlatformWalletError::InvalidIdentityData`. The server is never contacted; the failure is 100% local.
- **Preconditions**: caller registers a wallet via `register_wallet`, then attempts to add an `IdentityTopUp` account afterwards. This is the normal usage pattern — AL-001 and ID-002b both do exactly this.
- **Scenario**:
  1. Construct and register a wallet via `register_wallet` (includes the `downgrade_to_external_signable()` call at `wallet_lifecycle.rs:244`).
  2. Call `add_account(AccountType::IdentityTopUp { registration_index: 0 }, None)` on the returned wallet handle.
- **Assertions** (the proof shape):
  - Counter-assertion (today's buggy behaviour): `add_account` returns `Err("Invalid parameter: External signable wallet has no private key")`.
  - Expected (after fix): `add_account` succeeds and the new account's xpub is correctly derived.
- **Expected** (after fix): either (a) provide `add_account_with_xpub` for external-signable wallets so callers can supply the pre-derived xpub, or (b) provision all required `IdentityTopUp` accounts inside `register_wallet` BEFORE the `downgrade_to_external_signable()` call.
- **Actual** (current code): `register_wallet` at `wallet_lifecycle.rs:244` calls `downgrade_to_external_signable()` unconditionally; any `add_account` call requiring HD derivation on the post-registration wallet always fails.
- **Harness extensions required**: none — the AL-001 / ID-002b harness already exercises the exact path; the failure surfaces at the precondition step.
- **Estimated complexity**: M (API design decision required; both fix options touch the lifecycle boundary and downstream callers).
- **Rationale**: IdentityTopUp is a core platform-wallet use-case. The downgrade-to-external-signable pattern is correct security hygiene (stripping key material post-registration), but it must not preclude adding accounts that require derivation. Every test that drives the TopUp path via `register_wallet` + `add_account` hits this wall today. Fixed by provisioning accounts before the downgrade or by supplying xpubs to the post-downgrade path.
