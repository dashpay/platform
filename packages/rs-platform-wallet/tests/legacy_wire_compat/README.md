...
- **A real legacy dash-wallet INSTALL** — the strongest check:
  `legacy_install_yabba2_wire_compat_vector`. Its blob was NOT generated by this
  repo — a stock **dash-wallet 11.9** Android install (shipping dashj crypto
  path) registered DPNS username `yabba2` on testnet, did a send + receive,
  saved metadata, and published one encrypted `txMetadata` document to Platform
  under identity `ESR1nfF3bj4TR2ZkLmDuSeu6r7VzpTurYi47BV6XwsoP` (`keyIndex = 2`,
  `encryptionKeyIndex = 1`, version 1/protobuf). That document was fetched from
  testnet once and its bytes checked in, so the test is network-free; the Rust
  crypto decrypts it to its real protobuf `TxMetadataBatch` plaintext (two
