...
pub const CONTRACT_VERSIONS_V6: DPPContractVersions = DPPContractVersions {
    ...
    document_type_versions: DocumentTypeVersions {
        ...
        schema: DocumentTypeSchemaVersions {
            ...
            find_identifier_and_binary_paths: 1, // changed: identifier and byte array items of a typed array are registered as `path[]` conversion paths
            ...
            parse_typed_array: Some(0), // changed: the meta-schema v3 typed scalar array (`type: array` with an `items` schema) is parsed into `DocumentPropertyType::TypedArray`; None before this version means an array property must be a byte array, as it always had to be
