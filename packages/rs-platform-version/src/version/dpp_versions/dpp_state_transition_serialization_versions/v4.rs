//! V4 (PV15): identical to V3 except the contract update transition admits
//! the delta-based V1 form (`DataContractUpdateTransitionV1`) and defaults to
//! it: `contract_update_state_transition` bounds become 0..=1 with default 1.
//! Everything else keeps V3's bounds.
...
pub const STATE_TRANSITION_SERIALIZATION_VERSIONS_V4: DPPStateTransitionSerializationVersions =
    DPPStateTransitionSerializationVersions {
        ...
        contract_update_state_transition: FeatureVersionBounds {
            min_version: 0,
            max_version: 1, // changed: delta-based (V1) contract updates join the wire
            default_current_version: 1, // changed: clients send deltas by default
        },
