...
/// Drive version 10.
/// Introduced in protocol v17, the 5.0 protocol version, for the per-issuer
/// token lifecycle ledger: every contract that issues tokens carries a supply
/// rollup under `[Tokens] / 224 / contract_id`, and a destroyed issuer moves
/// that rollup into the destroyed supply scalar at `[Tokens] / 224 / [0]`
/// without visiting a holder or a token.
///
/// * **Genesis**: `initialization.create_initial_state_structure` 3 -> 4
///   creates the lifecycle subtree, its zero destroyed-supply scalar and its
///   cleanup queue tree with sequential inserts; the upgrade path
///   (`Platform::transition_to_version_17`) creates the same elements with
///   insert-if-not-exists and then backfills one record per issuer after
///   reconciling every token's supply with its balance sum.
/// * **Supply writes**: `DRIVE_TOKEN_METHOD_VERSIONS_V2` bumps
///   `create_token_trees`, `add_to_token_total_supply` and
...
pub const DRIVE_VERSION_V10: DriveVersion = DriveVersion {
    ...
    methods: DriveMethodVersions {
        initialization: DriveInitializationMethodVersions {
            create_initial_state_structure: 4, // changed in v10: adds the token lifecycle ledger under [Tokens]
        },
        ...
        token: DRIVE_TOKEN_METHOD_VERSIONS_V2, // changed in v10: issuer supply rollups, destroyed-issuer ledger and the lifecycle method group
