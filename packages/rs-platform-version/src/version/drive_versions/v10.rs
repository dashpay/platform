...
/// Drive version 10. Introduced in protocol version 15 with token shielded pools:
/// `create_initial_state_structure` 4 adds the token shielded pools BigSumTree under the
/// Tokens root at genesis, and `DRIVE_TOKEN_METHOD_VERSIONS_V2` carries the pool operations
/// (pool creation, shield, unshield, shielded transfer, mint to and burn from the pool) plus
/// `calculate_total_tokens_balance` v1, which adds the pool balances to the conservation
/// check.
///
...
pub const DRIVE_VERSION_V10: DriveVersion = DriveVersion {
    ...
    methods: DriveMethodVersions {
        initialization: DriveInitializationMethodVersions {
            create_initial_state_structure: 4, // changed in v10: adds the token shielded pools BigSumTree under the Tokens root (v8 added the credit shielded pool trees)
        },
        ...
        document: DRIVE_DOCUMENT_METHOD_VERSIONS_V4, // unchanged since v9: v2 index walkers + v1 update walker (shared-prefix aggregate indexes become insertable) and the detect_ranked_mode slot
        vote: DRIVE_VOTE_METHOD_VERSIONS_V2,
        contract: DRIVE_CONTRACT_METHOD_VERSIONS_V4, // unchanged since v9: add_contract_to_storage v1 writes the contract version item beside the contract
        ...
        identity: DRIVE_IDENTITY_METHOD_VERSIONS_V2, // unchanged since v9: v1 withdrawal-by-transaction-index query builder (structural, identical lowering)
        token: DRIVE_TOKEN_METHOD_VERSIONS_V2, // changed in v10: token shielded pools (pool creation + shield / unshield / shielded transfer / mint to / burn from ops, conservation v1)
    ...
    // unchanged since v9: GROVE_V4 activates the indexed-tree batch cleanup
