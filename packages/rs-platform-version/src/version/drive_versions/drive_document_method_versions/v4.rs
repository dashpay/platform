...
/// V4 is introduced in protocol version 14 for contract-level ranked
/// aggregates. It is byte-identical to V3 today: every slot carries the
/// same value, including `query.detect_ranked_mode: 0`.
///
/// The table exists so the ranked query surface has a v14-gated home to
/// bump into without disturbing V3, which stays selected by protocol
/// versions 12 and 13. Nothing in V4 is reachable differently from V3
/// until the ranked routing lands and flips `detect_ranked_mode`.
