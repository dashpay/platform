...
/// * Document expiry (protocol version 14): a document type may declare a `ttl` of at most
///   one year (`max_document_ttl_seconds`), and the platform deletes at most 128 expired
///   documents per block (`max_document_expirations_per_block`), both joined this table in
///   place while protocol version 14 was unreleased. `None` in the earlier tables, whose
///   parsers never read the keyword and whose blocks never run the cleanup.
...
pub const SYSTEM_LIMITS_V4: SystemLimits = SystemLimits {
    ...
    max_document_expirations_per_block: Some(128), // document ttl (new in v14): expired documents deleted per block
