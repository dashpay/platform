...
pub struct SystemLimits {
    ...
    /// prepaid storage of such a document can live, which is what the per-epoch price of
    ...
    ///
    /// `None` on protocol versions that predate document expiry (the event does not run).
    pub max_document_expirations_per_block: Option<u16>,
