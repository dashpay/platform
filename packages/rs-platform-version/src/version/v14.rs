...
/// 6. **Deterministic token reward math**: `TOKEN_VERSIONS_V3` bumps
///    `distribution_function_evaluate_version` 0 → 1, so the Polynomial,
///    Exponential, Logarithmic and InvertedLogarithmic perpetual
///    distribution functions evaluate `pow` / `exp` / `ln` through the
///    pinned `libm` crate instead of the host's std `f64` methods, which
///    differ across CPU architectures and libc versions by up to one ulp —
///    enough to flip the truncating integer casts and pay different token
///    amounts on different nodes for the same claim. v13 keeps
///    `TOKEN_VERSIONS_V2` (version 0, std math), so pre-v14 blocks replay
///    byte-for-byte on the nodes that produced them.
///
...
pub const PLATFORM_V14: PlatformVersion = PlatformVersion {
    ...
    dpp: DPPVersion {
        ...
        token_versions: TOKEN_VERSIONS_V3, // changed: distribution_function_evaluate_version 1 — libm reward math
...
mod tests {
    ...
    /// Deterministic (libm) distribution function evaluation activates at v14
    /// and nowhere earlier: the v13 half guards replay of already-committed
    /// blocks, the v14 half guards against the activation silently being lost
    /// (a `TOKEN_VERSIONS_V*` copy-paste that keeps version 0 would otherwise
    /// compile and pass every other test).
    #[test]
    fn deterministic_distribution_function_evaluation_activates_at_v14() {
        assert_eq!(
            PLATFORM_V13
                .dpp
                .token_versions
