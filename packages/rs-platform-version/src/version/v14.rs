...
/// 18. **The app-connect system contract**: `SystemDataContract::AppConnect`
///     (`app_connect: 1` in `SYSTEM_DATA_CONTRACT_VERSIONS_V3`) is a new
///     persisted system contract carrying the wallet-to-app login handshake,
///     the wallet's encrypted `loginKeyResponse` and the app's `appManifest`.
///     Fresh chains register it at genesis (`create_genesis_state` 2,
///     `DRIVE_ABCI_METHOD_VERSIONS_V10`); chains upgrading from 13 receive it
///     from `transition_to_version_14`, which inserts it beside the DashPay v2
///     and withdrawals v2 rewrites. Below 14 it does not exist in state and
///     the system contract cache reports it absent.
///
