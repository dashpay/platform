...
///
/// 47. **Documents with a time to live**: the doctype-level `ttl` keyword (meta-schema v3,
...
///     `indexOnly` and a contested index, is at most `max_document_ttl_seconds` (one year)
///     at registration, and is fixed on update (`validate_update` 1). References treat such
///     a type as deletable. Its documents are stored without storage flags, indexed in the
///     documents expirations tree under `Misc` (created by `create_initial_state_structure`
///     4 and `transition_to_version_14`), and pay the `document_ttl` group of
///     `FEE_VERSION3`: a tiered price per byte for the time they live, into the processing
///     fees under two epochs and the storage pool above, plus their deletion prepaid as
///     processing. From its expiry on, a document can no longer be replaced,
///     transferred, bought, repriced or restored by a moderator (`DocumentExpiredError`,
///     40140), judged from its own `$createdAt`; its owner may still delete it. See
