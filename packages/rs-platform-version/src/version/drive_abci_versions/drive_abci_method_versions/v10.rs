...
/// Drive ABCI method versions 10. Introduced in protocol v14: bumps
/// `pool_withdrawals_into_transactions_queue` to 2 so the withdrawal limit mirrors Core's
/// credit pool rule on the balances Core reports at the chain locked height, turns off
/// `cleanup_expired_locks_of_withdrawal_amounts` (`None`) because withdrawals now count while
/// in flight, keyed by transaction index, instead of expiring by time, and bumps
/// `rebroadcast_expired_withdrawal_documents` to 2 so an expired withdrawal whose payout is
/// below Core's dust threshold is marked FAILED instead of re-signed forever.
...
pub const DRIVE_ABCI_METHOD_VERSIONS_V10: DriveAbciMethodVersions = DriveAbciMethodVersions {
    ...
    withdrawals: DriveAbciIdentityCreditWithdrawalMethodVersions {
        ...
        pool_withdrawals_into_transactions_queue: 2, // changed in v14: the limit mirrors Core's credit pool rule on the balances Core reports at the chain locked height
        ...
        cleanup_expired_locks_of_withdrawal_amounts: None, // gone in v14: withdrawals count while in flight, keyed by transaction index, so nothing expires by time
    },
