...
//
// Identical to DRIVE_ABCI_METHOD_VERSIONS_V10 (the protocol-v13 method set)
// except ONE field: `validate_fees_of_event` 0 -> 1. The v1 drops the
// per-transition GroveDB cost-estimate gate from `PaidFromAssetLockToPool`
// (the `ShieldFromAssetLock` path). The pool fee that transition books is the
// flat `compute_minimum_shielded_fee(num_actions) + asset-lock base cost` —
// the same kind of flat, epoch-amortized fee a `ShieldedTransfer`/`Unshield`
// carries, and those pool-paid events were never estimate-gated. Under
// GROVE_V4 the commitment-tree estimator is a per-append UPPER BOUND (a full
// epoch compaction and dense recompute on every note), so the v0 gate
// `fee >= estimate` degenerated into a fixed comparison of two consensus
// constants that rejected every ShieldFromAssetLock. Everything else —
// `PaidFromAddressInputs` (the transparent `Shield`) included — is unchanged.
// Admission changes, so the bump MUST stay inactive on v13 (V9 keeps it 0).
