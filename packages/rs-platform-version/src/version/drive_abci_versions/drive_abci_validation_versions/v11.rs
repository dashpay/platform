...
// PROTOCOL_VERSION_15: token shielded pools. The batch validators of the token pool
// transitions (`token_shield`, `token_unshield`, `token_shielded_transfer`,
// `token_mint_to_pool`, `token_burn_from_pool`, `token_claim_to_pool` and
// `token_direct_purchase_to_pool`) are reachable from this version on; the transitions
// themselves are gated by `TOKEN_SHIELDED_POOL_INITIAL_PROTOCOL_VERSION` in
// `is_allowed`. v10 remains unchanged for PROTOCOL_VERSION_14 chain replay.
    ...
    DriveAbciValidationVersions {
        state_transitions: DriveAbciStateTransitionValidationVersions {
            ...
            batch_state_transition: DriveAbciDocumentsStateTransitionValidationVersions {
                ...
                document_base_transition_state_validation: 1, // changed: a token cost paid from the token's shielded pool skips the owner's balance checks
