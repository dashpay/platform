...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Fetching contested DPNS usernames for identity: {}\n", identity_id_str);
    
    // Get non-resolved contests for this identity
    let identity_contests = sdk.get_non_resolved_dpns_contests_for_identity(
        identity_id.clone(),
    ...
    println!("Identity {} is contending in {} unresolved contests:\n", 
        identity_id_str, identity_contests.len());
    ...
    for (name, contest_info) in &identity_contests {
        ...
        println!("   Total contenders: {}", contest_info.contenders.contenders.len());
    ...
    println!("═══════════════════════════════════════════════════════");
    println!("Summary:");
    println!("  • Identity is competing for {} username(s)", identity_contests.len());
