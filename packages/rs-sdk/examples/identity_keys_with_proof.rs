...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("4. Block metadata: height={}, timestamp={}", 
        proof_data.metadata.height, 
        proof_data.metadata.time_ms
    );
    ...
    println!("Specific keys proof size: {} bytes", specific_proof_data.grovedb_proof.len());
    
    // Process the specific keys
    for (key_id, maybe_key) in specific_keys.iter() {
        if let Some(key) = maybe_key {
            println!("  Key ID {}: Type={:?}, Purpose={:?}", 
                key_id, key.key_type(), key.purpose());
        }
    }
