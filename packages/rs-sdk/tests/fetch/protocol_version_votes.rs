...
/// Given protxhash with only zeros, when I fetch protocol version vote_choices for nodes, I can retrieve them.
...
async fn test_protocol_version_votes_zeros() {
    ...
    let votings = MasternodeProtocolVote::fetch_many(&sdk, starting_protxhash)
        .await
        .expect("fetch protocol version vote_choices by node");
...
/// Given protxhash with only zeros, when I fetcg protocol version vote_choices for nodes with limit 2, I get exactly 2 items.
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
...
/// Given protxhash with only `0xFF`s, when I fetch protocol version vote_choices for nodes, I get nothing.
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
...
/// Given None as a  protxhash, when I fetch protocol version vote_choices for 0 nodes, I get error.
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
