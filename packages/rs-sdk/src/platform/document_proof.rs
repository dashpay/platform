...
pub fn extract_merkle_path_from_grove_proof(proof_bytes: &[u8]) -> Result<Vec<([u8; 32], bool)>, Error> {
    ...
    Err(Error::Generic("No valid Merk proofs found in GroveDB proof".to_string()))
}
    ...
    while offset < proof_bytes.len() {
        ...
        match op {
            ...
            0x12 => {
                ...
                let (_, bytes_read) = decode_varint(&proof_bytes[offset..])
                    .unwrap_or((0, 0));
                offset += bytes_read;
            }
...
pub async fn fetch_document_with_proof(
    ...
    use super::{DocumentQuery, Document};
