...
/// Container type for results with proof data for Document fetch_many.
pub type DocumentsWithProofData = (drive_proof_verifier::types::Documents, ProofData);

/// Extension trait to add fetch_many_with_proofs to Document.
#[async_trait::async_trait]
pub trait DocumentFetchManyWithProofs {
    ...
    ///
    /// * `sdk` - SDK instance to use for the request
    /// * `query` - Query parameters for fetching the documents
    ///
    /// # Returns
    ///
    /// Returns the fetched documents along with proof data.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    /// use dash_sdk::{Sdk, platform::{Document, DocumentQuery, DocumentFetchManyWithProofs}};
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let sdk = Sdk::new_mock();
    ...
    ///
    /// // Process documents with proof data...
    /// # Ok(())
    /// # }
    /// ```
    async fn fetch_many_with_proofs<Q: super::Query<super::DocumentQuery>>(
        sdk: &crate::Sdk,
        query: Q,
    ) -> Result<DocumentsWithProofData, crate::Error>;
}

// Implement DocumentFetchManyWithProofs for Document
#[async_trait::async_trait]
impl DocumentFetchManyWithProofs for dpp::document::Document {
    async fn fetch_many_with_proofs<Q: super::Query<super::DocumentQuery>>(
        sdk: &crate::Sdk,
        query: Q,
    ) -> Result<DocumentsWithProofData, crate::Error> {
        let (documents, metadata, proof) =
            <Self as super::FetchMany<
                dpp::identifier::Identifier,
                drive_proof_verifier::types::Documents,
            >>::fetch_many_with_metadata_and_proof(sdk, query, None)
            .await?;
        let proof_data = ProofData::new(proof, metadata);
        Ok((documents, proof_data))
    }
}
