...
use drive_proof_verifier::types::{
    ContestedResource, Contenders, VotePollsGroupedByTimestamp,
};
...
impl Sdk {
    ...
    pub async fn get_contested_non_resolved_usernames(
        ...
        let current_contests = self.get_current_dpns_contests(None, None, Some(100)).await?;
            ...
            match self.get_contested_dpns_vote_state(&name, None).await {
                Ok(contenders) => {
                    ...
                    if contenders.winner.is_none() {
                        non_resolved_names.insert(name, ContestInfo {
                            contenders,
                            end_time,
                        });
                    }
    ...
    pub async fn get_non_resolved_dpns_contests_for_identity(
        ...
        for (name, contest_info) in all_non_resolved {
            ...
            let is_contender = contest_info.contenders.contenders.iter()
...
mod tests {
    ...
    async fn test_get_current_dpns_contests() {
        ...
        let future_contests = sdk.get_current_dpns_contests(
            Some(current_time),
            None,
            Some(5)
        ).await;
        ...
        match paginated_contests {
            Ok(contests) => {
                println!("✅ Pagination test completed, fetched {} contested names", contests.len());
        ...
        if let Ok(all_contests) = all_contests {
            if !all_contests.is_empty() {
                ...
                println!("\n4. Testing with time range [{}, {}]...", start_time, end_time);
                let range_contests = sdk.get_current_dpns_contests(
                    Some(start_time),
                    Some(end_time),
                    Some(10)
                ).await;
    ...
    async fn test_get_contested_non_resolved_usernames() {
        ...
        match &non_resolved_names {
            Ok(names_map) => {
                println!("✅ Successfully fetched {} contested non-resolved usernames", names_map.len());
                
                // Display the first few names with their contenders
                for (i, (name, contest_info)) in names_map.iter().enumerate().take(10) {
                    println!("  {}. '{}' has {} contenders (ends at {} ms)", 
                        i + 1, name, contest_info.contenders.contenders.len(), contest_info.end_time);
                ...
                for (name, contest_info) in names_map {
                    assert!(contest_info.contenders.winner.is_none(), 
                        "Name '{}' should not have a winner (it's supposed to be unresolved)", name);
                }
        ...
        if let (Ok(non_resolved), Ok(contests)) = (&non_resolved_names, &current_contests) {
            ...
            for name in contest_names.iter().take(3) {
                if non_resolved.contains_key(name) {
                    ...
                    println!("  ⚠️ '{}' in current contests but not in non-resolved", name);
                }
            }
        }
        ...
        match non_resolved {
            Ok(contests) if !contests.is_empty() => {
                ...
                if let Some((test_identity, _)) = first_contest.contenders.contenders.iter().next() {
                    println!("Found test identity {} in contest '{}'", 
                    ...
                    match identity_contests {
                        Ok(contests_map) => {
                            ...
                            for (name, contest_info) in &contests_map {
                                ...
                                assert!(is_contender, 
                                    "Identity should be a contender in contest '{}'", name);
                                
                                println!("  - '{}' ({} contenders total, ends at {})", 
                                    name, contest_info.contenders.contenders.len(), contest_info.end_time);
                            }
                            
                            // The first contest should definitely be in the results
                            assert!(contests_map.contains_key(first_name),
                    ...
                    let empty_contests = sdk.get_non_resolved_dpns_contests_for_identity(
                        random_id,
                        Some(10)
                    ).await;
