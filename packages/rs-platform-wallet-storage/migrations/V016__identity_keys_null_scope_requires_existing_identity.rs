...
//!
//! V008's trigger pair aborts a NULL-scoped key whose identity is
...
//!
//! Recreated rather than edited into V008: refinery never re-runs an
//! applied migration, so editing V008 would tighten only freshly created
//! databases and leave every existing wallet on the permissive trigger.
//! `V012__drop_core_utxo_metadata` sets the same precedent.
//!
