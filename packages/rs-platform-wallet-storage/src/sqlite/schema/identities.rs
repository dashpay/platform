...
pub fn load_state(
    ...
    let wallet_id_param = wallet_id_to_param(wallet_id);
    let mut stmt = conn.prepare(
        "SELECT identity_id, length(entry_blob), entry_blob \
...
/// absent owner is corruption rather than routine bookkeeping — a
/// NULL-scoped `identity_keys` row naming an identity that never existed
/// is the reachable shape, since the compound FK is dormant there.
