...
//! V009 unified-migration schema tests. The unified migration sequences after
//! the seven migrations `v4.2-dev` already ships (V001-V007), whose version
...
#[test]
fn v009_is_embedded_and_supported() {
    assert!(
        mig::embedded_migrations().iter().any(|(v, _)| *v == 9),
        "V009 must be in the embedded migration set"
    );
    assert!(mig::max_supported_version() >= 9, "V009 must be applicable");
}

/// A fresh store applies V009 and migrates clean through to the
/// newest embedded migration (e.g. V004's DIP-13 invitations table), and
/// every V009 table exists.
#[test]
fn fresh_store_applies_unified_migration() {
    let (persister, _tmp, _path) = fresh_persister();
    let conn = persister.lock_conn_for_test();
    let applied: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM refinery_schema_history WHERE version = 9",
            [],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(applied, 1, "a fresh store must apply V009");
    let max: i64 = conn
        .query_row(
