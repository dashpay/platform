...
- Subscriber channels should be bounded (`tokio::mpsc::Sender`) so back-pressure from slow consumers propagates upstream. The design must allow tweaking capacity and drop policy per use case.
...
- `Subscription` holds a `filter: F` and an `mpsc::Sender<E>` with a configurable capacity.
