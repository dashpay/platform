...
# -m 2 flag in first grep fixing incorrect behaviour on grep v2.5.1
# -m 1 flag in second grep is to pick only first occurrence on correctly working versions of the grep
MASTERNODE_REWARD_SHARES_OWNER_PRIVATE_KEY=$(grep -m 2 -A 2 "Create a new owner addresses" "${SETUP_FILE_PATH}" | grep -m 1 "Private key" | awk  '{printf $4}')
