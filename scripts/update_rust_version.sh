...
echo Update the rust version in the Dockerfile
sed -i "s/^ARG RUST_VERSION=.*$/ARG RUST_VERSION=${VERSION}/" "${PROJECT_ROOT}/Dockerfile"
check "${PROJECT_ROOT}/Dockerfile"

echo Update the rust version in Github Actions
yq eval ".inputs.toolchain.default = \"${VERSION}\"" -i "${PROJECT_ROOT}/.github/actions/rust/action.yaml"
check "${PROJECT_ROOT}/.github/actions/rust/action.yaml"
