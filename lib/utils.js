...
const validProofOfWork = function (header) {
  const target = DashUtil.expandTarget(header.bits);
  const hash = header._getHash().reverse(); // replacement for require(bufer-reverse).reverse()
...
module.exports = {
  ...
  _normalizeHeader(_b) {
    const _el = JSON.parse(JSON.stringify(_b));
    let bh = {};
