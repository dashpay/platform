...
pub enum ContractModerators {
    ...
    Elected(ElectedModerators),           // see Elected Moderation below
}
...
pub struct ElectedModerators {
    ...
    pub abilities: BTreeSet<ModerationAbility>,          // DeleteDocuments, Ban, Suspend, Warn
    pub moderators_action_fee_maximums: BTreeMap<DocumentName, ModeratorsActionFeeMaximums>,
    pub interim: InterimModerators,       // ContractOwner, AppointedModerators(set), NotYetUsable
    pub owner_protected: bool,            // false by default
}
```

The declaration lives in `packages/rs-dpp/src/data_contract/config/moderation/elected.rs`. On the wire it is the third `$type` of the moderators, flat: `{"$type": "elected", "challengeCoolDown": 1209600, "moderatedDocumentTypes": ["post"], "abilities": ["ban", "deleteDocuments"], "moderatorsActionFeeMaximums": {"post": {"create": 1000}}, "interim": {"$type": "notYetUsable"}}`, with `joinWindow`, `voteWindow` and `ownerProtected` optional. Its parts:
...
- **The moderated set** is the document types the team moderates: non-empty, each a document type of the contract. The charter of a team will say how those types are moderated, never which. A type in the set need not be flagged `canBeDeletedByModerators`: deletions reach only the flagged types, within each type's window. Bans and suspensions stay contract-wide whatever the set says; it bounds deletions and the interim block.
- **The charter envelope** is what a charter may claim: the abilities (non-empty, and each backed by the contract: `ban` needs the banlist, `suspend` the suspension list, `warn` the warning list, `deleteDocuments` a flagged type), and the most a charter may charge the moderators part of each document action, by document type and action key of `actionFees`, in the units the type's `actionFees` are declared in. A type or an action left out allows nothing; a maximum names a document type of the contract, prices at least one action, and every amount is non-zero and at most `MAX_CREDITS`. The owner part of every action stays what the type's `actionFees` declare, immutable as before. Nothing reads the maximums yet: a document type's own `moderators` amounts are what is charged until charters price them.
- **The interim** says who moderates until a team is seated. `ContractOwner` and `AppointedModerators(set)` are the merged kinds, with their authority, their limit and their existence check (41110 at create): they moderate, they are protected, and they are the team that claims the moderators pot. `NotYetUsable` names nobody: nobody moderates, nobody claims the pot (it accumulates for the team to come, `ContractFeeClaimNotAllowedError` for everyone), and the moderated document types can not be used. A contract that never attracts a team keeps those types unusable for good; the other types work as on an unmoderated contract.
...
**Validation.** `validate_moderation_config` v0 checks the declaration with the rest of the moderation config, from the raw document schemas of the create or update transition, and refuses with `InvalidContractModerationConfigError` (10900, unpaid): a window or the cool-down outside its bounds, an empty moderated set or one naming a type the contract does not have, an empty envelope or an ability the contract can not back, a maximum naming an unknown type, pricing no action, or with a zero or overflowing amount, and an empty or oversized interim set. The create then checks that every interim moderator exists (41110), as it does for an appointed set.
