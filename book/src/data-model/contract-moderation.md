...
**The deleted document's owner gets no storage refund.** The batch carries `ContractModerationOperationType::ForfeitStorageRefunds`, a marker that writes nothing, and `apply_drive_operations` generation 1 turns every removal such a batch attributes to an identity into a removal attributed to nobody: the bytes still leave the system (`FeeResult::removed_bytes_from_system`), no refund is computed, and the credits stay in the storage pools they were distributed to when the document was written. An estimate carries no refund to begin with (refunds come from the flags of what is really removed), so the mempool's fee check is the same with or without the forfeiture. One identity is spared: when the deletion replaces the record of an earlier removal with a shorter one, the moderator who paid for that record is still refunded what it frees (`ForfeitStorageRefunds { except }`, read from the record being replaced). The rule names who is spared rather than who forfeits because a document's storage flags may still name an earlier owner after a transfer, and the ruling is that nobody is refunded for the document. An author who deletes the same document with an ordinary document transition is refunded as always.
