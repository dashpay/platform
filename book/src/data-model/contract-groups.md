...
The `Members` side is laid out so that "is document type `D` of contract `C` in group `G`" is one point lookup at `[124, 1, C, 1, D, G]`, and the forward side answers it at `[124, 0, G, 2, C || D]`. That is the shape `ContractBounds::ContractGroup` on identity keys needs: for every member of a batch, one existence check under the contract the member belongs to. Today the batch transformer reads the contract's whole `Members` entry (at most `max_contract_group_memberships_per_contract` entries) into the action and the check runs in memory; the point lookup is there when a consumer wants it.
