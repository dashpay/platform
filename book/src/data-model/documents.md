...
The rule is judged only when property data is written. Transfers, price updates and purchases carry no property data and are not judged, so a transfer or purchase that hands a document to the identity a `$ownerId`-distinct property names is accepted and leaves the stored document in the state the declaration forbids until the new owner replaces it. A document type that needs the property to stay distinct from the owner across ownership changes must keep its documents non-transferable and non-tradeable.
