...
From protocol version 14 a document batch's proof also carries the credit
balance of the batch's owner, read from the same state as the document, so a
wallet learns what the write left it with without a second query.
`VerifiedDocuments` holds the documents and that balance (`None` for a proof
