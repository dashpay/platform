...
The workspace version (the 4.2 development line, `4.2.0-dev.N` in the root
`Cargo.toml`; Rust edition 2021, MSRV 1.92) is shared by all member crates
through `version.workspace = true`.
...
| **System contracts** | `dpns-contract`, `dashpay-contract`, `withdrawals-contract`, `masternode-reward-shares-contract`, `wallet-utils-contract`, `token-history-contract`, `document-history-contract`, `keyword-search-contract`, `data-contracts` |
