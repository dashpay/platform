...
| Token creation | `create_token_trees` v1 | Inserts a zero record if the contract has none and refuses a destroyed issuer, scanning the batch it is handed so several tokens created together insert the record once; a record the batch already wrote is checked from the pending write, so a destruction in the same batch refuses the token too. |
