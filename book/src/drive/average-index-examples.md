...
> **Status:** the [`document_average_worst_case`](https://github.com/dashpay/platform/blob/v3.1-dev/packages/rs-drive/benches/document_average_worst_case.rs) bench lands the reproducible numbers below — same convention as the [Count](./count-index-examples.md) and [Sum](./sum-index-examples.md) chapters. All proof sizes are measured against a 31 620-grade fixture; verified `(count, sum)` values are the actual numbers the bench's matrix reports. One known write-side gap (the `documentsCountable` + `documentsSummable` primary-key tree not promoting to `CountSumTree` on the doctype root) is called out in Query 1 — the same family of bug we hit on the sum chapter's [Query 1](./sum-index-examples.md#query-1--unfiltered-total-sum). Everything else — point lookups, range aggregates, and both carrier variants — is wired through to grovedb PR #670's verifiers end-to-end.
...
> ⚠ **Known write-side gap.** The verified terminator here is currently `Tree (NormalTree)` with both `count_value` and `sum_value` defaulting to `0`, **not** the expected `CountSumTree { count: 10000, sum: ≈499265 }`. The proof itself is well-formed (614 bytes verifies against the same merk root the other queries share) and 24.2 µs is the right shape for a single-element merk path; what's broken is that **`documentsCountable: true` + `documentsSummable: "score"` together aren't propagating either metric into the primary-key tree at insert time**. The bench probes `[probe] byClass: ... CountSumTree { count=1000, sum=49118 }` correctly, so per-index aggregation works — only the top-level doctype-aggregation isn't populating. Same class of bug as the sum chapter's [Query 1](./sum-index-examples.md#query-1--unfiltered-total-sum); tracked as a separate write-side fix. The read path documented here will start returning the correct `(10000, ≈499265)` once the writer fix lands. No proof-shape change needed.
...
The chapter is now grounded in the [`document_average_worst_case`](https://github.com/dashpay/platform/blob/v3.1-dev/packages/rs-drive/benches/document_average_worst_case.rs) bench's measured numbers — Q2–Q7 verify cleanly end-to-end against the same fixture's root hash. Two known follow-ups remain:

1. **Q1's documentsCountable + documentsSummable write-side gap** (the doctype primary-key tree not promoting to `CountSumTree` at insert time). The read path is sound — the proof shape and 614-byte size are right; only the stored `(count, sum)` is zero. Same family of bug we hit on the sum chapter's [Query 1](./sum-index-examples.md#query-1--unfiltered-total-sum); both will lift once a single writer-side fix lands.
2. **A worked example of "exact-precision" averages** — for callers that need fractional averages (e.g. `avg = 50.7142857…` rather than `50.99`), the protocol-level approach is to return `(count, sum)` and let the client compute in its preferred numeric format (the chapter notes this in [Numerical Considerations](#numerical-considerations) above, but a future expansion could walk through the fixed-point vs. floating-point trade-offs).
